...
local function tryLogin(charInfo, tries)
  ...
  CharacterList.hide()
  g_game.loginWorld(G.account, G.password, charInfo.worldName, charInfo.worldHost, charInfo.worldPort, charInfo.characterName, G.authenticatorToken, G.sessionKey, "Default", characterPicked)
...
function showTravelCharacter(roomName)
	charactersWindow:setVisible(true)
	for i,child in pairs(characterPick:getChildren()) do
		local widgetName = child:getChildById('name')
		child:recursiveGetChildById('pick').onClick = function()
			sendAction(AUTOTRAVEL_OPCODE, "teleportTo", {
				room = roomName,
				vocation = widgetName:getText(),
			})
			charactersWindow:setVisible(false)
		end
	end
end
...
function CharacterList.create(characters, account, otui)
  ...
  characterPick = charactersWindow:getChildById('characterPick')
  characterPick:destroyChildren()

  local getVocations = {
    
		["Kokao Warrior"] = {avatar="/images/game/characters/kokao_warrior", description="A pure beast made of steel, trained in hell and spitting lava like a volcano out of its angry eyes... Well, that's what it should be...", attributes={
      ["TANK"] = "/images/game/attributes/Tank",
			["COMBATANT"] = "/images/game/attributes/Combatant"
		}},
		["Kaer Disciple"] = {avatar="/images/game/characters/kaer_disciple", description="No one knows where the magic comes from, not even them. The path of The Wisdom is long and winding. Luckily they're still young and brave.", attributes={
      ["ARCANE WISDOM"] = "/images/game/attributes/Arcane-Wisdom",
			["DPS"] = "/images/game/attributes/DPS"
		}},
		["Jade Mystic"] = {avatar="/images/game/characters/jade_mystic", description="Son of the Moon and the Sun, bla bla bla, lots of hippie bullshit. They'd better be prepared to fight, or to die in peace & harmony with a sword through the chest.", attributes={
			["SUMMONER"] = "/images/game/attributes/Summoner",
			["SUPPORT"] = "/images/game/attributes/Support",
			["AREA CONTROL"] = "/images/game/attributes/Area-Control"
		}},
		["Shepherd of Aureh"] = {avatar="/images/game/characters/shepherd_of_aureh", description="Tired of running after his fracking sheep and goats every single day of his life, he learned to use the Sling. Or so we hope it does.", attributes={
			["AREA CONTROL"] = "/images/game/attributes/Area-Control",
      ["DISTANCE FIGHTER"] = "/images/game/attributes/Distance",
			["DPS"] = "/images/game/attributes/DPS"
		}},
  }
  
  for i,child in pairs(getVocations) do
	
    local widget = g_ui.createWidget('CharacterPicker', characterPick)
  local widgetInfo = widget:recursiveGetChildById('info')
  local widgetAvatar = widget:recursiveGetChildById('avatar')
  local widgetAttributes = widget:recursiveGetChildById('icons')
  
  
  local widgetName = widget:getChildById('name')
   widget:recursiveGetChildById('pick').onClick = function()
		characterPicked = i
		CharacterList.doLogin() 
		charactersWindow:setVisible(false)
	end
	
 widgetName:setText(i)
  widgetInfo:setText(child.description)
  widgetAvatar:setImageSource(child.avatar)
  widgetAttributes:destroyChildren()
  
  for _i,_child in pairs(child.attributes) do
    local attribute = g_ui.createWidget('AttributeWidget', widgetAttributes)
		local attributeIcon = attribute:recursiveGetChildById('icon')
		attributeIcon:setImageSource(_child)
		local attributeName = attribute:recursiveGetChildById('name')
		attributeName:setText(_i)
	end
  end
