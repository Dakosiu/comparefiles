...
function displayRewards(t)
    for i = 0,15 do
	    setRewardItem(i, 0, false)
	end
	if not t then
	    return false
	end
	local containerItem = 0
	for i, v in ipairs(t) do
	    setRewardItem(containerItem, v.id, v.count)
...
function getChestAmmount(parent)
    local widget = parent:getChildById('arrowPanel')
	return tonumber(widget:getText())
end

function removeChestAmmount(parent)
    local widget = parent:getChildById('arrowPanel')
...
function setChestAmmount(parent, value)
    local widget = parent:getChildById('arrowPanel')
	widget:setText(value)
end    

function addChestAmmount(parent, count)
    local widget = parent:getChildById('arrowPanel')
    local value = tonumber(widget:getText())
	value = value + 1
	if value > count then
...
function onMainContainerOpen(container, previousContainer)
  ...
  for i,child in pairs(getInfo.chests) do
	...
	local hoverWidget = getChest:getChildById('hover')
	local buttonWidget = getChest:getChildById('button')
	local ammountWidget = getChest:getChildById('ammount')
	
	local arrowLeftWidget = getChest:getChildById('ArrowLeft')
	arrowLeftWidget.onClick = function()
	    local count = getChestAmmount(getChest)
		local player_ammount = tonumber(getNumbersFromText(ammountWidget:getText()))
		--print("Player Ammount: " .. player_ammount)
		if count <= 0 or player_ammount <= 0 then
		    print("Nie mozna dac w lewo.")
			return
		end
		print("Dziala")
		removeChestAmmount(getChest)
	end
	
	local arrowRightWidget = getChest:getChildById('ArrowRight')
	arrowRightWidget.onClick = function()
	    local count = getChestAmmount(getChest)
		local player_ammount = tonumber(getNumbersFromText(ammountWidget:getText()))
		print("Player Ammount: " .. player_ammount)
		print("Widget Count: " .. count)
		if count >= player_ammount or player_ammount <= 0 then
		    print("Nie mozna dac w prawo.")
			return
		end
		print("Dziala")
		addChestAmmount(getChest, player_ammount)
	end
	...
	ammountWidget:setText("AMOUNT: "..child.ammount)
	getChest:getChildById('button').onClick = function()
		sendAction('openChest', {
			id = i,
			count = getChestAmmount(getChest)
		})
		local value = getNumbersFromText(ammountWidget:getText())
		if value then
		    value = tonumber(value)
			if value > 0 then
			    value = value - 1
				ammountWidget:setText("AMOUNT: " .. value)
				setChestAmmount(getChest, 0)
			end
		end
		
	end
	hoverWidget.onHoverChange = function()
	    --displayRewards()
		if selectedWidget then
		    selectedWidget:setBackgroundColor("#c1d4a7")
		end
	    hoverWidget:setBackgroundColor("#CDDCB8")
		--selectedWidget = hoverWidget 
		--displayRewards(child.materials)
	end
	buttonWidget.onHoverChange = function()
	    --displayRewards()
		if selectedWidget then
		    selectedWidget:setBackgroundColor("#c1d4a7")
		end
...
function setRewardItem(index, id, count)
    if not count then
	    count = 1
	end
	
    local panel = containerWindow:recursiveGetChildById('rewardsPanel')
	--local itemPanel = containerWindow:recursiveGetChildById('Item')
