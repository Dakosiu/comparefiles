...
function togglePurse()
  local widget = modules.game_interface.getNewUI():recursiveGetChildById('inventory')
...
function check()
  if modules.client_options.getOption('autoChaseOverride') then
	...
	local player = g_game.getLocalPlayer()
	local inPz = false
  local bitsChanged = bit32.bxor(player:getStates(), 0)
  for i = 1, 32 do
    ...
    if bitChanged == PlayerStates.Pz then
	  ...
	  if bit.band(player:getStates(), PlayerStates.Pz) > 0 and modules.game_interface.returnHotkeysPanel():isVisible() then
		modules.game_console.enableChat()
		modules.game_console.show()
		modules.game_interface.returnHotkeysPanel():setVisible(false)
	  elseif bit.band(player:getStates(), PlayerStates.Pz) <= 0 and not modules.game_interface.returnHotkeysPanel():isVisible() then
		modules.game_interface.returnHotkeysPanel():setVisible(true)
		modules.game_console.hide()
...
function online()
  ...
  update()
  scheduleEvent(check,0)
  scheduleEvent(check,50)
  scheduleEvent(check,500)
end
