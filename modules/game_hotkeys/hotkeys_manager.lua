...
function init()
  ...
  ProtocolGame.registerExtendedOpcode(204, onHotkeysJSONOpcode)

  scheduleEvent(function() load() end, 100)
end
...
function load(forceDefaults)
  ...
  if SURVIVAL_MODE then
	...
	modules.game_interface.returnHotkeysPanel():destroyChildren()
	local getSpells = {}
	for i,child in pairs(Wikipedia.Spells) do
		for _i,_child in pairs(child.vocation) do
			if _child:lower() == vocationMap[g_game.getLocalPlayer():getVocation()]:lower() then
				table.insert(getSpells, child.word)
			end
		end
	end
	hotkeys = {}
	...
	for i,child in pairs(getSpells) do
		index = index + 1
		hotkeys[index] = {
			  keyCombo = tostring(i),
			  autoSend = true,
  ...
  if not forceDefaults then
    if not table.empty(hotkeys) then
      for keyCombo, setting in pairs(hotkeys) do
        local getKeyCombo = tostring(keyCombo)
		if setting.keyCombo and string.len(setting.keyCombo) > 0 then
			getKeyCombo = setting.keyCombo
		end
		addKeyCombo(getKeyCombo, setting)
        hotkeyList[getKeyCombo] = setting
      end
    end
  end
...
function reset()
  unload()
  scheduleEvent(function() load(true) end, 100)
end
...
function addKeyCombo(keyCombo, keySettings, focus)
  ...
  if focus then
	...
	local spellContainer = g_ui.createWidget('UIWidget',modules.game_interface.returnHotkeysPanel())
	spellContainer:setImageSource('/images/newUI/hotkeyBackground')
	spellContainer:setWidth(117)
	spellContainer:setHeight(111)
	local spellWidget = g_ui.createWidget('UIItem',spellContainer)
	spellWidget:setImageSource('/images/game/hotkeys/'..keyCombo)
	spellWidget:addAnchor(AnchorTop, 'parent', AnchorTop)
	spellWidget:addAnchor(AnchorLeft, 'parent', AnchorLeft)
	spellWidget:setWidth(84)
	spellWidget:setHeight(81)
	if not tonumber(keyCombo) then 
		spellWidget:setText(keyCombo)
	end
	spellWidget:setTextAlign(AlignBottomRight)
	spellWidget:setMarginTop(14)
	spellWidget:setMarginLeft(16)
	spellWidget.onClick = function() 
		modules.game_console.sendMessage(hotkeyLabel.value)
	end

	local countWidget = g_ui.createWidget('UIItem',spellContainer)
	countWidget:setImageSource('/images/newUI/hotkeyCountBackground')
	countWidget:addAnchor(AnchorTop, 'parent', AnchorTop)
	countWidget:addAnchor(AnchorLeft, 'parent', AnchorLeft)
	countWidget:setWidth(44)
	countWidget:setHeight(48)
	countWidget:setMarginLeft(35)
	countWidget:setMarginTop(-24)
	countWidget:setFont("lithos-pro-26px")
	countWidget:setColor("#ffffff")
	countWidget:setTextOffset(topoint('-5 10'))
	countWidget:setTextAlign(AlignCenter)
	if keySettings.itemId and keySettings.itemId > 0 then
		countWidget:setId("count-" .. keySettings.itemId)
		countWidget:setText("-")
	end
	
	table.insert(countWidgets, countWidget)
...
function updateHotkeyCount(itemId, count)
	local widgetId = "count-" .. itemId
	for _, widget in ipairs(countWidgets) do
...
function onHotkeysJSONOpcode(protocol, code, json_data_convert)
	...
	if action == 'updateCounts' then
		for itemId, count in pairs(data) do
			updateHotkeyCount(itemId, count)
		end
	end
end