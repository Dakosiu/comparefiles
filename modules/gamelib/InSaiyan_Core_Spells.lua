...
function Player.isVocation(player, neededVocation)
    ...
    if type(neededVocation) == "string" and neededVocation == "monster" and not player:isMonster() then
        return false
    end
...
function Player.getShieldingLevel(player)
    return player:getEffectiveSkillLevel(SKILL_SHIELD)
end
...
function Player.getAxeLevel(player)
    return player:getEffectiveSkillLevel(SKILL_AXE)
end
function Player.getClubLevel(player)
    return player:getEffectiveSkillLevel(SKILL_CLUB)
end
function Player.getHandcombatLevel(player)
    return player:getEffectiveSkillLevel(SKILL_FIST)
end
...
function calculate(player, formula)
    ...
    return math.floor(loadstring("return " .. formula)())
end
...
function spell_summon(playerID, spellT)
    ...
    local getSummon = Game.createMonster(spellT.summon, playerPos, true, true)
    if getSummon then
...
function spellcast_castSpell(player, spellWord, target)
    if not spellcast_canCast(player, spellWord, target) then return end
	local spellT = spellcast_getSpellT(spellWord)
    local getReturn = _G[spellT.func](player:getId(), spellT, target)
	if not getReturn then return end
	...
	if spellT.activeTime then
		spellCooldownMemory[getId]["spellActive/"..getName] = spellT.activeTime
		addEvent(function()
		spellCooldownMemory[getId]["spellActive/"..getName] = 0
		end, spellT.activeTime*1000)
	end
...
function spellcast_canCast(player, spellWord, target)
    ...
    if not player:isVocation(spellT.vocation) then return false, player:sendTextMessage(GREEN, "This is not your vocation spell") end
...
local calculationT = {}
    ...
    for key, formulaT in pairs(deepCopy(spellT.formula)) do
				...
				for _i,_child in pairs(getConditions) do
					if (_child.name == "customVariable" and _child.variable) or (_child.name == "storage" and _child.key and Wikipedia.storageTable[_child.key]) then
							...
							if (_child.name == "storage" and _child.key and Wikipedia.storageTable[_child.key]) then
								...
								varOutput = math.floor(loadstring("return " .. converFormulaToNumbers(player, _child.variable, getVariables))())
							end
					...
					elseif _child.name == "random" and _child.percent then
						...
						if type(varOutput) == "string" then
							varOutput = math.floor(loadstring("return " .. converFormulaToNumbers(player, _child.percent, getVariables))())
						end
						...
						if varOutput and _child.value and varOutput >= varRandom then
							...
							if _child.effect and _child.effect.success and _child.effect.success > 0 then
								player:getPosition():sendMagicEffect(_child.effect.success)
							end
						elseif varOutput and not _child.value and varOutput < varRandom then
							...
							if _child.effect and _child.effect.fail and _child.effect.fail > 0 then
								player:getPosition():sendMagicEffect(fail)
							end
						end
					end
				end
        ...
        local minAmount = math.floor(loadstring("return " .. getMin)())
        local maxAmount = math.floor(loadstring("return " .. converFormulaToNumbers(player, formulaT.max, getVariables))())
...
if getAttr.onlyCaster and self:getId() ~= getAttr.casterId then return true end
	 ...
	 getAttr.castPosition = self:getPosition()
		...
		local removeTimer = getAttr.condition.duration
			...
			stopEvent(globalDurationTimer[self:getId()])
			globalDurationTimer[self:getId()] = nil
			globalDurationTimer[self:getId()] = addEvent(afterTimeDuration, removeTimer, self:getId(),attribute)
...
elseif attribute == "conditionBuff"  then
	...
	for _i,_child in pairs(getAttr.stats) do
						if combinedValue[_child.subId] and combinedValue[_child.subId] > 0 then						
                                ...
                                localConditionContainer:setParameter(CONDITION_PARAM_TICKS, -1)
...
self:setCustomAttribute(attribute, value)
	...
	elseif attribute == "movementSpeed" then
		...
		if math.abs((self:getSpeed()) - (self:getBaseSpeed() + changeValue)) > 5 then
			...
			self:changeSpeed(self:getBaseSpeed() - self:getSpeed() + changeValue)
		end
