...
function Player.findEvent(player, room, vocation)
	...
	if forceAdd then 
	   ...
	   QUEUE_ROOM_SYSTEM:addPlayer(room, getRoom.ID, player, vocation)
	end
	...
	if #roomPlayers == #getConfig.players and QUEUE_ROOM_SYSTEM:getPlayers(room, getRoom.ID) == #getConfig.players then
	    eventRoomMemory[roomId].members = {}
...
function getHighscoreInfo(player, onlyRefresh)	
	...
	if not onlyRefresh then
		repetitiveSend(player, HIGHSCORE_OPCODE, "refreshHighscore",{info = _EVENTS_HIGHSCORES, refresh = _EVENTS_HIGHSCORES_LASTREFRESH, time_end = SEASON_RESTART_SYSTEM:getEndTimeString(), title = SEASON_TITLE_SYSTEM:getTitle(player)})
	end
end
