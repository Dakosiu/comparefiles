...
/* int32_t Player::getWeaponSkill(const Item* item) const
{
	...
	switch (weaponType) {
		...
		case WEAPON_CLUB:
		case WEAPON_AXE: {
			attackSkill = getStatisticLevel(STATISTIC_STRENGTH) + 10; // +10 because Tibia's skills start at 10 level
			break;
		}

		case WEAPON_DISTANCE: {
			attackSkill = getStatisticLevel(STATISTIC_DEXTERITY) + 10; // +10 because Tibia's skills start at 10 level
			break;
		}
	}
} */
...
double Player::getCriticalChance() const
{
	...
	for (slots_t slot : handsSlots) {
		Item* inventoryItem = inventory[slot];
		if (inventoryItem) {
			const ItemAttributes::CustomAttribute* attr = inventoryItem->getCustomAttribute("ITEM_STATISTIC_SYSTEM_CRITICAL_CHANCE");
			if (attr) {
...
int32_t Player::getLifeLeech() const
{
	...
	for (slots_t slot : handsSlots) {
		Item* inventoryItem = inventory[slot];
		if (inventoryItem) {
			const ItemAttributes::CustomAttribute* attr = inventoryItem->getCustomAttribute("ITEM_STATISTIC_SYSTEM_INCREASE_LIFE_LEECH");
			if (attr) {
				value += static_cast<uint32_t>(attr->getInt());
			}
			//armor += inventoryItem->getArmor();
...
int32_t Player::getManaLeech() const
{
	...
	for (slots_t slot : handsSlots) {
		Item* inventoryItem = inventory[slot];
		if (inventoryItem) {
			const ItemAttributes::CustomAttribute* attr = inventoryItem->getCustomAttribute("ITEM_STATISTIC_SYSTEM_INCREASE_MANA_LEECH");
			if (attr) {
				value += static_cast<uint32_t>(attr->getInt());
			}
			//armor += inventoryItem->getArmor();
...
double Player::getSpellDamage() const
{
	...
	for (slots_t slot : inventorySlots) {
		Item* inventoryItem = inventory[slot];
		if (inventoryItem) {
			const ItemAttributes::CustomAttribute* attr = inventoryItem->getCustomAttribute("ITEM_STATISTIC_SYSTEM_INCREASE_SPELL_DAMAGE");
			if (attr) {
				value += static_cast<uint32_t>(attr->getInt());
			}			
		}
...
int32_t Player::getDefense()
{
    ...
    int32_t defenseSkill = getStatisticLevel(STATISTIC_DEFENCE) + 10; // +10 because Tibia's skills start at 10 level
	const Item* weapon;
	const Item* shield;
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (source) {
	    ...
	    if (monster) {
	        //std::cout << "Experience from: " << monster->getName() << std::endl;
	        //std::cout << "Experience Before" << exp << std::endl;
		    if (monster->isElite()) {
			    double percent = monster->getIncreaseExperience();
			    if (percent > 0) {
				    exp = (exp + (exp * percent));
			    }
			}
		}
		//std::cout << "Experience After" << exp << std::endl;
	}
...
void Player::updateStatisticById(const Statistics_t statisticId, const int16_t points)
{
	const StatisticSkills& statisticSkills = PlayerStatistics::getInstance().getStatisticsBySkillId(statisticId);
	switch (statisticId)
	{
		...
		case STATISTIC_MAGIC:
		{
			// Update maximum mana and magic level
			manaMax += points * statisticSkills.getMana();
			mana += points * statisticSkills.getMana();
			break;
		}
		case STATISTIC_DEXTERITY:
		{
...
void Player::acceptStatistic(const std::map<Statistics_t, uint16_t>& stats)
{
	uint16_t pointsTotal = 0;
	for (auto& [_, points] : stats) {
		pointsTotal += points;
	}
	if (statisticPoints < pointsTotal) {
		return;
	}
	for (auto& [statisticId, points] : stats) {
		statistics[statisticId] += points;
		updateStatisticById(statisticId, points);
	}
	updateStatisticPoints();
	sendStatistics();
}

void Player::updateStatisticPoints()
{
	uint16_t pointsTotal = 0;
	for (uint8_t i = STATISTIC_FIRST; i <= STATISTIC_LAST; ++i) {
		pointsTotal += statistics[i];
	}

	uint16_t pointsExpected = 1; // 1 by default because player starts at 1 experience level
	const auto& levelPoints = PlayerStatistics::getInstance().getLevels();
	for (auto& statisticLevel : levelPoints) {
		if (statisticLevel.toLevel > 0 && level > statisticLevel.toLevel) {
			// Add missing points
			pointsExpected += (statisticLevel.toLevel - statisticLevel.fromLevel) * statisticLevel.points;
		}
		else if (level >= statisticLevel.fromLevel && (statisticLevel.toLevel == 0 || level < statisticLevel.toLevel)) {
			pointsExpected += (level - statisticLevel.fromLevel) * statisticLevel.points;
		}
	}

	if (pointsTotal > pointsExpected) {
		// Player died and lost some statistic points, take missing points from the most invested statistic
		uint16_t dif = pointsTotal - pointsExpected;
		while (dif > 0) {
			for (uint8_t i = STATISTIC_FIRST; i <= STATISTIC_LAST; ++i) {
				if (statistics[i] == 0) {
					continue;
	...
	else if (pointsTotal == pointsExpected) {
		// Player after dying/lvling up has the correct amount of points
		statisticPoints = 0;
	}
	else {
		// Update player's statistic points
		statisticPoints = pointsExpected - pointsTotal;
	}
}
...
{
	for (uint8_t i = STATISTIC_FIRST; i <= STATISTIC_LAST; ++i) {
		updateStatisticById(static_cast<Statistics_t>(i), -static_cast<int16_t>(statistics[i]));
		statistics[i] = 0;
	}
