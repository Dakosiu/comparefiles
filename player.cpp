...
void Player::onThink(uint32_t interval)
{
	...
	if ((!pzLocked && OTSYS_TIME() - lastPong >= 60000) && !getTile()->hasFlag(TILESTATE_NOLOGOUT)) {
		kickPlayer(true);
	}
...
void Player::addExperience(uint64_t exp, bool sendText/* = false*/, bool applyStages/* = true*/)
{
	...
	g_events->eventPlayerOnGainExperience(this, exp);
...
{
	if (!g_config.getBoolean(ConfigManager::TEST_MODE)) {
		// we do not have towns on test mode
...
ReturnValue Player::queryAdd(int32_t index, const Thing& thing, uint32_t count, uint32_t flags, Creature*) const
{
	...
	if (ret == RETURNVALUE_NOERROR || ret == RETURNVALUE_NOTENOUGHROOM) {
		...
		if (g_moveEvents->onPlayerEquip(const_cast<Player*>(this), const_cast<Item*>(item), static_cast<slots_t>(index), true) != RETURNVALUE_NOERROR) {
...
{
	if (holyLight) {
		static LightInfo holy_light;
		holy_light.level = 7;
		holy_light.color = 215;
		light = holy_light;
		return;
	}
...
void Player::onAttackedCreature(Creature* target)
{
	...
	if (targetPlayer && !isPartner(targetPlayer) && !isGuildMate(targetPlayer)) {
		if (!pzLocked && g_game.getWorldType() != WORLD_TYPE_PVP_ENFORCED) {
			Tile* myTile = getTile();
			Tile* targetTile = targetPlayer->getTile();
			if (myTile && targetTile) {
				if (!myTile->hasFlag(TILESTATE_PVPZONE) && !targetTile->hasFlag(TILESTATE_PVPZONE)) {
					pzLocked = true;
				}
			}
		...
		if (getSkull() == SKULL_NONE && getSkullClient(targetPlayer) == SKULL_YELLOW) {
			...
			if (!Combat::isInPvpZone(this, targetPlayer) && !isInWar(targetPlayer)) {
				...
				if (targetPlayer->getSkull() == SKULL_WHITE || targetPlayer->getSkull() == SKULL_RED) {
                    if (Guild* guild = targetPlayer->getGuild()) {
                        SpectatorVec spectators;
                        g_game.map.getSpectators(spectators, targetPlayer->getPosition(), false, true);
                        spectators.erase(targetPlayer);
                        for (Creature* spectator : spectators) {
                            if (Player* spectatorPlayer = spectator->getPlayer()) {
                                if (spectatorPlayer->getGuild() == guild) {
                                    addAttacked(spectatorPlayer);
                                    spectatorPlayer->sendCreatureSkull(this);
                                }
                            }
                        }
                    }
