...
int32_t Player::getDefense() const
{
	...
	if(_weapon)
	{
		...
		defenseValue += _weapon->getBonusDefense();
	...
	if(_shield && _shield->getDefense() > defenseValue)
	{
		...
		defenseValue += _shield->getBonusDefense();
...
int64_t Player::getPlayerInfo(playerinfo_t playerinfo) const
{
	switch(playerinfo)
	{
		...
		case PLAYERINFO_SOUL:
			return std::max((int64_t)0, ((int64_t)soul + varStats[STAT_SOUL]));
		default:
			break;
	}
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if((((g_game.getWorldType() == WORLDTYPE_OPTIONAL && !player->isEnemy(this, true) &&
		...
		player->getLevel() < (uint64_t)g_config.getNumber(ConfigManager::PROTECTION_LEVEL))) && player->getTile()->ground &&
...
void Player::drainHealth(Creature* attacker, CombatType_t combatType, int64_t damage)
{
	Creature::drainHealth(attacker, combatType, damage);
	char buffer[150];
	if(attacker)
		sprintf(buffer, "You lose %lld hitpoint%s due to an attack by %s.", damage, (damage != 1 ? "s" : ""), attacker->getNameDescription().c_str());
	else
...
void Player::drainMana(Creature* attacker, CombatType_t combatType, int64_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, "You lose %lld mana blocking an attack by %s.", damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "You lose %lld mana.", damage);
...
void Player::addExperience(uint64_t exp)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[100];
		sprintf(advMsg, "You advanced from Level %lld to Level %lld.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	{
		level--;
		healthMax = std::max((int64_t)0, (healthMax - (int32_t)vocation->getGain(GAIN_HEALTH)));
		manaMax = std::max((int64_t)0, (manaMax - (int32_t)vocation->getGain(GAIN_MANA)));
	...
	if(prevLevel != level)
	{
		...
		char advMsg[100];
		sprintf(advMsg, "You were downgraded from Level %lld to Level %lld.", prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
	}
	...
	bool checkDefense/* = false*/, bool checkArmor/* = false*/, bool reflect/* = true*/)
{
	...
	for(int32_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot)
	{
		...
		int32_t value = it.abilities.absorb[combatType];
		if (!value) {
			if (item) {
				value = item->getAbsorbAll();
			}
		}
		else {
			value += item->getAbsorbAll();
		...
		if(value > 0)
		{
			blocked += (int64_t)std::ceil((double)(damage * value) / 100.);
			if(item->hasCharges())
		...
		{
			reflected += (int64_t)std::ceil((double)(damage * it.abilities.reflect[REFLECT_PERCENT][combatType]) / 100.);
	...
	if(outfitAttributes)
	{
		int64_t tmp = Outfits::getInstance()->getOutfitAbsorb(defaultOutfit.lookType, sex, combatType);
		if(tmp)
			blocked += (int64_t)std::ceil((double)(damage * tmp) / 100.);

		if(reflect)
		{
			tmp = Outfits::getInstance()->getOutfitReflect(defaultOutfit.lookType, sex, combatType);
			if(tmp)
				reflected += (int64_t)std::ceil((double)(damage * tmp) / 100.);
		}
	}

	if(vocation->getAbsorb(combatType))
		blocked += (int64_t)std::ceil((double)(damage * vocation->getAbsorb(combatType)) / 100.);

	if(reflect && vocation->getReflect(combatType))
		reflected += (int64_t)std::ceil((double)(damage * vocation->getReflect(combatType)) / 100.);
...
double Player::getGainedExperience(Creature* attacker) const
{
	...
	if((min > 0 && level < (uint64_t)std::floor(attackerLevel * min)) || (max > 0 &&
		level > (uint64_t)std::floor(attackerLevel * max)))
		return 0;
	...
	*/
	uint32_t a = (uint64_t)std::floor(attackerLevel * 0.9), b = level;
...
void Player::onAttackedCreatureDrain(Creature* target, int64_t points)
{
	...
	char buffer[100];
	sprintf(buffer, "You deal %lld damage to %s.", points, target->getNameDescription().c_str());
	sendTextMessage(MSG_STATUS_DEFAULT, buffer);
}

void Player::onSummonAttackedCreatureDrain(Creature* summon, Creature* target, int64_t points)
{
	Creature::onSummonAttackedCreatureDrain(summon, target, points);

	char buffer[100];
	sprintf(buffer, "Your %s deals %lld damage to %s.", summon->getName().c_str(), points, target->getNameDescription().c_str());
	sendTextMessage(MSG_EVENT_DEFAULT, buffer);
}
...
{
	if(min > 0)
		min = (int64_t)(min * vocation->getMultiplier(MULTIPLIER_HEALING));
	else
		min = (int64_t)(min * vocation->getMultiplier(MULTIPLIER_MAGIC));

	if(max > 0)
		max = (int64_t)(max * vocation->getMultiplier(MULTIPLIER_HEALING));
	else
		max = (int64_t)(max * vocation->getMultiplier(MULTIPLIER_MAGIC));
	...
	for(; i < SLOT_LAST; ++i)
	{
		...
		if(min > 0)
		{
			...
			if(it.abilities.increment[HEALING_PERCENT])
				min = (int64_t)std::ceil((double)(min * it.abilities.increment[HEALING_PERCENT]) / 100.);
		}
		else
		{
			minValue -= it.abilities.increment[MAGIC_VALUE];
			if(it.abilities.increment[MAGIC_PERCENT])
				min = (int64_t)std::ceil((double)(min * it.abilities.increment[MAGIC_PERCENT]) / 100.);
		}
		...
		if(max > 0)
		{
			...
			if(it.abilities.increment[HEALING_PERCENT])
				max = (int64_t)std::ceil((double)(max * it.abilities.increment[HEALING_PERCENT]) / 100.);
		}
		else
		{
			maxValue -= it.abilities.increment[MAGIC_VALUE];
			if(it.abilities.increment[MAGIC_PERCENT])
				max = (int64_t)std::ceil((double)(max * it.abilities.increment[MAGIC_PERCENT]) / 100.);
		}
