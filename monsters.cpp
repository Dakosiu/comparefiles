...
void MonsterType::createLoot(Container* corpse, double lootIncrease)
{
	...
	for (auto it = info.lootItems.rbegin(), end = info.lootItems.rend(); it != end; ++it) {
		std::vector<Item*> itemList = createLootItem(*it, lootIncrease);
		if (itemList.empty()) {
			continue;
		}
...
std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock, double lootIncrease)
{
	int32_t itemCount = 0;

	uint32_t randvalue = Monsters::getLootRandom();
	if (lootIncrease) {
		randvalue -= static_cast<uint32_t>(randvalue * lootIncrease);
	}
...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
	...
	if (isScripted) {
		...
		if (tmpName == "bleed") {
			...
			condition->setSpeedDelta(normal_random(minSpeedChange, maxSpeedChange));
			...
			tmpName == "physicalcondition" || tmpName == "energycondition" ||
			tmpName == "firecondition") {
		...
		if (tmpName == "physical") {
			combat->setParam(CombatParam_t::COMBAT_PARAM_BLOCKARMOR, true);
			combat->setParam(CombatParam_t::COMBAT_PARAM_BLOCKSHIELD, true);
		}
