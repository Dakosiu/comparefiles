...
void MonsterType::createLoot(Container* corpse, Monster* monster)
{
	if (!monster) {
		return;
	}
	...
	for (auto it = info.lootItems.rbegin(), end = info.lootItems.rend(); it != end; ++it) {
		std::vector<Item*> itemList = createLootItem(*it, monster);
		if (itemList.empty()) {
			continue;
		}
		...
		for (Item* item : itemList) {
			...
			if (Container* container = item->getContainer()) {
				if (!createLootContainer(container, *it, monster)) {
					delete container;
					continue;
				}
			}
...
std::vector<Item*> MonsterType::createLootItem(const LootBlock& lootBlock, Monster* monster)
{
	int32_t itemCount = 0;
...
/* std::cout << "Monster Corpse: " << monster->getName() << std::endl;
   ...
   std::cout << "lootBlock Chance: " << lootBlock.chance << std::endl;
   std::cout << "Monster LootRate: " << monster->getIncreaseLoot() << std::endl; */
   
    uint32_t chance = lootBlock.chance;
	if (monster->getIncreaseLoot() > 0) {
		chance = (chance + (chance * monster->getIncreaseLoot()));
	}
...
bool MonsterType::createLootContainer(Container* parent, const LootBlock& lootblock, Monster* monster)
{
	...
	for (; it != end && parent->size() < parent->capacity(); ++it) {
		auto itemList = createLootItem(*it, monster);
