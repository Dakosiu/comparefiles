...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((node = monsterNode.child("look"))) {
		...
		if ((attr = node.attribute("auraBack"))) {
			mType->info.outfit.lookAuraBack = pugi::cast<uint16_t>(attr.value());
		}

		if ((attr = node.attribute("auraFront"))) {
			mType->info.outfit.lookAuraFront = pugi::cast<uint16_t>(attr.value());
		}
