...
class CooldownCallback final : public CallBack
{
	public:
		void getCooldown(Player* player, uint32_t& cd) const;
};

class ManaCallback final : public CallBack
{
	public:
		void getManaCost(Player* player, uint32_t& cost) const;
};
...
class Spell : public BaseSpell
{
	public:
		...
		CallBack* getCallback(CallBackSpell_t key);
		bool setCallback(CallBackSpell_t key);
		...
		std::shared_ptr<CooldownCallback> cooldownCallback;
		std::shared_ptr<CooldownCallback> groupCooldownCallback;
		std::shared_ptr<CooldownCallback> secondaryGroupCooldownCallback;
		std::shared_ptr<ManaCallback> manaCallback;
