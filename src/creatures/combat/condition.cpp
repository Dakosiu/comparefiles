...
Condition* Condition::createCondition(ConditionId_t id, ConditionType_t type, int32_t ticks, int32_t param/* = 0*/, bool buff/* = false*/, uint32_t subId/* = 0*/)
{
	switch (type) {
		...
		case CONDITION_SPECIALATTRIBUTES:
			return new ConditionSpecialAttributes(id, type, ticks, buff, subId);
...
bool ConditionAttributes::setParam(ConditionParam_t param, int32_t value)
{
  ...
  case CONDITION_PARAM_SKILL_ALL: {
    skills[SKILL_CLUB] = value;
    skills[SKILL_AXE] = value;
    skills[SKILL_SWORD] = value;
  ...
  case CONDITION_PARAM_SKILL_ALLPERCENT: {
    skillsPercent[SKILL_CLUB] = value;
    skillsPercent[SKILL_AXE] = value;
    skillsPercent[SKILL_SWORD] = value;
  ...
  case CONDITION_PARAM_SKILL_LIFE_LEECH_AMOUNT: {
	...
	//skills[SKILL_LIFE_LEECH_CHANCE] = 100;
    return true;
  }
  ...
  case CONDITION_PARAM_SKILL_MANA_LEECH_AMOUNT: {
	...
	//skills[SKILL_MANA_LEECH_CHANCE] = 100;
    return true;
  }
...
/*   case CONDITION_PARAM_SKILL_MANA_LEECH_CHANCE: {
  ...
  case CONDITION_PARAM_STAT_DAMAGEMITIGATION: {
	stats[STAT_DAMAGEMITIGATION] = value;
	return true;
  }
  ...
  case CONDITION_PARAM_SKILL_LIFE_LEECH_CUSTOM: {
	if (value > 0) {
	   ...
	   skills[SKILL_LIFE_LEECH_CHANCE] = 0;
	}
	skills[SKILL_LIFE_LEECH_AMOUNT] = value;
	return true;
  }
  
  case CONDITION_PARAM_SKILL_MANA_LEECH_CUSTOM: {
	if (value > 0) {
       skills[SKILL_MANA_LEECH_CHANCE] = 100;
	} else {
	   skills[SKILL_MANA_LEECH_CHANCE] = 0;
	}
	skills[SKILL_MANA_LEECH_AMOUNT] = value;
	return true;
  }

  case CONDITION_PARAM_STAT_EXTRADAMAGE: {
	stats[STAT_EXTRADAMAGE] = std::max<int32_t>(0, value);
	return true;
  }
...
void ConditionSpecialAttributes::addCondition(Creature* creature, const Condition* addCondition)
{
	if (updateCondition(addCondition)) {
		setTicks(addCondition->getTicks());

		const ConditionSpecialAttributes& conditionAttrs = static_cast<const ConditionSpecialAttributes&>(*addCondition);
		//Remove the old condition
		endCondition(creature);

		//Apply the new one
		memcpy(absorbPercent, conditionAttrs.absorbPercent, sizeof(absorbPercent));
		memcpy(absorbPercentValue, conditionAttrs.absorbPercentValue, sizeof(absorbPercentValue));
		manaLeechOnHit = conditionAttrs.manaLeechOnHit;
		lifeLeechOnHit = conditionAttrs.lifeLeechOnHit;


		updateAbsorbs(creature);

		creature->setManaLeechOnHit(creature->getManaLeechOnHit() + manaLeechOnHit);
		creature->setLifeLeechOnHit(creature->getLifeLeechOnHit() + lifeLeechOnHit);
	}
}

bool ConditionSpecialAttributes::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	if (attr == CONDITIONATTR_ABSORBPERCENT) {
		return propStream.read<int32_t>(absorbPercent[currentAbsorbPercent++]);
	} else if (attr == CONDITIONATTR_LIFELEECHONHIT) {
		return propStream.read<uint16_t>(lifeLeechOnHit);
	} else if (attr == CONDITIONATTR_MANALEECHONHIT) {
		return propStream.read<uint16_t>(manaLeechOnHit);
	}
	return Condition::unserializeProp(attr, propStream);
}

void ConditionSpecialAttributes::serialize(PropWriteStream& propWriteStream)
{
	Condition::serialize(propWriteStream);

	for (uint8_t i = 0; i < COMBAT_COUNT; i++) {
		propWriteStream.write<uint8_t>(CONDITIONATTR_ABSORBPERCENT);
		propWriteStream.write<int32_t>(absorbPercent[i]);
	}

	propWriteStream.write<uint8_t>(CONDITIONATTR_LIFELEECHONHIT);
	propWriteStream.write<uint16_t>(lifeLeechOnHit);

	propWriteStream.write<uint8_t>(CONDITIONATTR_MANALEECHONHIT);
	propWriteStream.write<uint16_t>(manaLeechOnHit);
}

bool ConditionSpecialAttributes::startCondition(Creature* creature)
{
	if (!Condition::startCondition(creature)) {
		return false;
	}
...
void ConditionSpecialAttributes::updateAbsorbs(Creature* creature)
{
	for (uint8_t i = 0; i < COMBAT_COUNT; i++) {
		if (absorbPercent[i] != 0) {
			absorbPercentValue[i] = std::round((100 - creature->getAbsorbPercent(indexToCombatType(i))) * absorbPercent[i] / 100.);
			creature->setAbsorbPercent(indexToCombatType(i), absorbPercentValue[i]);
		}
	}
}

bool ConditionSpecialAttributes::executeCondition(Creature* creature, int32_t interval)
{
	return ConditionGeneric::executeCondition(creature, interval);
}

void ConditionSpecialAttributes::endCondition(Creature* creature)
{
	for (uint8_t i = 0; i < COMBAT_COUNT; i++) {
		if (absorbPercentValue[i] != 0) {
			creature->setAbsorbPercent(indexToCombatType(i), -absorbPercentValue[i]);
		}
	}
	creature->setManaLeechOnHit(creature->getManaLeechOnHit() - manaLeechOnHit);
...
bool ConditionSpecialAttributes::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = ConditionGeneric::setParam(param, value);

	switch (param) {

		case CONDITION_PARAM_SPECIAL_MANALEECHONHIT: {
			manaLeechOnHit = std::max<int32_t>(0, value);
			return true;
		}

		case CONDITION_PARAM_SPECIAL_LIFELEECHONHIT: {
			lifeLeechOnHit = std::max<int32_t>(0, value);
			return true;
		}

		case CONDITION_PARAM_ABSORB_PHYSICALPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_FIREPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_FIREDAMAGE)] = value;
			return true; }
		
		case CONDITION_PARAM_ABSORB_ENERGYPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_ENERGYDAMAGE)] = value;
			return true; }
		
		case CONDITION_PARAM_ABSORB_ICEPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_ICEDAMAGE)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_EARTHPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_EARTHDAMAGE)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_DEATHPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_DEATHDAMAGE)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_HOLYPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_HOLYDAMAGE)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_LIFEDRAINPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_LIFEDRAIN)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_MANADRAINPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_MANADRAIN)] = value;
			return true; }

		case CONDITION_PARAM_ABSORB_DROWNPERCENT: {
			absorbPercent[combatTypeToIndex(COMBAT_DROWNDAMAGE)] = value;
			return true; }
