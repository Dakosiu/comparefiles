...
class ConditionSpecialAttributes final : public ConditionGeneric
{
	public:
		ConditionSpecialAttributes(ConditionId_t initId, ConditionType_t initType, int32_t initTicks, bool initBuff = false, uint32_t initSubId = 0) :
			ConditionGeneric(initId, initType, initTicks, initBuff, initSubId) { }
		...
		ConditionSpecialAttributes* clone() const final {
			return new ConditionSpecialAttributes(*this);
		}
	...
	private:
		...
		int32_t absorbPercentValue[COMBAT_COUNT] = { 0 };
		int32_t currentAbsorbPercent = 0;
		uint16_t manaLeechOnHit = 0;
		uint16_t lifeLeechOnHit = 0;

		void updateAbsorbs(Creature* player);
};
