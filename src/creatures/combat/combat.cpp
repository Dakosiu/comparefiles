...
bool Combat::setParam(CombatParam_t param, uint32_t value)
{
	switch (param) {
		...
		case COMBAT_PARAM_MANALEECH: {
			params.manaLeech = value;
			return true;
		}

		case COMBAT_PARAM_LIFELEECH: {
			params.lifeLeech = value;
			return true;
		}

		case COMBAT_PARAM_CRITDAMAGE: {
			params.critDamage = value;
			return true;
		}

		case COMBAT_PARAM_ISBEAM: {
			params.isBeam = (value != 0);
			return true;
		}

		case COMBAT_PARAM_MAXTARGETS: {
			params.maxTargets = value;
			return true;
		}

		case COMBAT_PARAM_MAXCHAINRANGE: {
			params.maxChainRange = value;
			return true;
		}
	}
	return false;
...
void Combat::CombatHealthFunc(Creature* caster, Creature* target, const CombatParams& params, CombatDamage* data)
{
	...
	if (damage.primary.value < 0) {
		if (damage.primary.type == COMBAT_PHYSICALDAMAGE && damage.physAmplification != 0)
			damage.primary.value *= ((100 + damage.physAmplification)/100.);
		if (damage.primary.type == COMBAT_HOLYDAMAGE && damage.holyAmplification != 0)
			damage.primary.value *= ((100 + damage.holyAmplification)/100.);
	}
...
bool Combat::doChain(Creature* caster, Creature* target) const
{
	SpectatorHashSet spectators;
	g_game().map.getSpectators(spectators, caster->getPosition(), false, false);

	std::vector<std::pair<std::forward_list<Direction>, Creature*>> orderedSpectators;
	Creature* lastCreature = target;
	Position lastPosition = lastCreature->getPosition();
	std::forward_list<Direction> dirList;

	spectators.erase(caster);
	spectators.erase(target);
	...
	fpp.clearSight = true;
	fpp.maxSearchDist = params.maxChainRange;

	if (g_game().map.getPathMatching(caster->getPosition(), dirList, FrozenPathingConditionCall(lastPosition), fpp)) {
		orderedSpectators.push_back(std::pair<std::forward_list<Direction>, Creature*>(dirList, lastCreature));
	} else {
		return false;
	}
	...
	for (Creature* spectator : spectators) {
		if (Combat::canDoCombat(caster, spectator) != RETURNVALUE_NOERROR) {
			toDelete.push_back(spectator);
			continue;
		}
	}

	for (Creature* spectator : toDelete) {
		spectators.erase(spectator);
	}

	while (distance(orderedSpectators.begin(), orderedSpectators.end()) < params.maxTargets && spectators.size() > 0) {
		Creature* lastSpectator = nullptr;
		std::forward_list<Direction> lastForwardList;
		uint32_t lastDistance = 0;
		for (Creature* spectator : spectators) {
			dirList.clear();
			const Position& tempPosition = spectator->getPosition();
			uint32_t distanceX = Position::getDistanceX(lastPosition, tempPosition);
			uint32_t distanceY = Position::getDistanceY(lastPosition, tempPosition);

			if (std::max(distanceX, distanceY) <= params.maxChainRange) {
				uint32_t diagonals = 0;
				diagonals = std::min(distanceY, distanceX);
				distanceY -= diagonals;
				distanceX -= diagonals;
				uint32_t tempDistance = (diagonals * 14) + ((distanceY + distanceX) * 10);
				if ((lastSpectator == nullptr || tempDistance < lastDistance) && g_game().map.getPathMatching(lastPosition, dirList, FrozenPathingConditionCall(tempPosition), fpp)) {
					lastSpectator = spectator;
					lastForwardList = dirList;
					lastDistance = tempDistance;
				}
			}
		}
		if (lastSpectator != nullptr) {
			orderedSpectators.push_back(std::pair<std::forward_list<Direction>, Creature*>(lastForwardList, lastSpectator));
			spectators.erase(lastSpectator);
			lastCreature = lastSpectator;
			lastPosition = lastSpectator->getPosition();
			continue;
		} else {
			break;
		}
	...
	for (auto it = orderedSpectators.begin(); it != orderedSpectators.end(); it++) {
		dirList = it->first;
		Creature* chainCreature = it->second;
		for (Direction dir : dirList) {
			if (dir == DIRECTION_NORTH) {
				...
				lastPosition.x--;
			} else if (dir == DIRECTION_NORTHEAST) {
				lastPosition.y--;
				lastPosition.x++;
			} else if (dir == DIRECTION_SOUTHEAST) {
				lastPosition.y++;
				lastPosition.x++;
			} else if (dir == DIRECTION_SOUTHWEST) {
				lastPosition.y++;
				lastPosition.x--;
			} else if (dir == DIRECTION_NORTHWEST) {
				lastPosition.y--;
				lastPosition.x--;
			}
			g_game().addMagicEffect(lastPosition, params.impactEffect);
		}
		lastPosition = chainCreature->getPosition();
		if (params.combatType != COMBAT_NONE) {
			CombatDamage damage = getCombatDamage(caster, chainCreature);
			if (damage.primary.type != COMBAT_MANADRAIN) {
				damage.lifeLeech = params.lifeLeech;
				damage.manaLeech = params.manaLeech;
				doCombatHealth(caster, chainCreature, damage, params);
			} else {
				doCombatMana(caster, chainCreature, damage, params);
			}
		} else {
			doCombatDefault(caster, chainCreature, params);
		}
	}
	return true;
}
...
void Combat::CombatFunc(Creature* caster, const Position& pos, const AreaCombat* area, const CombatParams& params, CombatFunction func, CombatDamage* data)
{
	...
	if (data) {
		...
		tmpDamage.critDamage = data->critDamage;
		tmpDamage.physAmplification = data->physAmplification;
		tmpDamage.holyAmplification = data->holyAmplification;
	}

	tmpDamage.affected = affected;

	if (tmpDamage.affected > 0 && caster->getPlayer() && caster->getPlayer()->getVocation()->getBaseId() == VOCATION_SORCERER && params.isBeam)
	{
		Player* player = caster->getPlayer();
		uint16_t total = player->getWheelOfDestinyPerk(38);
		if (total >= 250) 
		{
			uint8_t level = 1;
			if (total >= 1000)
			...
			for (Condition* condition : player->getConditions()) {
				if (condition->getType() == CONDITION_SPELLCOOLDOWN) {
					...
					ticks -= std::min(3000, tmpDamage.affected * 1000);
					condition->setTicks(ticks);
					player->sendSpellCooldown(subId, std::max(0, ticks));
				}
			}
			tmpDamage.primary.value *= std::min(1.3 + ((level - 1) * 0.06), 1 + (tmpDamage.affected * (0.1 + (level - 1) * 0.02)));
			tmpDamage.secondary.value *= std::min(1.3 + ((level - 1) * 0.06), 1 + (tmpDamage.affected * (0.1 + (level - 1) * 0.02)));
		}
	} 
	for (Tile* tile : tileList) {
		if (canDoCombat(caster, tile, params.aggressive) != RETURNVALUE_NOERROR) {
...
void Combat::doCombatHealth(Creature* caster, Creature* target, CombatDamage& damage, const CombatParams& params)
{
	...
	if (caster && caster->getPlayer()) {
		...
		if (chance != 0 && uniform_random(1, 100) <= chance) {
			...
			damage.primary.value += (damage.primary.value * (damage.critDamage + params.critDamage + caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE))) / 100;
			damage.secondary.value += (damage.secondary.value * (damage.critDamage + params.critDamage + caster->getPlayer()->getSkillLevel(SKILL_CRITICAL_HIT_DAMAGE))) / 100;
		}
