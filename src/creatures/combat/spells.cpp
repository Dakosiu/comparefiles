...
bool Spell::playerSpellCheck(Player* player) const
{
	...
	uint32_t cost = getManaCost(player);
	if (manaCallback)
		manaCallback->getManaCost(player, cost);

	if (player->getMana() < cost && !player->hasFlag(PlayerFlag_HasInfiniteMana)) {
...
CallBack* Spell::getCallback(CallBackSpell_t key)
{
	switch (key) {
		case CALLBACK_SPELL_COOLDOWN: {
			return cooldownCallback.get();
		}

		case CALLBACK_SPELL_GROUP_COOLDOWN: {
			return groupCooldownCallback.get();
		}

		case CALLBACK_SPELL_SECONDARY_GROUP_COOLDOWN: {
			return secondaryGroupCooldownCallback.get();
		}

		case CALLBACK_SPELL_MANA: {
			return manaCallback.get();
		}
	}
	return nullptr;
}

bool Spell::setCallback(CallBackSpell_t key)
{
	switch (key) {
		case CALLBACK_SPELL_COOLDOWN: {
			cooldownCallback.reset(new CooldownCallback());
			return true;
		}

		case CALLBACK_SPELL_GROUP_COOLDOWN: {
			groupCooldownCallback.reset(new CooldownCallback());
			return true;
		}

		case CALLBACK_SPELL_SECONDARY_GROUP_COOLDOWN: {
			secondaryGroupCooldownCallback.reset(new CooldownCallback());
			return true;
		}

		case CALLBACK_PARAM_SKILLVALUE: {
			manaCallback.reset(new ManaCallback());
			return true;
		}
	}
	return false;
...
void CooldownCallback::getCooldown(Player* player, uint32_t& cooldown) const
{
	//getCooldown(...)
	if (!scriptInterface->reserveScriptEnv()) {
		SPDLOG_ERROR("[CooldownCallback::getCooldown - Player {}] "
                     "Call stack overflow. Too many lua script calls being nested.",
                     player->getName());
		return;
	}
	...
	if (lua_pcall(L, parameters, 1, 0) != 0) {
		...
		cooldown = LuaScriptInterface::getNumber<uint32_t>(L, -1);
		lua_pop(L, 1);
	}
...
void ManaCallback::getManaCost(Player* player, uint32_t& cost) const
{
	//getManaCost(...)
	if (!scriptInterface->reserveScriptEnv()) {
		SPDLOG_ERROR("[ManaCallback::getManaCost - Player {}] "
                     "Call stack overflow. Too many lua script calls being nested.",
                     player->getName());
		return;
	}
	...
	if (lua_pcall(L, parameters, 1, 0) != 0) {
		...
		cost = LuaScriptInterface::getNumber<uint32_t>(L, -1);
		lua_pop(L, 1);
	}
...
void Spell::applyCooldownConditions(Player* player) const {
	uint32_t cd = cooldown;

	if (cooldownCallback)
		cooldownCallback->getCooldown(player, cd);

	if (cd > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLCOOLDOWN, cd / g_configManager().getFloat(RATE_SPELL_COOLDOWN), 0, false, spellId);
		player->addCondition(condition);
	}

	cd = groupCooldown;

	if (groupCooldownCallback)
		groupCooldownCallback->getCooldown(player, cd);

	if (cd > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, cd / g_configManager().getFloat(RATE_SPELL_COOLDOWN), 0, false, group);
		player->addCondition(condition);
	}

	cd = secondaryGroupCooldown;

	if (secondaryGroupCooldownCallback)
		secondaryGroupCooldownCallback->getCooldown(player, cd);

	if (cd > 0) {
		Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_SPELLGROUPCOOLDOWN, cd / g_configManager().getFloat(RATE_SPELL_COOLDOWN), 0, false, secondaryGroup);
		player->addCondition(condition);
	}
}
...
void Spell::postCastSpell(Player* player, bool finishedCast /*= true*/, bool payCost /*= true*/) const
{
	...
	if (payCost) {
		...
		if (manaCallback)
			manaCallback->getManaCost(player, cost);
		Spell::postCastSpell(player, cost, getSoulCost());
	}
}
