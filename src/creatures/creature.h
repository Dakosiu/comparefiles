...
class Creature : virtual public Thing
{
	...
	public:
		...
		virtual uint32_t getDamageMitigation() const {
			return 0;
		}
    ...
    void setBuff(buffs_t buff, int32_t modifier) {
      varBuffs[buff] += modifier;
        ...
        */
		...
		uint16_t getLifeLeechOnHit() const {
			return lifeLeechOnHit;
		}

		void setLifeLeechOnHit(uint16_t value)
		{
			lifeLeechOnHit = value;
		}

		uint16_t getManaLeechOnHit() const {
			return manaLeechOnHit;
		}

		void setManaLeechOnHit(uint16_t value)
		{
			manaLeechOnHit = value;
		}
