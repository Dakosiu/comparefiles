                               ...
                               bool checkDefense /* = false */, bool checkArmor /* = false */, bool /* field  = false */)
{
    ...
    if (isImmune(combatType)) {
        ...
        if (checkDefense && hasDefense && canUseDefense) {
            ...
            defenseReduction = uniform_random(defense / 2, defense);
        ...
        if (checkArmor) {
            ...
            if (armor > 3) {
                armorReduction = uniform_random(armor / 2, armor - (armor % 2 + 1));
...
uint32_t Creature::getManaShield() const
{
...
uint32_t Creature::getMaxManaShield() const
{
	if (g_configManager().getBoolean(LEGACY_MANASHIELD)) {
		return 0;
	}	
...
void Creature::setMaxManaShield(uint32_t value)
{
	if (g_configManager().getBoolean(LEGACY_MANASHIELD)) {
		value = 0;
	}	
