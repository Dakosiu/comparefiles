...
// Konfiguracja przyspieszenia respawnu w zaleznosci od liczby zalogowanych graczy
static constexpr int32_t PLAYER_INTERVAL_STEP = 100; // Co ile graczy ma sie zwiekszac przyspieszenie
static constexpr int32_t TIME_DECREASE_PER_STEP = 3000; // O ile milisekund ma sie zmniejszac czas respawnu (5000 ms = 5 sekund)
	...
	for (auto spawnMonsterNode : doc.child("monsters").children()) {
		...
		for (auto childMonsterNode : spawnMonsterNode.children()) {
			if (strcasecmp(childMonsterNode.name(), "monster") == 0) {
				...
				//uint32_t interval = pugi::cast<uint32_t>(childMonsterNode.attribute("spawntime").value()) * 1000 / std::max((uint32_t)1, (g_configManager().getNumber(RATE_SPAWN) * boostedrate * eventschedule));
...
int32_t getAdjustedSpawnInterval(int32_t baseInterval) {
	int32_t onlinePlayers = g_game().getPlayersOnline();
	int32_t adjustment = (onlinePlayers / PLAYER_INTERVAL_STEP) * TIME_DECREASE_PER_STEP;

	return std::max(baseInterval - adjustment, MONSTER_MINSPAWN_INTERVAL);
}

void SpawnMonster::startSpawnMonsterCheck()
{
	if (checkSpawnMonsterEvent == 0) {
		int32_t adjustedInterval = getAdjustedSpawnInterval(getInterval());

		checkSpawnMonsterEvent = g_scheduler().addEvent(createSchedulerTask(adjustedInterval, std::bind(&SpawnMonster::checkSpawnMonster, this)));
	}
}
...
void SpawnMonster::checkSpawnMonster()
{
    ...
    for (auto& it : spawnMonsterMap) {
		...
		if (sb.spawnScheduled) {
            continue;
        }

		int32_t adjustedInterval = getAdjustedSpawnInterval(sb.interval);

		if (OTSYS_TIME() < sb.lastSpawn + adjustedInterval) {
			continue;
		}
...
void SpawnMonster::scheduleSpawn(uint32_t spawnMonsterId, spawnBlock_t& sb, uint16_t interval)
{
    if (interval <= 0) {
        spawnMonster(spawnMonsterId, sb.monsterType, sb.pos, sb.direction);
        sb.spawnScheduled = false;
    } else {
        g_game().addMagicEffect(sb.pos, CONST_ME_TELEPORT);
        g_scheduler().addEvent(createSchedulerTask(1400, std::bind(&SpawnMonster::scheduleSpawn, this, spawnMonsterId, std::ref(sb), interval - NONBLOCKABLE_SPAWN_MONSTER_INTERVAL)));
    }
}
