	...
	mType(mType)
{
	...
	name = getRealName();
	Familiar = mType->info.isFamiliar;
	...
	registerCreatureEvent("SUMMON_SYSTEM_onHealthChange");
	registerCreatureEvent("SUMMON_SYSTEM_onDeath");
	registerCreatureEvent("DAILY_TASK_MONSTER_SYSTEM_onDeath");
	registerCreatureEvent("BONUS_TASK_MONSTER_SYSTEM_onDeath");
	registerCreatureEvent("BOSS24_SYSTEM_onDeath");
	registerCreatureEvent("AILLIKA_MISSIONS_onDeath");
	registerCreatureEvent("HADRIN_MISSIONS_onDeath");
	registerCreatureEvent("onWandItemUpgrade");
	registerCreatureEvent("azar_onDeath");
	registerCreatureEvent("onQuestDamage");
	registerCreatureEvent("onDeath_JEWEL_BOX_EVENT");
	registerCreatureEvent("onHealthChange_JEWEL_BOX_EVENT");
	registerCreatureEvent("expPlaceBoss_onDeath");
}
...
void Monster::changeHealth(int32_t healthChange, bool sendHealthChange/* = true*/)
{
	if (mType && !mType->info.soundVector.empty() && mType->info.soundChance >= static_cast<uint32_t>(uniform_random(1, 100))) {
		uint32_t index = uniform_random(0, static_cast<int32_t>(mType->info.soundVector.size() - 1));
		g_game().sendSingleSoundEffect(this->getPosition(), mType->info.soundVector[index], this);
	}
