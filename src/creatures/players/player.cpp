...
uint16_t Player::getSkillLevel(uint8_t skill) const {
	...
	if (auto it = maxValuePerSkill.find(skill);
		...
		if (skill != SKILL_CRITICAL_HIT_CHANCE || skillLevel < 100)
			skillLevel = std::min<uint16_t>(it->second, skillLevel);
		else
			skillLevel = 100;
	}

	if (skill == SKILL_SHIELD || skill == SKILL_SWORD || skill == SKILL_AXE || skill == SKILL_CLUB)
	{
		if (getWheelOfDestinyPerk(1) == 200)
		{
			uint16_t totalCreatures = 0;
			for (uint16_t x = getPosition().x - 1; x <= getPosition().x + 1; x++)
			{
				for (uint16_t y = getPosition().y - 1; y <= getPosition().y + 1; y++)
				{
					Tile* tile = g_game().map.getTile(x, y, getPosition().z);
					if (tile && !tile->isMoveableBlocking()) {
						if (CreatureVector* tileCreatures = tile->getCreatures()) {
							for (Creature* tileCreature : *tileCreatures) {
								if (tileCreature && tileCreature->getMonster()) {
									totalCreatures += 1;
								}
							}
						}
			...
			if (totalCreatures >= 5)
			{
				totalCreatures = std::min(3, totalCreatures - 5);
				if (skill == SKILL_SHIELD)
					skillLevel += 6 * (1 + totalCreatures);
				else
					skillLevel += 1 * (1 + totalCreatures);
			}
		}
	}
...
uint32_t Player::getDamageMitigation() const {
	uint32_t result = std::floor(getLevel() / 20);
	...
	if (shield) {
		...
		if (getVocation()->getBaseId() == VOCATION_KNIGHT) {
			uint16_t total = getWheelOfDestinyPerk(40);
			if (total >= 1000) 
				defenseValue += 30;
			else if (total >= 500)
				defenseValue += 20;
				...
				defenseValue += 10;
			SPDLOG_DEBUG("Defense value after Wheel of Destiny perk: {}", defenseValue);
		}
	}
	...
	switch(fightMode) {
		case FIGHTMODE_ATTACK:
			if (shield) {
				result += std::floor(defenseValue * 1.4);
			} else if (weapon) {
				result += std::floor(defenseValue * 0.75);
			}
			result += std::floor(getSkillLevel(SKILL_SHIELD) / 0.8);
			// result += std::floor(getSkillLevel(SKILL_SHIELD) / 1.25);
			break;

		case FIGHTMODE_BALANCED:
			if (shield) {
				result += std::floor(defenseValue * 1.75);
			} else if (weapon) {
				result += std::floor(defenseValue * 1);
			}
			result += std::floor(getSkillLevel(SKILL_SHIELD) / 0.5);
			// result += std::floor(getSkillLevel(SKILL_SHIELD) / 1);
			break;

		case FIGHTMODE_DEFENSE:
			if (shield) {
				result += std::floor(defenseValue * 2.1);
			} else if (weapon) {
				result += std::floor(defenseValue * 1.2);
			}
			result += std::floor(getSkillLevel(SKILL_SHIELD) / 0.3);
			// result += std::floor(getSkillLevel(SKILL_SHIELD) / 0.8);
			break;
	}

	float finalResult = result * (1 + varStats[STAT_DAMAGEMITIGATION] / 10000.0);

	return std::floor(finalResult);
}
...
int32_t Player::getWeaponSkill(const Item* item) const
{
	...
	switch (weaponType) {
		...
		case WEAPON_DISTANCE: {
			...
			if (getWheelOfDestinyPerk(1) == 200 && getVocation()->getBaseId() == VOCATION_PALADIN)
			{
				bool found = false;
				for (uint16_t x = getPosition().x - 1; x <= getPosition().x + 1; x++)
				{
	...
	if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
		if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
			...
			familiars.emplace_back(
				value >> 16);
			return;
		} else if (IS_IN_KEYRANGE(key, WHEELOFDESTINY_RANGE)) {
			wheelOfDestiny[key - PSTRG_WHEELOFDESTINY_RANGE_START] = value;
		} else {
...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		//g_game().addPlayerIpAddress(this);
...
uint16_t Player::getWheelOfDestinyPerk(uint16_t key) const {
	auto it = wheelOfDestiny.find(key);
	if (it != wheelOfDestiny.end())
		return it->second;
	return 0;
}
...
void Player::genReservedStorageRange()
{
	...
	// generate wheel of destiny range
	uint32_t wheel_key = PSTRG_WHEELOFDESTINY_RANGE_START;
	for (uint16_t i = 1; i < PSTRG_WHEELOFDESTINY_RANGE_SIZE; i++)
	{
		storageMap[wheel_key + i] = wheelOfDestiny[i];
	}
}
...
void Player::addUnjustifiedDead(const Player* attacked)
{
	...
	skullTicks += g_configManager().getNumber(FRAG_TIME);
...
void Player::removeFromWarList(uint32_t guildId) 
{	
	guildWarVector.erase(remove(guildWarVector.begin(),guildWarVector.end(),guildId),guildWarVector.end());
}

void Player::addToWarList(uint32_t guildId)
{ 
    guildWarVector.push_back(guildId);
}
...
void Player::sendUnjustifiedPoints()
{
	if (client) {
		...
		if (skullTicks != 0) {
			...
			skullDuration = std::floor<uint8_t>(skullTicks / (24 * 60 * 60));
		}
