...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void removeFromWarList(uint32_t guildId);
		void addToWarList(uint32_t guildId);
		...
		uint32_t getDamageMitigation() const override;

		void setWheelOfDestinyPerk(uint16_t key, uint16_t value) {
			wheelOfDestiny[key] = value;
		}

		uint16_t getWheelOfDestinyPerk(uint16_t key) const;

		std::map<uint16_t, uint16_t> getWheelOfDestiny()
		{
		...
		double_t getExperienceBonus() const {
			return experienceBonus;
		}
		void addExperienceBonus(double value) {
		...
		uint32_t getCapacity() const {
			...
			return capacity + bonusCapacity + varStats[STAT_CAPACITY];
		}
		...
		uint32_t getExtraDamage() const {
			return varStats[STAT_EXTRADAMAGE];
		}

		uint32_t getExtraHealing() const {
			return varStats[STAT_EXTRAHEALING];
		}
		...
		uint16_t getGoldenAccountXpBoost() const {
			return goldenAccountXpBoost;
		}
		void setGoldenAccountXpBoost(uint16_t value) {
			goldenAccountXpBoost = std::min<uint16_t>(std::numeric_limits<uint16_t>::max(), value);
		}
		...
		uint32_t getDamageReflection() const
		{
	...
	private:
		...
		uint16_t goldenAccountXpBoost = 0;
		uint32_t damagereflection = 0;
		...
		std::map<uint16_t, uint16_t> wheelOfDestiny;
