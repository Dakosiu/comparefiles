...
static constexpr int32_t PLAYER_MAX_SPEED = 9000;
...
class Player final : public Creature, public Cylinder
{
	public:
		...
		int32_t getBonusHealth() const override {
			return std::max<int32_t>(0, varStats[STAT_MAXHITPOINTS]);
		}

		int32_t getBonusMana() const override {
			return std::max<int32_t>(0, varStats[STAT_MAXMANAPOINTS]);
		}
		
		uint32_t getExtraHealth() const {
			return extraHealth;
		}

		uint32_t getExtraMana() const {
			return extraMana;
		}

		uint32_t getExtraHealthAddon() const {
			return extraHealthAddon;
		}

		uint32_t getExtraManaAddon() const {
			return extraManaAddon;
		}

		uint32_t getExtraHealthMount() const {
			return extraHealthMount;
		}

		uint32_t getExtraManaMount() const {
			return extraManaMount;
		}

		uint32_t getExtraHealthRegeneration() const {
			return extraHealthRegeneration;
		}

		uint32_t getExtraManaRegeneration() const {
			return extraManaRegeneration;
		}
		...
		void openTimeCurrencyWindow(Npc* npc, const std::list<ShopInfo>& shop);
		void openWarriorCurrencyWindow(Npc* npc, const std::list<ShopInfo>& shop);
		void doShopRoutines(Npc* npc, const std::list<ShopInfo>& shop);
		...
		int32_t getReflectItems() const;
		int32_t getDodge() const;
		int32_t getItemsBonusDamage() const;
		double getExtraHealingBase() const;		
		double getProtectionAllBase() const;		
		...
		void sendAnimatedText(const std::string& message, const Position& pos, uint8_t color) {
			if (client) {
		...
		void sendDistanceShoot(const Position& from, const Position& to, unsigned short type) const {
			if (client) {
...
/* 		void sendQuestLog() {
	...
	private:
		...
		bool isTimeCurrency = false;
		bool isWarriorCurrency = false;
		...
		uint32_t mana = 0;
		uint32_t manaMax = 0;
		uint32_t extraHealth = 0;
		uint32_t Dodge = 0;
		uint32_t reflection = 0;
		uint32_t extraMana = 0;
		uint32_t extraHealthAddon = 0;
		uint32_t extraManaAddon = 0;
		uint32_t extraHealthMount = 0;
		uint32_t extraManaMount = 0;
		uint32_t extraHealthRegeneration = 0;
		uint32_t extraManaRegeneration = 0;
		double extraHealingBase = 0;
		double protectionAllBase = 0;
