...
class Player final : public Creature, public Cylinder
{
	public:
		...
		const std::string& getPickedRoom() const {
			return pickedRoom;
		}
		void setPickedRoom(const std::string& pickedRoom) {
			this->pickedRoom = pickedRoom;
		}
		const std::string& getPickedCharacter() const {
			return pickedCharacter;
		}
		void setPickedCharacter(const std::string& pickedCharacter) {
		...
		bool hasAuraBack() const
		{
			return defaultOutfit.lookAuraBack != 0;
		}
		
		bool hasAuraFront() const
		{
			return defaultOutfit.lookAuraFront != 0;
		}
		
		bool addWings(uint8_t mountId);
		bool addAura(uint8_t mountId);
		bool addShader(uint8_t mountId);
		bool hasWings() const
		{
		...
		uint16_t getCurrentAuraBack() const;
		uint16_t getCurrentAuraFront() const;
		void setCurrentAuraBack(uint16_t auraId);
		void setCurrentAuraFront(uint16_t auraId);
		...
		void addPoints(int32_t value) {
			points += value;
		}

		void removePoints(int32_t value) {
			points -= value;
		}
		...
		void setDivision(std::string value) {
			division = value;
		}
		...
		void setSkillLevel(skills_t skill, uint16_t level, bool notify);
		...
		void setMagicLevel(uint32_t level, bool notify);
		...
		uint16_t getBasicAttackAnimation() const;
		...
		void sendUpdateProgressbar(uint32_t id, uint8_t percent) {
            if (client) {
                client->sendUpdateProgressbar(id, percent);
            }
        }
	...
	private:
		...
		std::string division;
		int32_t points;
		int32_t survival_points;
		int16_t basicAttack = 0;
		...
		std::string pickedRoom;
		std::string pickedCharacter;
