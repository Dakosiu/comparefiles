...
void Game::playerUseWithCreature(uint32_t playerId, const Position& fromPos, uint8_t fromStackPos, uint32_t creatureId, uint16_t spriteId)
{
	...
	if (!Position::areInRange<16, 9, 0>(creature->getPosition(), player->getPosition())) {
		return;
	}
...
void Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	...
	const Outfit* playerOutfit = Outfits::getInstance().getOutfitByLookType(player->getVocation()->getId(), outfit.lookType);
	if (!playerOutfit) {
		outfit.lookMount = 0;
	}
...
void Game::internalCreatureChangeOutfit(Creature* creature, const Outfit_t& outfit)
{
	...
	Player* getPlayer = creature->getPlayer();
	if (getPlayer) {
		getPlayer->setCurrentWing(outfit.lookWings);
		getPlayer->setCurrentAuraBack(outfit.lookAuraBack);
		getPlayer->setCurrentAuraFront(outfit.lookAuraFront);
		
		getPlayer->setCurrentShader(outfit.lookShader);
	}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (attackerPlayer) {
			double damageBuff = (double)attackerPlayer->getBuff(BUFF_DAMAGE) / 100;
			if (damageBuff > 0 && damage.origin != ORIGIN_NONE) {
				damage.primary.value = (damage.primary.value + (damage.primary.value * damageBuff));
				if (g_config.getBoolean(ConfigManager::DEBUG_PLAYER_BUFFS)) {
					std::cout << "Base Damage: " << damage.primary.value << std::endl;
					std::cout << "Damage Buff%: " << damageBuff * 100 << std::endl;
					std::cout << "Base Damage After Buff: " << damage.primary.value << std::endl;
				}
			}
		}
        ...
        if (attacker && target && targetPlayer) { 
		    ...
		    double defenseBuff = (double)targetPlayer->getBuff(BUFF_DEFENSE) / 100;
			if (defenseBuff > 0 && damage.origin != ORIGIN_NONE) {
				damage.primary.value = (damage.primary.value - (damage.primary.value * defenseBuff));
				//damage.primary.value -= (damage.primary.value * defenseBuff)
				
				
				if (g_config.getBoolean(ConfigManager::DEBUG_PLAYER_BUFFS)) {
					std::cout << "Base Damage1 : " << damage.primary.value << std::endl;
					std::cout << "Defense Buff1 %: " << defenseBuff * 100 << std::endl;
					std::cout << "Base Damage After Buff1 : " << damage.primary.value << std::endl;
				}
            }		

    
		    double reflectBuff = (double)targetPlayer->getBuff(BUFF_REFLECT) / 100;
			if (reflectBuff > 0 && damage.origin != ORIGIN_REFLECT && damage.origin != ORIGIN_NONE) {
				std::cout << "Reflect Value: " << reflectBuff << std::endl;
                CombatDamage reflectDamage;
				
				
				
				
/* 				std::cout << damage.primary.value << " Damage dealt by " << attacker->getName() << " to " << target->getName() << std::endl;
				
				std::cout << "Damage: " << damage.primary.value << std::endl;
				std::cout << "Reflected Damage " << static_cast<int32_t>(-damage.primary.value * reflectBuff) << std::endl; */
				
				
				
                reflectDamage.primary.value = static_cast<int32_t>(-damage.primary.value * reflectBuff);
		...
		if (target) {
			uint32_t maxShield = target->getMaxShield();
			//std::cout << "Max Shield: " << maxShield << std::endl;
			if (maxShield > 0) {
				
				int32_t currentShield = target->getCurrentShield();
				if (currentShield > 0) {
					int32_t shieldDamage = healthChange;
					target->setCurrentShield(currentShield - shieldDamage);
					startUpdateProgressBar(target, target->getShieldPercentage());
					
					
					//std::cout << "Percentage: " << target->getShieldPercentage() << std::endl;
					//std::cout << "Current Shield: " << currentShield << std::endl;
					...
					std::string spectatorMessage;
					message.primary.value = shieldDamage;
				    ...
				    for (Creature* spectator : spectators) {
					    ...
					    if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
						    ...
						    message.text = fmt::format("{:s} loses {:d} shield due to your attack.", target->getNameDescription(), shieldDamage);
						    message.text[0] = std::toupper(message.text[0]);
					    } else if (tmpPlayer == targetPlayer) {
						    message.type = MESSAGE_DAMAGE_RECEIVED;
						  if (!attacker) {
							message.text = fmt::format("You lose {:d} shield.", shieldDamage);
						} else if (targetPlayer == attackerPlayer) {
							message.text = fmt::format("You lose {:d} shield due to your own attack.", shieldDamage);
						} else {
							message.text = fmt::format("You lose {:d} shield due to an attack by {:s}.", shieldDamage, attacker->getNameDescription());
						}
					} else {
						message.type = MESSAGE_DAMAGE_OTHERS;
						if (spectatorMessage.empty()) {
							if (!attacker) {
								spectatorMessage = fmt::format("{:s} loses {:d} shield.", target->getNameDescription(), shieldDamage);
							} else if (attacker == target) {
								spectatorMessage = fmt::format("{:s} loses {:d} shield due to {:s} own attack.", target->getNameDescription(), shieldDamage, targetPlayer->getSex() == PLAYERSEX_FEMALE ? "her" : "his");
							} else {
								spectatorMessage = fmt::format("{:s} loses {:d} shield due to an attack by {:s}.", target->getNameDescription(), shieldDamage, attacker->getNameDescription());
							}
							spectatorMessage[0] = std::toupper(spectatorMessage[0]);
						}
						message.text = spectatorMessage;
				...
				damage.primary.value -= shieldDamage;
				if (damage.primary.value < 0) {
...
void Game::startProgressBar(Creature* creature, int32_t duration, bool ltr)
{
    SpectatorVec spectators;
    map.getSpectators(spectators, creature->getPosition(), false, true);
    for (Creature* spectator : spectators) {
...
void Game::startUpdateProgressBar(Creature* creature, uint8_t percent)
{
    SpectatorVec spectators;
    map.getSpectators(spectators, creature->getPosition(), false, true);
    for (Creature* spectator : spectators) {
        if (Player* tmpPlayer = spectator->getPlayer()) {
            tmpPlayer->sendUpdateProgressbar(creature->getID(), percent);
        }
    }
}