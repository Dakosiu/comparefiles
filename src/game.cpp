                          ...
                          uint16_t spriteId, uint8_t fromStackPos, const Position& toPos, uint8_t count, Item* item, Cylinder* toCylinder)
{
	...
	if (item->getContainer() && item->hasAttribute(ITEM_ATTRIBUTE_ACTIONID)) {
		player->sendCancelMessage(RETURNVALUE_NOTMOVEABLE);
		return;
	}
...
void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
	...
	uint32_t ambarCoins = money / 1000000;
	money -= ambarCoins * 1000000;
	while (ambarCoins > 0) {
		const uint32_t count = std::min<uint32_t>(100, ambarCoins);
...
void Game::combatGetTypeInfo(CombatType_t combatType, Creature* target, uint8_t& color, uint8_t& effect)
{
	switch (combatType) {
		case COMBAT_PHYSICALDAMAGE: {
			Item* splash = nullptr;
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (realHealthChange > 0 && !target->isInGhostMode()) {
			...
			message.primary.color = g_config.getNumber(ConfigManager::RECOVERY_HEALTH_COLOUR);
		...
		if (targetPlayer && target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				...
				if (target && targetPlayer && damage.origin != ORIGIN_REFLECT && damage.origin != ORIGIN_NONE) {
                    int32_t reflection = targetPlayer->getReflection();
					int32_t number;
					targetPlayer->getStorageValue(10009, number);
					if (number > 0) {
						reflection = targetPlayer->getReflection() + (number / 100);
					}
                    if (reflection != 0) {
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		if (realManaChange > 0 && !targetPlayer->isInGhostMode()) {
			...
			message.primary.color = g_config.getNumber(ConfigManager::RECOVERY_MANA_COLOUR);
...
void Game::addAnimatedText(const std::string& message, const Position& pos, uint8_t color)
{
	SpectatorVec spectators;
...
void Game::addAnimatedText(const SpectatorVec& spectators, const std::string& message, const Position& pos, uint8_t color)
{
	if (spectators.empty()) {
		return;
	}
