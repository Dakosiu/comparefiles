...
Item* Player::getWeapon(slots_t slot, bool ignoreAmmo) const
{
	...
	if (!ignoreAmmo && weaponType == WEAPON_DISTANCE) {
		...
		if (it.ammoType != AMMO_NONE) {
            ...
            {
					if (container->getID() == 12425 || container->getID() == 38218 || container ->getID() == 38505 || container ->getID() == 38504 || container -> getID() == 39322 || container->getID() == 38180)
					{
...
int32_t Player::getReflection() const
{
    ...
    static const slots_t armorSlots[] = { CONST_SLOT_HEAD, CONST_SLOT_NECKLACE, CONST_SLOT_ARMOR, CONST_SLOT_LEGS, CONST_SLOT_FEET, CONST_SLOT_RING, CONST_SLOT_LEFT, CONST_SLOT_RIGHT, CONST_SLOT_BACKPACK, CONST_SLOT_AMMO };
    for (slots_t slot : armorSlots) {
        Item* inventoryItem = inventory[slot];
        if (inventoryItem) {
			if (isItemAbilityEnabled(slot)) {
              value += inventoryItem->getReflection();
			}
        }
    }
	 value += reflection;
...
int32_t Player::getReflectItems() const
{
    int32_t value = 0;
...
double Player::getExtraHealingBase() const
{
    return extraHealingBase;
}

double Player::getProtectionAllBase() const
{
    return protectionAllBase;
}
...
int32_t Player::getItemsBonusDamage() const
{
	int32_t value = 0;
	...
	for (slots_t slot : armorSlots) {
		...
		if (inventoryItem) {
			if (isItemAbilityEnabled(slot)) {
				value += inventoryItem->getBonusDamage();
			}
		}
	}
	return value;
...
{
	int32_t bonusAttackSpeed = 0;
	getStorageValue(99951, bonusAttackSpeed);
	const Item* weapon = getWeapon(true);
	if (!weapon || weapon->getAttackSpeed() == 0) {
		return vocation->getAttackSpeed() - bonusAttackSpeed;
	}

	return weapon->getAttackSpeed() - bonusAttackSpeed;
}
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if (creature->isSummon() && creature->getMaster() == this) {
        return true;
    }
...
DepotChest* Player::getRewardBag(uint32_t depotId, bool autoCreate)
{
	...
	it = depotChests.emplace(depotId, new DepotChest(ITEM_REWARD_BAG)).first;
	it->second->setDepotId(depotId);
...
{  
	...
	it = depotLockerMap.emplace(REWARD_CHEST_DEPOTID, new DepotLocker(ITEM_REWARD_CHEST)).first;
	it->second->setDepotId(REWARD_CHEST_DEPOTID);
/* 	it->second->internalAddThing(Item::CreateItem(ITEM_MARKET));
	it->second->internalAddThing(inbox); */
	it->second->internalAddThing(getRewardBag(REWARD_CHEST_DEPOTID, true));
	return it->second.get();
}
...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		...
		if (g_config.getBoolean(ConfigManager::PLAYER_CONSOLE_LOGS)) {
			std::cout << name << " \033[1;32mhas logged in.\033[0m" << std::endl;
		}
...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		if (g_config.getBoolean(ConfigManager::PLAYER_CONSOLE_LOGS)) {
			std::cout << getName() << " \033[1;31mhas logged out.\033[0m" << std::endl;
		}
...
{
    isTimeCurrency = false;
    isWarriorCurrency = false;
    doShopRoutines(npc, shop);
}

void Player::openTimeCurrencyWindow(Npc* npc, const std::list<ShopInfo>& shop)
{
    isTimeCurrency = true;
	 isWarriorCurrency = false;
    doShopRoutines(npc, shop);
...
void Player::openWarriorCurrencyWindow(Npc* npc, const std::list<ShopInfo>& shop)
{
    isWarriorCurrency = true;
	 isTimeCurrency = false;
    doShopRoutines(npc, shop);
}

uint32_t Player::getTimeMoney()
{
	std::unordered_map<uint32_t, uint64_t> prices{
		{31947, 1},
		{31949, 100},
		{31948, 10000},
		{31950, 1000000}
	...
	for (const auto& price : prices) {
		uint32_t item = price.first;
		uint64_t value = price.second;
		uint32_t itemCount = getItemTypeCount(item);
		if (itemCount >= 1) {
			money += itemCount * value;
		}
	}
	return money;
}

void Player::doShopRoutines(Npc* npc, const std::list<ShopInfo>& shop)
{
    shopItemList = shop;
    sendShop(npc);
    sendSaleItemList();
...
void Player::onWalk(Direction& dir)
{
	...
	//setNextAction(OTSYS_TIME() + getStepDuration(dir)); //Make players able to execute actions while walking
}
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	if (sendText) {
		...
		int16_t color = g_config.getNumber(ConfigManager::EXPERIENCE_COLOUR);
	    std::stringstream ss;
	    ss << (uint64_t)exp;	
	    g_game.addAnimatedText(ss.str(), getPosition(), (uint8_t)color);
	}
...
bool Player::updateSaleShopList(const Item* item)
{
	...
	if (itemId != ITEM_GOLD_COIN && itemId != ITEM_PLATINUM_COIN && itemId != ITEM_CRYSTAL_COIN && itemId != ITEM_AMBER_COIN) {
		...
		if (it == shopItemList.end()) {
			...
			if (item->getID() == ITEM_WARRIOR_COIN) {
                goto reloadItemsAndGetOut;
            }

			if (item->getID() == ITEM_TIME_COIN) {
				goto reloadItemsAndGetOut;
			}
			...
			if (!Combat::isInPvpZone(this, targetPlayer) && !isInWar(targetPlayer)) {
				...
				if (getSkull() == SKULL_NONE) {
					if (targetPlayer->getGuild()) {
						for (auto guildMember : targetPlayer->getGuild()->getMembersOnline()) {
							guildMember->sendCreatureSkull(this);
						}
					}
					targetPlayer->sendCreatureSkull(this);
				}
...
Skulls_t Player::getSkullClient(const Creature* creature) const
{
	...
	if (this->getGuild()) {
		bool guildMemberAttacked = false;
		for (auto guildMember : this->getGuild()->getMembersOnline()) {
			if (player->hasAttacked(guildMember)) {
				guildMemberAttacked = true;
				break;
			}
		}
		if (guildMemberAttacked) {
			return SKULL_YELLOW;
		}
	}
...
{
	if ((OTSYS_TIME() - lastToggleMount) < 2000 && !wasMounted) {
