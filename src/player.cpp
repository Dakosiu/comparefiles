...
Player::Player(ProtocolGame_ptr p) :
	Creature(), lastPing(OTSYS_TIME()), lastPong(lastPing), inbox(new Inbox(ITEM_INBOX)), storeInbox(new StoreInbox(ITEM_BAG)), client(std::move(p))
{
...
{
	uint32_t getBase = vocation->getAttackSpeed();
	const Item* weapon = getWeapon(true);
	if (weapon && weapon->getAttackSpeed() > 0) {
		getBase = weapon->getAttackSpeed();
	}

	Player* getCreature = const_cast<Player*>(this);
	if (getCreature) {
		const ItemAttributes::CustomAttribute* attr = getCreature->getCustomAttribute("attackSpeed");
		if (attr != nullptr) {
			getBase = getBase / (static_cast<uint16_t>(boost::get<int64_t>(attr->value))/100.0f);
		}
	}

	double buff = (double)getBuff(BUFF_ATTACKSPEED) / 100;
	if (buff > 0) {
		getBase = (getBase - (getBase * buff));
		if (g_config.getBoolean(ConfigManager::DEBUG_PLAYER_BUFFS)) {
			std::cout << "Base Speed: " << getBase << std::endl;
			std::cout << "Attack Speed Buff%: " << buff * 100 << std::endl;
			std::cout << "Base Speed After Buff : " << getBase << std::endl;
		}
	}

	//const ItemAttributes::CustomAttribute* attr = getCustomAttribute("attackSpeedMultiplier");
	//const ItemAttributes::CustomAttribute* attr = getCustomAttribute("attackSpeedMultiplier");
	return getBase;
}
...
void Player::setSkillLevel(skills_t skill, uint16_t level, bool notify) {

	uint64_t tries = vocation->getReqSkillTries(skill, level);
	uint16_t oldLevel = skills[skill].level;

	if (notify && level > oldLevel) {
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You advanced to {:s} level {:d}.", getSkillName(skill), level));
		g_creatureEvents->playerAdvance(this, skill, oldLevel, level);
	}
...
uint16_t Player::getCurrentWing() const
{
	int32_t value;
...
void Player::setCurrentWing(uint16_t wingId)
{
...
uint16_t Player::getCurrentAuraFront() const
{
	int32_t value;
...
uint16_t Player::getCurrentAuraBack() const
{
	int32_t value;
	if (getStorageValue(PSTRG_AURAS_CURRENTAURA, value)) {
		return value;
...
bool Player::addWings(uint8_t mountId)
{

	const uint8_t tmpWingId = mountId - 1;
...
bool Player::addAura(uint8_t mountId)
{

	const uint8_t tmpWingId = mountId - 1;
	const uint32_t key = PSTRG_AURAS_RANGE_START + (tmpWingId / 31);
...
bool Player::addShader(uint8_t mountId)
{

	const uint8_t tmpWingId = mountId - 1;
	const uint32_t key = PSTRG_SHADERS_RANGE_START + (tmpWingId / 31);
...
void Player::setCurrentAuraBack(uint16_t auraId)
{
	addStorageValue(PSTRG_AURAS_CURRENTAURA, auraId);
}

void Player::setCurrentAuraFront(uint16_t auraId)
{
	addStorageValue(PSTRG_AURAS_CURRENTAURA, auraId);
}
...
void Player::setCurrentShader(uint16_t shaderId) { addStorageValue(PSTRG_SHADERS_CURRENTSHADER, shaderId); }
...
void Player::setMagicLevel(uint32_t level, bool notify)
{
	uint64_t newManaSpent = vocation->getReqMana(level);

	if (notify && level > magLevel) {
		sendTextMessage(MESSAGE_EVENT_ADVANCE, fmt::format("You advanced to magic level {:d}.", level));
	}
...
void Player::onPlacedCreature()
{
	...
	Outfit_t newOutfit = this->getCurrentOutfit();
	newOutfit.lookWings = getCurrentWing();
	newOutfit.lookAuraBack = getCurrentAuraBack();
	newOutfit.lookAuraFront = getCurrentAuraFront();
	newOutfit.lookShader = getCurrentShader();
	this->defaultOutfit = newOutfit;
	
	g_game.internalCreatureChangeOutfit(this, this->defaultOutfit);
}
...
bool Player::canWear(uint32_t lookType, uint8_t addons) const
{
	...
	const Outfit* outfit = Outfits::getInstance().getOutfitByLookType(getVocation()->getId(), lookType);
	if (!outfit) {
		outfit = Outfits::getInstance().getOutfitByAddon(getVocation()->getId(), lookType);
	}
...
bool Player::hasOutfit(uint32_t lookType, uint8_t addons)
{
	...
	if (g_config.getBoolean(ConfigManager::UNLOCKED_ADDONS_FOR_ALL)) {
		return true;
	}
...
bool Player::toggleMount(bool mount)
{
	...
	if (mount) {
		...
		const Outfit* playerOutfit = Outfits::getInstance().getOutfitByLookType(getVocation()->getId(), defaultOutfit.lookType);
		if (!playerOutfit) {
			return false;
		}
...
uint16_t Player::getBasicAttackAnimation() const {
	
/* 	uint16_t ba = this->basicAttack
	if (ba > 0) {
		return ba;
	} */
		
	Outfit_t currentOutfit = this->getDefaultOutfit();
	uint16_t lookType = currentOutfit.lookType;
	uint8_t vocationId = getVocation()->getId();
	const Outfit* outfit = Outfits::getInstance().getOutfitByLookType(vocationId, lookType);
	if (!outfit) {
		outfit = Outfits::getInstance().getOutfitByAddon(vocationId, lookType);
	}
	...
	///this->basicAttack = outfit->basicAttack;
	return outfit->basicAttack;
}