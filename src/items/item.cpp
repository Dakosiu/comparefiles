...
bool Item::equals(const Item* otherItem) const
{
    ...
    if (!attributes && !otherItem->attributes) {
        return true;
    }
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_REFIL: {
			...
			setIntAttr(ITEM_ATTRIBUTE_REFIL, value);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_REFIL)) {
		propWriteStream.write<uint8_t>(ATTR_REFIL);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_REFIL));
	}
...
SoundEffect_t Item::getMovementSound(Cylinder* toCylinder) const {
	...
	switch (items[id].type) {
		case ITEM_TYPE_ARMOR: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_ARMORS;
		}
		case ITEM_TYPE_AMULET: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_NECKLACES;
		}
		case ITEM_TYPE_BOOTS: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_BOOTS;
		}
		case ITEM_TYPE_CONTAINER: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_BACKPACK;
		}
		case ITEM_TYPE_HELMET: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_HELMETS;
		}
		case ITEM_TYPE_LEGS: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_LEGS;
		}
		case ITEM_TYPE_RING: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_RINGS;
		}
		case ITEM_TYPE_DISTANCE: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_DISTANCE;
		}
		case ITEM_TYPE_QUIVER: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_QUIVERS;
		}
		case ITEM_TYPE_VALUABLE: {
			return SOUND_EFFECT_TYPE_ITEM_MOVE_STACKABLE;
		}
...
std::string Item::parseShowAttributesDescription(const Item *item, const uint16_t itemId)
{
	...
	if (itemType.armor != 0 || (item && item->getArmor() != 0) || itemType.showAttributes) {
		...
		if (itemType.abilities) {
			...
			if (itemType.abilities->perfectShotRange != 0) {
					if (begin) {
						begin = false;
						itemDescription << " (";
					} else {
                    ...
                    itemDescription << std::showpos << itemType.abilities->perfectShotDamage << std::noshowpos << " at range " << unsigned(itemType.abilities->perfectShotRange);
			}	
				
/* 			if (itemType.damagereflection > 0) {
					if (begin) {
						begin = false;
						itemDescription << " (";
					} else {
					...
					itemDescription << "damage reflection" << ' ' << std::showpos << itemType.damagereflection << std::noshowpos;
			} */
			
			if (itemType.abilities->reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)] != 0) {
					if (begin) {
						begin = false;
						itemDescription << " (";
					} else {
                    ...
                    itemDescription << "damage reflection" << ' ' << std::showpos << itemType.abilities->reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)] << std::noshowpos;
			}	
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			if (it.abilities) {
				...
				if (it.abilities->perfectShotRange != 0) {
                    ...
                    s << std::showpos << it.abilities->perfectShotDamage << std::noshowpos << " at range " << unsigned(it.abilities->perfectShotRange);
			    }	
				
/* 				if (it.damagereflection > 0) {
					if (begin) {
						begin = false;
						s << " (";
					} else {
					...
					s << "damage reflection" << ' ' << std::showpos << it.damagereflection << std::noshowpos;
				} */
				if (it.abilities->reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)] != 0) {
					if (begin) {
						begin = false;
						s << " (";
					} else {
...
/* 		std::string wand_key = "item_upgrade_wand_damage";
		const ItemAttributes::CustomAttribute* attr = item->getCustomAttribute(wand_key);
		std::string wand_value = "empty";
		if (attr != nullptr) {
			std::string tmp = static_cast<std::string>(boost::get<std::string>(attr->value));
			wand_value = (std::string)tmp;
		}
		if (wand_value != "empty")
		{
		   s << std::endl << "Additional Damage: " << wand_value;
		} */
		
		int32_t wand_damage = 0;
		const ItemAttributes::CustomAttribute* attribute = item->getCustomAttribute("item_upgrade_wand_damage");
		if (attribute != nullptr) {
			wand_damage = static_cast<uint32_t>(attribute->getInt());
		}
        if (wand_damage > 0) {
            s << std::endl << "Additional Damage: " << wand_damage;
        }			
		
		std::string key = "item_upgrade_description";
		const ItemAttributes::CustomAttribute* attr = item->getCustomAttribute(key);
		
		std::string upgradeLevel = "empty";
		if (attr != nullptr) {
			std::string tmp = static_cast<std::string>(boost::get<std::string>(attr->value));
			upgradeLevel = (std::string)tmp;
		}
		
		if (upgradeLevel != "empty")
		{
		s << std::endl << upgradeLevel;
		}
		
		key = "bonuses1";
		attr = item->getCustomAttribute(key);
		
		std::string Bonus1 = "empty";
		if (attr != nullptr) {
			std::string tmp = static_cast<std::string>(boost::get<std::string>(attr->value));
			Bonus1 = (std::string)tmp;
		}
		
		if (Bonus1 != "empty")
		{
		s << std::endl << "Bonus: " << Bonus1;
		}
		
		key = "bonuses2";
		attr = item->getCustomAttribute(key);
		
		std::string Bonus2 = "empty";
		if (attr != nullptr) {
			std::string tmp = static_cast<std::string>(boost::get<std::string>(attr->value));
			Bonus2 = (std::string)tmp;
		}
		
		if (Bonus2 != "empty")
		{
		s << std::endl << " Bonus: " << Bonus2;
		}
		
		key = "bonuses3";
		attr = item->getCustomAttribute(key);
		
		std::string Bonus3 = "empty";
		if (attr != nullptr) {
			std::string tmp = static_cast<std::string>(boost::get<std::string>(attr->value));
			Bonus3 = (std::string)tmp;
		}
		
		if (Bonus3 != "empty")
		{
		s << std::endl << " Bonus: " << Bonus3;
		}
		
		key = "bonuses4";
		attr = item->getCustomAttribute(key);
		
		std::string Bonus4 = "empty";
		if (attr != nullptr) {
			std::string tmp = static_cast<std::string>(boost::get<std::string>(attr->value));
			Bonus4 = (std::string)tmp;
		}
		
		if (Bonus4 != "empty")
		{
		s << std::endl << " Bonus: " << Bonus4;
		}
	}
