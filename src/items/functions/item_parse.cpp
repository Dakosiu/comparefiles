...
void ItemParse::initParse(const std::string& tmpStrValue, pugi::xml_node attributeNode, pugi::xml_attribute keyAttribute, pugi::xml_attribute valueAttribute, ItemType& itemType) {
	...
	ItemParse::parseRefil(tmpStrValue, valueAttribute, itemType);
	ItemParse::parseDamageReflectionFlat(tmpStrValue, valueAttribute, itemType);
	ItemParse::parseDamageReflectionPercent(tmpStrValue, valueAttribute, itemType);
...
void ItemParse::parseRefil(const std::string& tmpStrValue, pugi::xml_attribute valueAttribute, ItemType& itemType) {
	std::string stringValue = tmpStrValue;
	if (stringValue == "refil") {
		itemType.refil = pugi::cast<int32_t>(valueAttribute.value());
	}
}

void ItemParse::parseDamageReflectionFlat(const std::string& tmpStrValue, pugi::xml_attribute valueAttribute, ItemType& itemType) {
	std::string stringValue = tmpStrValue;
	if (stringValue == "damagereflection") {
		itemType.getAbilities().reflectFlat[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)] += pugi::cast<int32_t>(valueAttribute.value());
	}
}

void ItemParse::parseDamageReflectionPercent(const std::string& tmpStrValue, pugi::xml_attribute valueAttribute, ItemType& itemType) {
	std::string stringValue = tmpStrValue;
	if (stringValue == "damagereflectionpercentall") {
...
void ItemParse::parsePerfectShotRange(const std::string& tmpStrValue, pugi::xml_attribute valueAttribute, ItemType& itemType) {
	...
	if (stringValue == "perfectshotrange") {
		itemType.getAbilities().perfectShotRange = pugi::cast<int32_t>(valueAttribute.value());
	}
}
