...
class Item : virtual public Thing
{
	public:
		...
		int32_t getRefil() const {			
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("refilable_potion_charges");
			if (attribute != nullptr) {
				return static_cast<uint32_t>(attribute->getInt());
			}
			
			return items[id].refil;
		}
		
		int32_t getDamageReflection() const {						
			return items[id].damagereflection;
		}
		
		void setRefil(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("refilable_potion_charges");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getPhysicalResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_physical");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_PHYSICALDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addPhysicalResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_physical");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getEnergyResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_energy");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_ENERGYDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addEnergyResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_energy");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getEarthResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_earth");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_EARTHDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addEarthResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_earth");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getFireResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_fire");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_FIREDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addFireResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_fire");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getUndefinedResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_undefined");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_UNDEFINEDDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addUndefinedResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_undefined");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getLifeDrainResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_lifedrain");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_LIFEDRAIN)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addLifeDrainResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_lifedrain");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getManaDrainResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_manadrain");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_MANADRAIN)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addManaDrainResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_manadrain");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}
		...
		int32_t getHealing() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_healing");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_HEALING)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
...
/* 		void addHealing(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_healing");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		} */
		...
		int32_t getDrownResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_drown");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_DROWNDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addDrownResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_drown");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getIceResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_ice");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_ICEDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addIceResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_ice");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}
		
		int32_t getHolyResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_holy");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_HOLYDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addHolyResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_holy");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}

		int32_t getDeathResist() const {
			int32_t value = 0;
			const ItemAttributes::CustomAttribute* attribute = getCustomAttribute("item_bonus_resist_death");
			if (attribute != nullptr) {
				value += static_cast<uint32_t>(attribute->getInt());
			}
			
			if (items[id].abilities) {
			   int32_t itemsValue = items[id].abilities->absorbPercent[combatTypeToIndex(COMBAT_DEATHDAMAGE)];
			   if (itemsValue > 0) {
			      value += itemsValue;
			   }
			}
		...
		void addDeathResist(int32_t value2) {			 
			std::string key = boost::lexical_cast<std::string>("item_bonus_resist_death");
	        ItemAttributes::CustomAttribute value;
	        value.set<int64_t>(value2);
	        setCustomAttribute(key, value);
		}
