...
void Weapon::internalUseWeapon(Player* player, Item* item, Creature* target, int32_t damageModifier) const
{
    ...
    if (damage.secondary.type == COMBAT_NONE) {
		...
		if (player->getVocation()->getBaseId() == VOCATION_PALADIN) 
		{
			if (player->getWheelOfDestinyPerk(36) == 200) {
				if (weaponType == WEAPON_AMMO) {
					if (item->getAmmoType() == AMMO_BOLT)
						damage.critDamage += 10;
					else if (item->getAmmoType() == AMMO_ARROW) {
						damage.physAmplification = -2;
						damage.holyAmplification = -2;
					}
				}
			}
		}
		if (player->getVocation()->getBaseId() == VOCATION_KNIGHT && item->getSlotPosition() == SLOTP_TWO_HAND) {
			uint16_t total = player->getWheelOfDestinyPerk(40);
			if (total >= 1000) 
				damage.critDamage += 12;
			else if (total >= 500)
				damage.critDamage += 8;
			else if (total >= 250)
				damage.critDamage += 4;
...
bool WeaponDistance::useWeapon(Player* player, Item* item, Creature* target) const
{
	...
	if (chance >= uniform_random(1, 100) || (id == 42337)) {
