...
void NpcScriptInterface::registerFunctions()
{
   ...
   lua_register(luaState, "openTimeCurrencyWindow", NpcScriptInterface::luaOpenTimeCurrencyWindow);
	lua_register(luaState, "openWarriorCurrencyWindow", NpcScriptInterface::luaOpenWarriorCurrencyWindow);
...
int NpcScriptInterface::luaOpenWarriorCurrencyWindow(lua_State* L)
{
	//openShopWindow(cid, items, onBuy callback, onSell callback)
	int32_t sellCallback;
	if (lua_isfunction(L, -1) == 0) {
	...
	npc->addShopPlayer(player);
	player->setShopOwner(npc, buyCallback, sellCallback);
	player->openWarriorCurrencyWindow(npc, items);

	pushBoolean(L, true);
	return 1;
}

int NpcScriptInterface::luaOpenTimeCurrencyWindow(lua_State* L)
{
	//openShopWindow(cid, items, onBuy callback, onSell callback)
	int32_t sellCallback;
	if (lua_isfunction(L, -1) == 0) {
	...
	npc->addShopPlayer(player);
	player->setShopOwner(npc, buyCallback, sellCallback);
	player->openTimeCurrencyWindow(npc, items);
