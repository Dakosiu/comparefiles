...
void ProtocolGame::login(const std::string &name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!foundPlayer || g_configManager().getBoolean(ALLOW_CLONES))
	{
		...
		if (!g_game().canConnect(player)) {
			std::ostringstream ss;
			ss << "Too many players connected from this ip address." << "\n" << "Limit for same ip address players is: " << g_configManager().getNumber(MAX_PLAYERS_PER_IP) << ".";
			disconnectClient(ss.str());
	        return;
		}
		...
		std::string delayedRestartString = g_game().getDelayedRestartString();
		if (!delayedRestartString.empty()) {
			disconnectClient(delayedRestartString);
	        return;
		}
...
/* 		time_t ttime = time(0);
		...
		std::string restartTimeString = "21:15";
		...
		if (currentTime >= restartTime) {			
			if (currentTime < restartTime + 5) {
				uint16_t timeleft = restartTime + 5 - currentTime;
				std::ostringstream ss2;
				if (timeleft < 1) {
					ss2 << "Server will open in less than minute.";
				} else {
				    ss2 << "Server will open in " << timeleft << " minutes.";
				}
				disconnectClient(ss2.str());
				return;
			}
		} */
...
/* 	    if (restartTime >= currentTime) {
			if (currentTime < restartTime ) {
			    disconnectClient("Nie mozna sie zalogowac na server teraz.");
			    return;
			}
		} */
		...
		//std::cout << "Player: " << player->getName() << " Sie zalogowal." << std::endl;
		...
		g_game().addPlayerIpAddress(player);
		acceptPackets = true;
	}
	else
	{
...
void ProtocolGame::logout(bool displayEffect, bool forced)
{
	...
	//std::cout << "Removed IP Address:" << player->getIP() << std::endl;
	g_game().removePlayerIpAddress(player);
...
void ProtocolGame::sendCyclopediaCharacterGeneralStats()
{
	...
	for (uint8_t i = SKILL_FIRST; i < SKILL_CRITICAL_HIT_CHANCE; ++i)
	{
		...
		if ((i == SKILL_LIFE_LEECH_AMOUNT) || (i == SKILL_MANA_LEECH_AMOUNT)) {
		   msg.add<uint16_t>(std::min<int32_t>(player->getSkillLevel(i) * 100, std::numeric_limits<uint16_t>::max()));
		} else {
...
/* 		if (!it.abilities)
		...
		if (COMBAT_COUNT == 12)
		{
			absorbs[0] += item->getPhysicalResist();
			absorbs[1] += item->getEnergyResist();
			absorbs[2] += item->getEarthResist();
			absorbs[3] += item->getFireResist();
			absorbs[4] += item->getUndefinedResist();
			absorbs[5] += item->getLifeDrainResist();
			absorbs[6] += item->getManaDrainResist();
			absorbs[7] += item->getHealing();
			absorbs[8] += item->getDrownResist();
			absorbs[9] += item->getIceResist();
			absorbs[10] += item->getHolyResist();
			absorbs[11] += item->getDeathResist();
...
void ProtocolGame::AddCreature(NetworkMessage &msg, const Creature *creature, bool known, uint32_t remove)
{
	...
	else
	{
		...
		{
			// if (creature->getNpc()) {
			// 	msg.addString(fmt::format("<h4><font color=\"#735EF5\"/>{:s}</font></h4>", creature->getName()));
			// } else {
			// 	// msg.addString(fmt::format("<font color=\"#735EF5\"/>{:s}</font>", creature->getName()));
			// 	msg.addString(creature->getName());
			// }
...
void ProtocolGame::AddPlayerStats(NetworkMessage &msg)
{
	...
	//msg.add<uint16_t>(player->getGoldenAccountXpBoost()); // Golden Acc
