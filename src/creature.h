...
class Creature : virtual public Thing
{
	...
	public:
		...
		void setMaxShield(int32_t value) {
			maxShield = value;
		}
		
		int32_t getCurrentShield() {
			return std::max<int32_t>(0, currentShield);
		}
		
		void setCurrentShield(int32_t value) {
			currentShield = std::max<int32_t>(0, value);
		}
		
		void setShield(int32_t value) {
			currentShield = value;
			maxShield = value;
		}
		
		double getShieldPercentage() {
			return (double)(getCurrentShield() * 100 / maxShield);
		}
		...
		virtual int32_t getDashDuration() const {
			return dashDuration;
		}
		void setDashDuration(int16_t dashDur) {
		...
		void setCustomAttribute(std::string& key, R value) {
			...
			if (attrMap) {
				...
				attrMap = new CustomAttributeMap();
			}
			attrMap->emplace(key, value);
		}
		...
		void setCustomAttribute(std::string& key, ItemAttributes::CustomAttribute& value) {
			...
			attrMap->insert(std::make_pair(std::move(key), std::move(value)));
		}
		...
		const ItemAttributes::CustomAttribute* getCustomAttribute(const std::string& key) {
			if (const CustomAttributeMap* customAttrMap = attrMap) {
		...
		bool removeCustomAttribute(const std::string& key) {
			if (CustomAttributeMap* customAttrMap = attrMap) {
				auto it = customAttrMap->find(asLowerCaseString(key));
				if (it != customAttrMap->end()) {
	...
	protected:
		...
		typedef std::unordered_map<std::string, ItemAttributes::CustomAttribute> CustomAttributeMap;
		CustomAttributeMap* attrMap = new CustomAttributeMap();
};
