...
bool IOLoginData::loginserverAuthentication(const std::string& name, const std::string& password, Account& account)
{
	...
	DBResult_ptr result = db.storeQuery(fmt::format("SELECT `id`, `name`, `password`, `secret`, `type`, `premium_ends_at`, `proxy_id` FROM `accounts` WHERE `name` = {:s}", db.escapeString(name)));
	if (!result) {
		return false;
	}
...
{
	Database& db = Database::getInstance();
	return loadPlayer(player, db.storeQuery(fmt::format("SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`,`lookaura`,`lookwings`,`lookshader`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `direction`, `extraHealth`, `extraMana`, `extraHealthAddon`, `extraManaAddon`, `extraHealthRegeneration`, `extraManaRegeneration`, `extraHealing`, `Dodge`, `reflect`, `protectionAll`, `saveStatement` FROM `players` WHERE `id` = {:d}", id)));
}

bool IOLoginData::loadPlayerByIdEvent(Player* player, uint32_t id)
{
	Database& db = Database::getInstance();
	return loadPlayerEvent(player, db.storeQuery(fmt::format("SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `lookaura`, `lookwings`, `lookshader`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `direction`, `extraHealth`, `extraMana`, `extraHealthAddon`, `extraManaAddon`, `extraHealthRegeneration`, `extraManaRegeneration`, `extraHealing`, `Dodge`, `reflect`, `protectionAll`, `saveStatement` FROM `players_event` WHERE `id` = {:d}", id)));
}

bool IOLoginData::loadPlayerByName(Player* player, const std::string& name)
{
	Database& db = Database::getInstance();
	return loadPlayer(player, db.storeQuery(fmt::format("SELECT `id`, `name`, `account_id`, `group_id`, `sex`, `vocation`, `experience`, `level`, `maglevel`, `health`, `healthmax`, `blessings`, `mana`, `manamax`, `manaspent`, `soul`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons`, `lookaura`, `lookwings`, `lookshader`, `posx`, `posy`, `posz`, `cap`, `lastlogin`, `lastlogout`, `lastip`, `conditions`, `skulltime`, `skull`, `town_id`, `balance`, `offlinetraining_time`, `offlinetraining_skill`, `stamina`, `skill_fist`, `skill_fist_tries`, `skill_club`, `skill_club_tries`, `skill_sword`, `skill_sword_tries`, `skill_axe`, `skill_axe_tries`, `skill_dist`, `skill_dist_tries`, `skill_shielding`, `skill_shielding_tries`, `skill_fishing`, `skill_fishing_tries`, `direction`, `extraHealth`, `extraMana`, `extraHealthAddon`, `extraManaAddon`, `extraHealthRegeneration`, `extraManaRegeneration`, `extraHealing`, `Dodge`, `reflect`, `protectionAll`, `saveStatement` FROM `players` WHERE `name` = {:s}", db.escapeString(name))));
}
...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	uint32_t statement = result->getNumber<uint32_t>("saveStatement");
	if (statement == 1) {
		loadPlayerByIdEvent(player, player->getGUID());
		return true;
	}
	...
	player->extraHealth = result->getNumber<int32_t>("extraHealth");
	player->extraMana = result->getNumber<int32_t>("extraMana");

	player->extraHealthAddon = result->getNumber<int32_t>("extraHealthAddon");
	player->extraManaAddon = result->getNumber<int32_t>("extraManaAddon");

	player->extraHealthRegeneration = result->getNumber<int32_t>("extraHealthRegeneration");
	player->extraManaRegeneration = result->getNumber<int32_t>("extraManaRegeneration");

	player->extraHealingBase = result->getNumber<double>("extraHealing");

	player->protectionAllBase = result->getNumber<double>("protectionAll");

	player->Dodge = result->getNumber<int32_t>("Dodge");

	player->reflection = result->getNumber<int32_t>("reflect");

    player->saveStatement = result->getNumber<uint32_t>("saveStatement");
...
bool IOLoginData::loadPlayerEvent(Player* player, DBResult_ptr result)
{
	...
	if ((result = db.storeQuery(fmt::format("SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_items_event` WHERE `player_id` = {:d} ORDER BY `sid` DESC", player->getGUID())))) {
		loadItems(itemMap, result);
	...
	if ((result = db.storeQuery(fmt::format("SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_depotitems_event` WHERE `player_id` = {:d} ORDER BY `sid` DESC", player->getGUID())))) {
		loadItems(itemMap, result);
	...
	if ((result = db.storeQuery(fmt::format("SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_inboxitems_event` WHERE `player_id` = {:d} ORDER BY `sid` DESC", player->getGUID())))) {
		loadItems(itemMap, result);
	...
	if ((result = db.storeQuery(fmt::format("SELECT `pid`, `sid`, `itemtype`, `count`, `attributes` FROM `player_storeinboxitems_event` WHERE `player_id` = {:d} ORDER BY `sid` DESC", player->getGUID())))) {
		loadItems(itemMap, result);
	...
	//load storage map
	if ((result = db.storeQuery(fmt::format("SELECT `key`, `value` FROM `player_storage_event` WHERE `player_id` = {:d}", player->getGUID())))) {
		do {
			player->addStorageValue(result->getNumber<uint32_t>("key"), result->getNumber<int32_t>("value"), true);
		} while (result->next());
	}
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	query << "`extraHealth` = " << player->extraHealth << ',';
	query << "`extraMana` = " << player->extraMana << ',';
	query << "`extraHealthAddon` = " << player->extraHealthAddon << ',';
	query << "`extraManaAddon` = " << player->extraManaAddon << ',';
	query << "`extraHealthRegeneration` = " << player->extraHealthRegeneration << ',';
	query << "`extraManaRegeneration` = " << player->extraManaRegeneration << ',';
	query << "`extraHealing` = " << player->extraHealingBase << ',';
	query << "`Dodge` = " << player->Dodge << ',';
	query << "`reflect` = " << player->reflection << ',';
	query << "`protectionAll` = " << player->protectionAllBase << ',';
	query << "`saveStatement` = " << player->saveStatement << ',';
...
bool IOLoginData::savePlayerEvent(Player* player)
{
	if (player->getHealth() <= 0) {
		player->changeHealth(1);
	}
	...
	//First, an UPDATE query to write the player itself
	std::ostringstream query;
	query << "UPDATE `players_event` SET ";
	query << "`level` = " << player->level << ',';
	query << "`group_id` = " << player->group->id << ',';
	query << "`vocation` = " << player->getVocationId() << ',';
	query << "`health` = " << player->health << ',';
	...
	//item saving
	if (!db.executeQuery(fmt::format("DELETE FROM `player_items_event` WHERE `player_id` = {:d}", player->getGUID()))) {
		return false;
	}

	DBInsert itemsQuery("INSERT INTO `player_items_event` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `attributes`) VALUES ");
	...
	if (!saveItems(player, itemList, itemsQuery, propWriteStream, openContainers)) { //EVENT cant have openContainer and ccant leave without //! Recommend to try understand why it cant and make it possible
		return false;
	}

	if (player->lastDepotId != -1) {
		//save depot items
		if (!db.executeQuery(fmt::format("DELETE FROM `player_depotitems_event` WHERE `player_id` = {:d}", player->getGUID()))) {
			return false;
		}

		DBInsert depotQuery("INSERT INTO `player_depotitems_event` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `attributes`) VALUES ");
		itemList.clear();
	...
	//save inbox items
	if (!db.executeQuery(fmt::format("DELETE FROM `player_inboxitems_event` WHERE `player_id` = {:d}", player->getGUID()))) {
		return false;
	}

	DBInsert inboxQuery("INSERT INTO `player_inboxitems_event` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `attributes`) VALUES ");
	itemList.clear();
	...
	//save store inbox items
	if (!db.executeQuery(fmt::format("DELETE FROM `player_storeinboxitems_event` WHERE `player_id` = {:d}", player->getGUID()))) {
		return false;
	}

	DBInsert storeInboxQuery("INSERT INTO `player_storeinboxitems_event` (`player_id`, `pid`, `sid`, `itemtype`, `count`, `attributes`) VALUES ");
	itemList.clear();
	...
	if (!db.executeQuery(fmt::format("DELETE FROM `player_storage_event` WHERE `player_id` = {:d}", player->getGUID()))) {
		return false;
	}

	DBInsert storageQuery("INSERT INTO `player_storage_event` (`player_id`, `key`, `value`) VALUES ");
	player->genReservedStorageRange();
