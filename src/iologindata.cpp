...
bool IOLoginData::getOutfitByName(const std::string& name,
    uint16_t& lookType, uint8_t& head, uint8_t& body, uint8_t& legs, uint8_t& feet, uint8_t& addons, uint16_t& mount)
{
    Database& db = Database::getInstance();
	std::ostringstream q;
	q << "SELECT `looktype`, `lookhead`, `lookbody`, `looklegs`, `lookfeet`, `lookaddons` "
		 "FROM `players` WHERE `name` = " << db.escapeString(name) << " LIMIT 1";

	if (DBResult_ptr r = db.storeQuery(q.str())) {
		lookType = r->getNumber<uint16_t>("looktype");
		head     = r->getNumber<uint8_t>("lookhead");
		body     = r->getNumber<uint8_t>("lookbody");
		legs     = r->getNumber<uint8_t>("looklegs");
		feet     = r->getNumber<uint8_t>("lookfeet");
		addons   = r->getNumber<uint8_t>("lookaddons");
		mount    = 0; // brak kolumny w DB
		return true;
	}
    return false;
}
...
bool IOLoginData::getLevelByName(const std::string& name, uint16_t& level)
{
    Database& db = Database::getInstance();
    std::ostringstream query;
    query << "SELECT `level` FROM `players` WHERE `name` = " << db.escapeString(name) << " LIMIT 1";
    if (DBResult_ptr result = db.storeQuery(query.str())) {
        level = result->getNumber<uint16_t>("level");
        return true;
    }
    return false;
}
