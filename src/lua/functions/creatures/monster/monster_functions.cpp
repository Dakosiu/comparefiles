...
int MonsterFunctions::luaMonsterGetRealName(lua_State* L) {
	// monster:getRealName()
	Monster *monster = getUserdata<Monster>(L, 1);
	if (monster) {
...
int MonsterFunctions::luaMonsterGetLevel(lua_State* L) {
	// monster:getLevel();
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
...
int MonsterFunctions::luaMonsterSetLevel(lua_State *L) {
	// monster:setLevel(value)
	Monster *monster = getUserdata<Monster>(L, 1);
	if (!monster) {
		reportErrorFunc(getErrorDesc(LUA_ERROR_MONSTER_NOT_FOUND));
		pushBoolean(L, false);
	...
	int32_t value = getNumber<uint32_t>(L, 2);
	monster->setLevel(value);
	return 1;
}

int MonsterFunctions::luaMonsterAddLevel(lua_State *L) {
	// monster:addLevel()
	Monster *monster = getUserdata<Monster>(L, 1);
	if (!monster) {
		reportErrorFunc(getErrorDesc(LUA_ERROR_MONSTER_NOT_FOUND));
...
int MonsterFunctions::luaMonsterSetFamiliar(lua_State* L) {
	// monster:setFamiliar(boolean)
	Monster* monster = getUserdata<Monster>(L, 1);
	if (!monster) {
		lua_pushnil(L);
		return 1;
...
int MonsterFunctions::luaMonsterIsFamiliar(lua_State* L) {
	Monster* monster = getUserdata<Monster>(L, 1);	
	if (monster) {
