...
int PlayerFunctions::luaPlayerGetWheelOfDestiny(lua_State* L)
{
	// player:getWheelOfDestiny()
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	std::map<uint16_t, uint16_t> playerWheel = player->wheelOfDestiny;
	for (int index = 1; index <= 40; index++) {
		if (!playerWheel[index])
			lua_pushnumber(L, 0);
		else
			lua_pushnumber(L, playerWheel[index]);
		lua_rawseti(L, -2, index);
	}
	return 1;
}

int PlayerFunctions::luaPlayerGetWheelOfDestinyPerk(lua_State* L)
{
	// player:getWheelOfDestinyPerk(key)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	}
	uint16_t key = getNumber<uint16_t>(L, 2);
	if (key > 40 || key == 0) {
		lua_pushnil(L);
		return 1;
	}
	lua_pushnumber(L, player->getWheelOfDestinyPerk(key));
	return 1;
}

int PlayerFunctions::luaPlayerSetWheelOfDestinyPerk(lua_State* L)
{
	// player:setWheelOfDestinyPerk(key, value)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	uint16_t value = getNumber<uint16_t>(L, 3, 0);
	player->setWheelOfDestinyPerk(key, value);
	pushBoolean(L, true);
	return 1;
}
...
int PlayerFunctions::luaPlayerGetSkullTime(lua_State* L) {
	...
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->skullTicks);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int PlayerFunctions::luaPlayerGetGoldenAccountXpBoost(lua_State *L) {
	// player:getGrindingXpBoost()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getGoldenAccountXpBoost());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int PlayerFunctions::luaPlayerSetGoldenAccountXpBoost(lua_State *L) {
	// player:setGrindingXpBoost(value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->setGoldenAccountXpBoost(getNumber<uint16_t>(L, 2));
		player->sendStats();
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
...
int PlayerFunctions::luaPlayerGetExperienceBonus(lua_State* L) {
	// player:getExperienceBonus()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getExperienceBonus());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int PlayerFunctions::luaPlayerCanConnect(lua_State* L) {
	// player:canConnect()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		pushBoolean(L, player->canConnect());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int PlayerFunctions::luaPlayerAllowConnection(lua_State* L) {
	// player:addExperience(experience[, sendText = false])
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		bool enabled = getBoolean(L, 2);
		player->setCanConnect(enabled);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int PlayerFunctions::luaPlayerAddExperienceBonus(lua_State* L) {
	// player:addExperience(experience[, sendText = false])
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		double value = getNumber<double>(L, 2);
		player->addExperienceBonus(value);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int PlayerFunctions::luaPlayerRemoveFromWarList(lua_State* L) {
	// player:setCapacity(capacity)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->removeFromWarList(getNumber<uint32_t>(L, 2));
		g_game().sendUpdateCreature(player);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
int PlayerFunctions::luaPlayerAddToWarList(lua_State* L) {
	// player:setCapacity(capacity)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->addToWarList(getNumber<uint32_t>(L, 2));
		g_game().sendUpdateCreature(player);
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
