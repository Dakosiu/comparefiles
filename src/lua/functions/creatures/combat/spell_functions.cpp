...
int SpellFunctions::luaSpellSetCallback(lua_State* L)
{
	// spell:setCallback(key, callback)
	...
	CallBackSpell_t key = getNumber<CallBackSpell_t>(L, 2);
	if (!spell->setCallback(key)) {
		lua_pushnil(L);
		return 1;
	}

	CallBack* callback = spell->getCallback(key);
	if (!callback) {
		lua_pushnil(L);
		return 1;
	}
