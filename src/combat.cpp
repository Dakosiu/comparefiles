...
if (caster) {
		if (target && (Combat::canDoCombat(caster, target) != RETURNVALUE_NOERROR && caster != target)){
			return;
		}
	const auto& events = caster->getCreatureEvents(CREATURE_EVENT_DAMAGECHANGE);
	if (!events.empty()) {
		for (CreatureEvent* creatureEvent : events) {
			if(target) {
			creatureEvent->executeDamageChange(caster, target, damage);
			} else {
			creatureEvent->executeDamageChange(caster, nullptr, damage);
			}
		}
	}
}
	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		...
		if (casterPlayer) {
			...
			if (!damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				...
				uint16_t skill = 0;
				bool randomResult = normal_random(1, 100) <= chance;
				if (randomResult)
					skill += 100 + casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITAMOUNT);

				if(damage.origin == ORIGIN_MELEE || damage.origin == ORIGIN_RANGED) {
					const ItemAttributes::CustomAttribute* attr = casterPlayer->getCustomAttribute("baseDamage");
					if (attr != nullptr) {
						skill += static_cast<uint16_t>(boost::get<int64_t>(attr->value));
						chance = 100;
					}
				}
				if(damage.origin == ORIGIN_SPELL) {
					const ItemAttributes::CustomAttribute* attr = casterPlayer->getCustomAttribute("spellDamage");
					if (attr != nullptr) {
						skill += static_cast<uint16_t>(boost::get<int64_t>(attr->value));
						chance = 100;
					}
				}
				if (chance > 99 && skill > 0 || chance > 0 && skill > 0 && randomResult) {
	...
	if (success) {
		...
		if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
			...
			if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
				...
				bool randomResult = normal_random(1, 100) <= chance;
				if (randomResult)
					skill += casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHAMOUNT) * 100;
				
				const ItemAttributes::CustomAttribute* attr = casterPlayer->getCustomAttribute("healthVamp");
				if (attr != nullptr) {
					skill += static_cast<uint16_t>(boost::get<int64_t>(attr->value));
					chance = 100;
				}
			...
			if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
				...
				bool randomResult = normal_random(1, 100) <= chance;
				if (randomResult)
					skill += casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT) * 100;
				
				const ItemAttributes::CustomAttribute* attr = casterPlayer->getCustomAttribute("manaVamp");
				if (attr != nullptr) {
					skill += static_cast<uint16_t>(boost::get<int64_t>(attr->value));
					chance = 100;
				}
...
void Combat::doAreaCombat(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
	...
	for (Creature* creature : toDamageCreatures) {
		...
		if (!events.empty()) {
			for (CreatureEvent* creatureEvent : events) {
			if(creature) {
			creatureEvent->executeDamageChange(caster, creature, damageCopy);
			} else {
			creatureEvent->executeDamageChange(caster, nullptr, damageCopy);
			}
			}
		}
