	...
	if (damage.primary.type != COMBAT_MANADRAIN) {
		...
		if (casterPlayer) {
			...
			if (!damage.critical && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				int32_t criticalChanceBonus;
				casterPlayer->getStorageValue(10014, criticalChanceBonus);
				uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITCHANCE) + (criticalChanceBonus / 100);
				int32_t criticalDamageBonus;
				casterPlayer->getStorageValue(10015, criticalDamageBonus);
				uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_CRITICALHITAMOUNT) + (criticalDamageBonus / 100);
	...
	if (success) {
		...
		if (!damage.leeched && damage.primary.type != COMBAT_HEALING && casterPlayer && damage.origin != ORIGIN_CONDITION) {
			...
			if (casterPlayer->getHealth() < casterPlayer->getMaxHealth()) {
				int32_t lifeLeechChanceBonus;
				casterPlayer->getStorageValue(10016, lifeLeechChanceBonus);
				uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHCHANCE) + (lifeLeechChanceBonus/100);
				int32_t lifeLeechAmountBonus;
				casterPlayer->getStorageValue(10017, lifeLeechAmountBonus);
				uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_LIFELEECHAMOUNT) + (lifeLeechAmountBonus / 100);
				if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
			...
			if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
				int32_t manaLeechChanceBonus;
				casterPlayer->getStorageValue(10018, manaLeechChanceBonus);
				uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHCHANCE) + (manaLeechChanceBonus/100);
				int32_t manaLeechAmountBonus;
				casterPlayer->getStorageValue(10019, manaLeechAmountBonus);
				uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT) + (manaLeechAmountBonus/100);
				if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
					leechCombat.primary.value = std::round(totalDamage * (skill / 100.));
		...
		if (success) {
			...
			if (casterPlayer && !damage.leeched && damage.primary.type != COMBAT_HEALING && damage.origin != ORIGIN_CONDITION) {
				...
				if (casterPlayer->getMana() < casterPlayer->getMaxMana()) {
					...
					casterPlayer->getStorageValue(10018, manaLeechChanceBonus);
					uint16_t chance = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHCHANCE) + (manaLeechChanceBonus/100);
					int32_t manaLeechhAmountBonus;
					casterPlayer->getStorageValue(10019, manaLeechhAmountBonus);
					uint16_t skill = casterPlayer->getSpecialSkill(SPECIALSKILL_MANALEECHAMOUNT) + (manaLeechhAmountBonus/100);
					if (chance > 0 && skill > 0 && normal_random(1, 100) <= chance) {
						leechCombat.primary.value = std::ceil(totalDamage * ((skill / 100.) + ((targetsCount - 1) * ((skill / 100.) / 10.))) / targetsCount);
						g_game.combatChangeMana(nullptr, casterPlayer, leechCombat);
						casterPlayer->sendMagicEffect(casterPlayer->getPosition(), CONST_ME_MAGIC_BLUE);
