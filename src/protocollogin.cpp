...
void ProtocolLogin::getCharacterList(const std::string& accountName, const std::string& password, const std::string& token, uint16_t version)
{
	...
	for (uint8_t i = 0; i < size; i++) {
		...
		uint16_t level = 1;
		IOLoginData::getLevelByName(character, level); // patrz punkt B
		output->add<uint16_t>(level);		
		
		// OUTFIT – pobierz z DB
		uint16_t lookType = 128; // jakiś sensowny default (male citizen)
		uint8_t  lookHead = 78, lookBody = 69, lookLegs = 58, lookFeet = 94, lookAddons = 0;
		uint16_t lookMount = 0;

		IOLoginData::getOutfitByName(character, lookType, lookHead, lookBody, lookLegs, lookFeet, lookAddons, lookMount);

		// wyślij w UZGODNIONEJ KOLEJNOŚCI:
		output->add<uint16_t>(lookType);
		output->addByte(lookHead);
		output->addByte(lookBody);
		output->addByte(lookLegs);
		output->addByte(lookFeet);
		output->addByte(lookAddons);
		output->add<uint16_t>(lookMount); // opcjonalnie, ale polecam dodać
