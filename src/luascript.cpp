...
void LuaScriptInterface::registerEnums()
{
	...
	registerEnum(ITEM_ATTRIBUTE_IDLEBONUS)
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerMethod("Item", "getIdleBonus", LuaScriptInterface::luaItemGetIdleBonus);
	...
	registerMethod("ItemType", "getIdleBonus", LuaScriptInterface::luaItemTypeGetIdleBonus);
...
int LuaScriptInterface::luaItemGetIdleBonus(lua_State* L)
{
    Item* item = getUserdata<Item>(L, 1);
    if (item) {
        const ItemType& itemType = Item::items[item->getID()]; // użyj getID()
        int idleBonus = itemType.idleBonus; // pole
        lua_pushinteger(L, idleBonus);
    } else {
        lua_pushnil(L);
    }
    return 1;
...
int LuaScriptInterface::luaItemTypeGetIdleBonus(lua_State* L)
{
	// itemType:getIdleBonus()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		lua_pushnumber(L, itemType->idleBonus);
	} else {
		lua_pushnil(L);
	}
	return 1;
