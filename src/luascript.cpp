...
Outfit_t LuaScriptInterface::getOutfit(lua_State* L, int32_t arg)
{
	...
	outfit.lookAuraFront = getField<uint16_t>(L, arg, "lookAuraFront");
	outfit.lookAuraBack = getField<uint16_t>(L, arg, "lookAuraBack");
...
Outfit LuaScriptInterface::getOutfitClass(lua_State* L, int32_t arg)
{
	...
	uint16_t addon1 = getField<uint16_t>(L, arg, "addon1");
	uint16_t addon2 = getField<uint16_t>(L, arg, "addon2");
	uint16_t addon3 = getField<uint16_t>(L, arg, "addon3");
	uint16_t basicAttack = getField<uint16_t>(L, arg, "basicattack");
	lua_pop(L, 8);
	return Outfit(name, lookType, premium, unlocked, addon1, addon2, addon3, basicAttack);
}
...
void LuaScriptInterface::pushOutfit(lua_State* L, const Outfit_t& outfit)
{
	...
	setField(L, "lookAuraBack", outfit.lookAuraBack);
	setField(L, "lookAuraFront", outfit.lookAuraFront);
...
void LuaScriptInterface::registerFunctions()
{
	...
	///buffs
	registerEnum(CONDITION_PARAM_BUFF_DAMAGE)
	registerEnum(CONDITION_PARAM_BUFF_ATTACKSPEED)
	registerEnum(CONDITION_PARAM_BUFF_REFLECT)
	registerEnum(CONDITION_PARAM_BUFF_DEFENSE)
	...
	registerEnum(CREATURE_EVENT_DAMAGECHANGE)
	...
	registerEnum(BUFF_DAMAGE)
	registerEnum(BUFF_ATTACKSPEED)
	registerEnum(BUFF_REFLECT)
	registerEnum(BUFF_DEFENSE)
	registerEnum(BUFF_FIRST)
	...
	registerMethod("Game", "createCustomMonster", LuaScriptInterface::luaGameCreateCustomMonster);
	...
	registerMethod("Creature", "getName2", LuaScriptInterface::luaCreatureGetName);
	...
	registerMethod("Creature", "setCustomAttribute", LuaScriptInterface::luaCreatureSetCustomAttribute);
	registerMethod("Creature", "getCustomAttribute", LuaScriptInterface::luaCreatureGetCustomAttribute);
	registerMethod("Creature", "removeCustomAttribute", LuaScriptInterface::luaCreatureRemoveCustomAttribute);
	
	registerMethod("Creature", "sendProgressBar", LuaScriptInterface::luaCreatureSetProgressBar);
    registerMethod("Creature", "sendUpdateProgressBar", LuaScriptInterface::luaCreatureSetUpdateProgressBar);
	
	
	registerMethod("Creature", "getMaxShield", LuaScriptInterface::luaCreatureGetMaxShield);
	registerMethod("Creature", "setShield", LuaScriptInterface::luaCreatureSetShield);
	...
	registerMethod("Player", "getPoints", LuaScriptInterface::luaPlayerGetPoints);
	registerMethod("Player", "setPoints", LuaScriptInterface::luaPlayerSetPoints);
	registerMethod("Player", "addPoints", LuaScriptInterface::luaPlayerAddPoints);
	registerMethod("Player", "removePoints", LuaScriptInterface::luaPlayerRemovePoints);
	registerMethod("Player", "getDivision", LuaScriptInterface::luaPlayerGetDivision);
	registerMethod("Player", "setDivision", LuaScriptInterface::luaPlayerSetDivision);
	
	registerMethod("Player", "getBasicAttackAnimation", LuaScriptInterface::luaPlayerGetBasicAttackAnimation);
	...
	registerMethod("Player", "getBuff", LuaScriptInterface::luaPlayerGetBuff);
	...
	registerMethod("Monster", "getRealName", LuaScriptInterface::luaMonsterGetRealName);
	...
	registerMethod("CreatureEvent", "onDamageChange", LuaScriptInterface::luaCreatureEventOnCallback);
...
int LuaScriptInterface::luaGameCreateCustomMonster(lua_State* L)
{
	// Game.createMonster(monsterName, position[, extended = false[, force = false]])
	Monster* monster = Monster::createMonster(getString(L, 1));
	if (!monster) {
    ...
    std::string customName = getString(L, 5);
	monster->setName(customName);
...
int LuaScriptInterface::luaCreatureGetCustomAttribute(lua_State* L) {
	// creature:getCustomAttribute(key)
	Creature* item = getUserdata<Creature>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
	}
...
int LuaScriptInterface::luaCreatureSetCustomAttribute(lua_State* L) {
	// creature:setCustomAttribute(key, value)
	Creature* item = getUserdata<Creature>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
...
int LuaScriptInterface::luaCreatureRemoveCustomAttribute(lua_State* L) {
	// creature:removeCustomAttribute(key)
	Creature* item = getUserdata<Creature>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
...
{
	// creature:teleportTo(position[, pushMovement = false, dashDuration=0])
	...
	uint16_t dashDuration = getNumber<int16_t>(L, 4, 0);
	creature->setDashDuration(dashDuration);
...
int LuaScriptInterface::luaCreatureGetDamageMap(lua_State* L)
{
	...
	for (const auto& damageEntry : creature->damageMap) {
		...
		setField(L, "attackerName", damageEntry.second.attackerName);
		setField(L, "attackerGuid", damageEntry.second.attackerGuid);
...
int LuaScriptInterface::luaCreatureSetProgressBar(lua_State* L)
{
    ...
    Creature* creature = getUserdata<Creature>(L, 1);
    uint32_t duration = getNumber<int32_t>(L, 2);
...
int LuaScriptInterface::luaCreatureSetUpdateProgressBar(lua_State* L)
{
    // creature:sendUpdateProgressBar(percent)
    Creature* creature = getUserdata<Creature>(L, 1);
    uint8_t percent = getNumber<int32_t>(L, 2);
    if (creature) {
        g_game.startUpdateProgressBar(creature, percent);
        pushBoolean(L, true);
    }
    else {
        lua_pushnil(L);
...
int LuaScriptInterface::luaCreatureGetMaxShield(lua_State* L)
{
	// creature:getMaxShield()
	Creature* creature = getUserdata<Creature>(L, 1);
	if (creature) {
		lua_pushnumber(L, creature->getMaxShield());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaCreatureSetShield(lua_State* L)
{
	// creature:setShield(value)
	Creature* creature = getUserdata<Creature>(L, 1);
	if (creature) {
		creature->setShield(getNumber<uint32_t>(L, 2));
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
	}
...
{
	// player:setMagicLevel(level, notify)
	bool notify = getBoolean(L, 3, false);
...
{
	// player:setSkillLevel(skillType, level, notify)
	bool notify = getBoolean(L, 4, false);
...
int LuaScriptInterface::luaPlayerGetBuff(lua_State* L)
{
	// player:getBuff(id)
	buffs_t buff = getNumber<buffs_t>(L, 2);
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getBuff(buff));
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaPlayerAddWings(lua_State* L) {
	// player:addWings(mountId or mountName)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	if(mountId && mountId > 0)
	pushBoolean(L, player->addWings(mountId));

	return 1;
}
int LuaScriptInterface::luaPlayerAddAura(lua_State* L) {
	// player:addAura(mountId or mountName)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	if(mountId && mountId > 0)
	pushBoolean(L, player->addAura(mountId));

	return 1;
}
int LuaScriptInterface::luaPlayerAddShader(lua_State* L) {
	// player:addShader(mountId or mountName)
	Player* player = getUserdata<Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	if(mountId && mountId > 0)
	pushBoolean(L, player->addShader(mountId));

	return 1;
}

int LuaScriptInterface::luaPlayerHasShader(lua_State* L) {
	// player:hasShader(mountId or mountName)
	const Player* player = getUserdata<const Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
...
int LuaScriptInterface::luaPlayerHasAura(lua_State* L) {
	// player:hasAura(mountId or mountName)
	const Player* player = getUserdata<const Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
...
int LuaScriptInterface::luaPlayerHasWings(lua_State* L) {
	// player:hasWings(mountId or mountName)
	const Player* player = getUserdata<const Player>(L, 1);
	if (!player) {
		lua_pushnil(L);
		return 1;
	...
	if (wings) {
		pushBoolean(L, player->hasWing(wings));
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaPlayerGetPoints(lua_State* L)
{
	// player:getPoints()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getPoints());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerSetPoints(lua_State* L)
{
	// player:setPoints(int32_t value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
...
int LuaScriptInterface::luaPlayerAddPoints(lua_State* L)
{
	// player:addPoints(int32_t value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		int32_t value = getNumber<int32_t>(L, 2);
...
int LuaScriptInterface::luaPlayerRemovePoints(lua_State* L)
{
	// player:removePoints(int32_t value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		int32_t value = getNumber<int32_t>(L, 2);
...
int LuaScriptInterface::luaPlayerGetDivision(lua_State* L)
{
	// player:getDivision()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		std::string value = player->getDivision();
		pushString(L, value);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerSetDivision(lua_State* L)
{
	// player:setDivision(std::string value)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
...
int LuaScriptInterface::luaPlayerGetBasicAttackAnimation(lua_State* L)
{
	// player:getBasicAttackAnimation()
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getBasicAttackAnimation());
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaPlayerCastSpell(lua_State* L)
{
	...
	if (player && spell) {
		pushBoolean(L, spell->playerCastInstant(player, param, true));
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaMonsterGetRealName(lua_State* L)
{
	// monster:getRealName()
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		pushString(L, monster->getRealName());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaConditionSetOutfit(lua_State* L)
{
	...
	if (isTable(L, 2)) {
		...
		outfit.lookShader = getNumber<uint16_t>(L, 13, outfit.lookShader);
		outfit.lookAuraFront = getNumber<uint16_t>(L, 12, outfit.lookAuraFront);
		outfit.lookAuraBack = getNumber<uint16_t>(L, 11, outfit.lookAuraBack);
...
int LuaScriptInterface::luaCreatureEventType(lua_State* L)
{
	...
	if (creature) {
		...
		} else if (tmpStr == "damagechange") {
			creature->setEventType(CREATURE_EVENT_DAMAGECHANGE);
		} else {
