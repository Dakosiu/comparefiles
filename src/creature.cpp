...
void Creature::addDamagePoints(Creature* attacker, int32_t damagePoints)
{
	...
	if (it == damageMap.end()) {
		...
		cb.attackerName = attacker->getPlayer() ? attacker->getName() : "";
		cb.attackerGuid = attacker->getPlayer() ? attacker->getPlayer()->getGUID() : 0;
...
bool Creature::tryToFixAutoWalk(Direction dir)
{
	...
	for (int x = 0; x < 3; ++x) {
		...
		for (int i = 0; i < 2; ++i) {
			...
			listWalkDir.erase(listWalkDir.begin());
		}
