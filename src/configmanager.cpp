...
bool ConfigManager::load()
{
	...
	boolean[CHECK_DUPLICATES_VALUES] = getGlobalBoolean(L, "checkDuplicateStorages", true);
    ...
    integer[EXPERIENCE_COLOUR] = getGlobalNumber(L, "GainExperienceColour", TEXTCOLOR_WHITE_EXP);
	integer[RECOVERY_HEALTH_COLOUR] = getGlobalNumber(L, "RecoveryHealthColour", TEXTCOLOR_PASTELRED);
	integer[RECOVERY_MANA_COLOUR] = getGlobalNumber(L, "RecoveryManaColour", TEXTCOLOR_MAYABLUE);
	...
	floating[MLVL_BONUSDMG] = getGlobalFloat(L, "levelMonsterBonusAttack", 0);
	floating[MLVL_BONUSSPEED] = getGlobalFloat(L, "levelMonsterBonusSpeed", 0);
	floating[MLVL_BONUSHP] = getGlobalFloat(L, "levelMonsterBonusHealth", 0);
