...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_REFLECTION: {
			int16_t value;
			if (!propStream.read<int16_t>(value)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_REFLECTION, value);
			break;
		}

		case ATTR_EXTRAHEALING: {
			int16_t value;
			if (!propStream.read<int16_t>(value)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_EXTRAHEALING, value);
			break;
		}
		...
		case ATTR_DODGE: {
			...
			setIntAttr(ITEM_ATTRIBUTE_DODGE, value);
			break;
		}
		
		case ATTR_BONUSDAMAGE: {
			int16_t value;
			if (!propStream.read<int16_t>(value)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_BONUSDAMAGE, value);
			break;
		}
		...
		case ATTR_SLOTS: {
			...
			setIntAttr(ITEM_ATTRIBUTE_SLOTS, value);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_REFLECTION)) {
		...
		propWriteStream.write<int16_t>(getIntAttr(ITEM_ATTRIBUTE_REFLECTION));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_EXTRAHEALING)) {
		propWriteStream.write<uint8_t>(ATTR_EXTRAHEALING);
		propWriteStream.write<int16_t>(getIntAttr(ITEM_ATTRIBUTE_EXTRAHEALING));
	}

	if (hasAttribute(ITEM_ATTRIBUTE_DODGE)) {
		propWriteStream.write<uint8_t>(ATTR_DODGE);
		propWriteStream.write<int16_t>(getIntAttr(ITEM_ATTRIBUTE_DODGE));
	}	
	
	if (hasAttribute(ITEM_ATTRIBUTE_BONUSDAMAGE)) {
		propWriteStream.write<uint8_t>(ATTR_BONUSDAMAGE);
		propWriteStream.write<int16_t>(getIntAttr(ITEM_ATTRIBUTE_BONUSDAMAGE));
	}	
	...
	if (hasAttribute(ITEM_ATTRIBUTE_SLOTS)) {
		propWriteStream.write<uint8_t>(ATTR_SLOTS);
		propWriteStream.write<int16_t>(getIntAttr(ITEM_ATTRIBUTE_SLOTS));
	}
...
bool Item::hasProperty(ITEMPROPERTY prop) const
{
	...
	const bool statPointsChest = (item && item->getActionId() >= 11500 && item->getActionId() <= 11600);
	//const bool questChest = (item && item->getContainer() && item->getActionId() > 0 && !hasAttribute(ITEM_ATTRIBUTE_CORPSEOWNER));
	const bool questChest = (item && item->getContainer() && item->getActionId() > 0);
	
	switch (prop) {
		case CONST_PROP_BLOCKSOLID: return it.blockSolid;
		case CONST_PROP_MOVEABLE: return it.moveable && !hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) && !statPointsChest && !questChest;
		...
		case CONST_PROP_IMMOVABLEBLOCKSOLID: return it.blockSolid && (!it.moveable || hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) || statPointsChest || questChest);
		case CONST_PROP_IMMOVABLEBLOCKPATH: return it.blockPathFind && (!it.moveable || hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) || statPointsChest || questChest);
		case CONST_PROP_IMMOVABLENOFIELDBLOCKPATH: return !it.isMagicField() && it.blockPathFind && (!it.moveable || hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) || statPointsChest || questChest);
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
    ...
    int32_t reflection, dodge, extrahealing, slots, bonusDamage;
	...
	if (item) {
		...
		dodge = item->getDodge();
		bonusDamage = item->getBonusDamage();
		extrahealing = item->getExtraHealing();
		slots = item->getSlotsBonus();
	}
	else
	{
		reflection = it.reflection;
		dodge = it.dodge;
		extrahealing = it.extrahealing;
		slots = it.slots;
	}
	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			begin = false;
		} else if (it.weaponType == WEAPON_DISTANCE && it.ammoType == AMMO_NONE) {
			s << " (Range:" << static_cast<uint16_t>(item ? item->getShootRange() : it.shootRange);
			...
			begin = false;
			}else if (it.weaponType != WEAPON_DISTANCE) {//else if (it.weaponType != WEAPON_AMMO) {
			...
			if (dodge != 0) {
                ...
                s << "dodge +" << dodge << std::noshowpos << "%";
            }

			if ((bonusDamage > 1) && (bonusDamage < 1000)) {
				if (begin) {
					begin = false;
					s << " (";
				}
				...
				s << "bonus damage +" << bonusDamage << std::noshowpos << "%";
			}
			...
			if (extrahealing != 0) {
                ...
                s << "extra healing +" << extrahealing << std::noshowpos << "%";
            }
		}
		...
		if (it.abilities) {
			...
			if (extrahealing != 0) {
				...
				if (slots != 0) {
                    ...
                    s << "bonus slots: " << slots << std::noshowpos;
            }
		}
	...
	if (item) {
		...
		if (!specialDescription.empty()) {
			s << '\n' << specialDescription;
		} else if (lookDistance <= 3 && !it.description.empty()) {
			s << '\n' << it.description;
		}
	} else if (lookDistance <= 3 && !it.description.empty()) {
		s << '\n' << it.description;
