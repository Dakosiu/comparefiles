...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_IDLEBONUS: {
			int32_t idleBonus;
			if (!propStream.read<int32_t>(idleBonus)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_IDLEBONUS, idleBonus);
			break;
		}		
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_IDLEBONUS)) {
		propWriteStream.write<uint8_t>(ATTR_IDLEBONUS);
		propWriteStream.write<int32_t>(getIntAttr(ITEM_ATTRIBUTE_IDLEBONUS));
	}	
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
    ...
    int32_t idleBonus = item ? item->getIdleBonus() : it.idleBonus;
    if (idleBonus > 0) {
...
std::string Item::getWeightDescription(const ItemType& it, uint32_t weight, uint32_t count)
{
