...
bool ConditionAttributes::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_BUFF_DAMAGE: {
			buffs[BUFF_DAMAGE] = std::max<int32_t>(0, value);
			return true;
		}
		
		case CONDITION_PARAM_BUFF_ATTACKSPEED: {
			buffs[BUFF_ATTACKSPEED] = std::max<int32_t>(0, value);
			return true;
		}	

		case CONDITION_PARAM_BUFF_REFLECT: {
			buffs[BUFF_REFLECT] = std::max<int32_t>(0, value);
			return true;
		}	
		
		case CONDITION_PARAM_BUFF_DEFENSE: {
			buffs[BUFF_DEFENSE] = std::max<int32_t>(0, value);
			return true;
		}		
...
int32_t ConditionAttributes::getParam(ConditionParam_t param)
{
	switch (param) {
		...
		case CONDITION_PARAM_BUFF_DAMAGE:
		    return buffs[BUFF_DAMAGE];
		
		case CONDITION_PARAM_BUFF_ATTACKSPEED:
			return buffs[BUFF_ATTACKSPEED];

		case CONDITION_PARAM_BUFF_REFLECT:
			return buffs[BUFF_REFLECT];	
			
		case CONDITION_PARAM_BUFF_DEFENSE:
			return buffs[BUFF_DEFENSE];	
