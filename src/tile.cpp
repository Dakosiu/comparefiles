...
{
	ReturnValue localReturn = RETURNVALUE_YOUCANNOTUSETHISBED;
		if (const Creature* creature = thing.getCreature()) {
			if (hasBitSet(FLAG_NOLIMIT, flags)) {
				if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
				localReturn = RETURNVALUE_NOERROR;
			}

			if (hasBitSet(FLAG_PATHFINDING, flags) && hasFlag(TILESTATE_FLOORCHANGE | TILESTATE_TELEPORT)) {
				if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
				localReturn = RETURNVALUE_NOTPOSSIBLE;
			}
				...
				if (monster->canPushCreatures() && !monster->isSummon()) {
					...
					for (const Creature* tileCreature : *creatures) {
						if (!tileCreature->isInGhostMode()) {
							if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
							localReturn = RETURNVALUE_NOTENOUGHROOM;
						}
					}
				}
				...
				if (playerTile && player->isPzLocked()) {
					if (!playerTile->hasFlag(TILESTATE_PVPZONE)) {
						...
						if (hasFlag(TILESTATE_PVPZONE)) {
							if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
							localReturn = RETURNVALUE_PLAYERISPZLOCKEDENTERPVPZONE;
						}
					}
					else if (!hasFlag(TILESTATE_PVPZONE)) {
						// player is trying to leave a pvp zone while being pz-locked
						if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
						localReturn = RETURNVALUE_PLAYERISPZLOCKEDLEAVEPVPZONE;
					}
					...
					if ((!playerTile->hasFlag(TILESTATE_NOPVPZONE) && hasFlag(TILESTATE_NOPVPZONE)) ||
						...
						if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
						localReturn = RETURNVALUE_PLAYERISPZLOCKED;
					}
				}
			}
			...
			Creature* getCreature = const_cast<Creature*>(creature);
				const auto& events = getCreature->getCreatureEvents(CREATURE_EVENT_MOVE);
				if (!events.empty()) {
					for (CreatureEvent* creatureEvent : events) {
						creatureEvent->executeCreatureMove(getCreature, getCreature->getPosition(), getPosition(), localReturn, hasBitSet(FLAG_PATHFINDING, flags));
					}
				}
			...
			if (item->isStoreItem()) {
				if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
				localReturn = RETURNVALUE_ITEMCANNOTBEMOVEDTHERE;
			}
				...
				if (items) {
					for (const Item* tileItem : *items) {
						if (tileItem->isHangable()) {
							if (localReturn == RETURNVALUE_YOUCANNOTUSETHISBED)
								localReturn = RETURNVALUE_NEEDEXCHANGE;
						}
					}
				}
			}
