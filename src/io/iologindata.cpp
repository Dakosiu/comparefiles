...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
  ...
  //double experience_bonus = result->getNumber<double>("experience_bonus");
  //player->experienceBonus = experience_bonus;
...
/* void IOLoginData::addExperienceBonus(uint32_t playerId, double value)
{
  std::ostringstream query;
  query << "UPDATE `players` SET `experience_bonus` = `experience_bonus` + " << value << " WHERE `id` = " << playerId;
  Database::getInstance().executeQuery(query.str());
}

void IOLoginData::removeExperienceBonus(uint32_t playerId, double value)
{
  std::ostringstream query;
  query << "UPDATE `players` SET `experience_bonus` = `experience_bonus` - " << value << " WHERE `id` = " << playerId;
  Database::getInstance().executeQuery(query.str());
