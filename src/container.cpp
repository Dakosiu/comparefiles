...
Container::Container(Tile* tile) : Container(ITEM_BROWSEFIELD, 30, false, true)
{
	...
	if (itemVector) {
		for (Item* item : *itemVector) {
			if ((item->getContainer() || item->hasProperty(CONST_PROP_MOVEABLE)) && !item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID) && !item->hasAttribute(ITEM_ATTRIBUTE_ACTIONID)) {
		...
		uint32_t& maxQueryCount, uint32_t flags) const
{
	...
	if (hasBitSet(FLAG_NOLIMIT, flags)) {
	 ...
	 //if (getName().find("quiver") != std::string::npos && !(item->getName().find("arrow") != std::string::npos)) {

	if (getName().find("quiver") != std::string::npos && !(item->getName().find("arrow") != std::string::npos)) {
		if (getName().find("quiver") != std::string::npos && !(item->getName().find("bolt") != std::string::npos)) {
