...
bool Game::removeCreature(Creature* creature, bool isLogout/* = true*/)
{
	...
	if (Player* player = creature->getPlayer()) {
		//std::cout << "Game: Ip Address:" << player->getIP() << std::endl;
		//std::cout << "Game: Last Ip Address:" << player->getLastIP() << std::endl;
		removePlayerIpAddress(player);
	}
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker, Creature* target, bool checkDefense, bool checkArmor, bool field)
{
	...
	if (damage.primary.type != COMBAT_NONE) {
		...
		if (attacker && attacker->getPlayer()) {
			if (damage.primary.type != COMBAT_HEALING)
				damage.primary.value -= attacker->getPlayer()->getExtraDamage();
			else
				damage.primary.value += attacker->getPlayer()->getExtraHealing();
		}
	...
	if (damage.secondary.type != COMBAT_NONE) {
		...
		if (!damage.extension && attacker && attacker->getPlayer()) {
			if (damage.secondary.type != COMBAT_HEALING)
				damage.secondary.value -= attacker->getPlayer()->getExtraDamage();
			else
				damage.secondary.value += attacker->getPlayer()->getExtraHealing();
		}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage, bool isEvent /*= false*/)
{
	...
	if (damage.primary.value > 0) {
		...
		if (attackerPlayer) {
			...
			if (targetMonster) {
				lifeSkill += targetMonster->getLifeLeechOnHit();
			}
			if (lifeSkill > 0 || normal_random(0, 100) < lifeChance) {
				// Vampiric charm rune
				if (targetMonster) {
				...
				double lifeLeech = (double)lifeSkill / 100;
				uint32_t finalValue = realDamage * lifeLeech;
				tmpDamage.primary.value = finalValue;
				
				//std::cout << "Final Value " << std::round(realDamage * (lifeSkill / 10000.) * (0.2 * affected + 0.9)) / affected << std::endl;
			...
			if (targetMonster)
				manaSkill += targetMonster->getManaLeechOnHit();
			if (manaSkill > 0 || normal_random(0, 100) < manaChance) {
				// Void charm rune
				if (targetMonster) {
				...
				//manaSkill += attackerPlayer->getSkillLevel(SKILL_MANA_LEECH_AMOUNT);
				manaSkill += damage.manaLeech;
				CombatParams tmpParams;
				CombatDamage tmpDamage;
				...
				double manaLeech = (double)manaSkill / 100;
				uint32_t finalValue = realDamage * manaLeech;
				tmpDamage.primary.value = finalValue;
...
bool Game::isIpAddressAdded(uint32_t ip)
{
...
void Game::addPlayerIpAddress(Player* player)
{
	if (player && player->getGroup()->id < account::GROUP_TYPE_GAMEMASTER) {
		uint32_t ip = player->getIP();
		uint32_t id = player->getID();
		std::vector<int> IpAddressArray;
		if (!isIpAddressAdded(ip)) {
            ipAddresses[ip] = IpAddressArray;
	    }
		ipAddresses[ip].push_back(id);
    }
}

void Game::removePlayerIpAddress(Player* player)
{
	if (player && player->getGroup()->id < account::GROUP_TYPE_GAMEMASTER) {
		uint32_t ip = player->getIP();
		if (ip == 0) {
		...
		uint32_t id = player->getID();
	    if (isIpAddressAdded(ip)) {
            auto it = std::find(ipAddresses[ip].begin(), ipAddresses[ip].end(), id);
            if(it != ipAddresses[ip].end()) {
               ipAddresses[ip].erase(it);
            }
        }
    }		
}

bool Game::canConnect(Player* player)
{  
   if (player) {
		...
		uint16_t maxPlayers = g_configManager().getNumber(MAX_PLAYERS_PER_IP);
		if (maxPlayers > 0 && ipAddresses[ip].size() >= maxPlayers) {
			return false;
		}
   }
   return true;
...
std::string Game::getDelayedRestartString() const
{  
   	std::string restartTimeString = g_configManager().getString(DELAYED_SAVE_SERVER);
	if (restartTimeString.empty()) {
		return "";
	}
	...
	std::erase(currentTimeString, ':');
	uint16_t currentTime = stoi(currentTimeString);
	std::erase(restartTimeString, ':');
	uint16_t restartTime = stoi(restartTimeString);
    if (currentTime >= restartTime) {			
		if (currentTime < restartTime + 15) {
			uint16_t timeleft = restartTime + 15 - currentTime;
			std::ostringstream ss2;
			ss2 << "Server will open in " << timeleft << " minutes.";
			return ss2.str();
		}
	}
	return "";
