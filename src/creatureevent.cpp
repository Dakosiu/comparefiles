...
bool CreatureEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (tmpStr == "login") {
		...
		type = CREATURE_EVENT_DAMAGECHANGE;
...
std::string CreatureEvent::getScriptEventName() const
{
	...
	switch (type) {
		...
		case CREATURE_EVENT_DAMAGECHANGE:
			return "onDamageChange";
...
bool CreatureEvent::executeOnLogin(Player* player) const
{
	...
	LuaScriptInterface::pushString(L, player->getPickedRoom());
	LuaScriptInterface::pushString(L, player->getPickedCharacter());
...
void CreatureEvent::executeCreatureMove(Creature* creature, const Position& fromPos, const Position& toPos, ReturnValue& base, bool pathFinding)
{
	//onMove(creature, fromPos, toPos)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeCreatureMove] Call stack overflow" << std::endl;
		return;
	}
	...
	int32_t getNumber = 6;
	if (base == RETURNVALUE_NOERROR)
		getNumber = 0;
	else if (base == RETURNVALUE_NOTPOSSIBLE)
		getNumber = 1;
	else if (base == RETURNVALUE_NOTENOUGHROOM)
		getNumber = 2;
	else if (base == RETURNVALUE_PLAYERISPZLOCKEDENTERPVPZONE)
		getNumber = 3;
	else if (base == RETURNVALUE_PLAYERISPZLOCKEDLEAVEPVPZONE)
		getNumber = 4;
	else if (base == RETURNVALUE_PLAYERISPZLOCKED)
		getNumber = 5;

	lua_pushinteger(L, getNumber);
	LuaScriptInterface::pushBoolean(L, pathFinding);

	if (scriptInterface->protectedCall(L, 5, 1) != 0) {
		LuaScriptInterface::reportError(nullptr, LuaScriptInterface::popString(L));
	}
	else {
		getNumber = std::abs(LuaScriptInterface::getNumber<int32_t>(L, -1));

	if (getNumber == 0)
		base = RETURNVALUE_NOERROR;
	else if (getNumber == 1)
		base = RETURNVALUE_NOTPOSSIBLE;
	else if (getNumber == 2)
		base = RETURNVALUE_NOTENOUGHROOM;
	else if (getNumber == 3)
		base = RETURNVALUE_PLAYERISPZLOCKEDENTERPVPZONE;
	else if (getNumber == 4)
		base = RETURNVALUE_PLAYERISPZLOCKEDLEAVEPVPZONE;
	else if (getNumber == 5)
		base = RETURNVALUE_PLAYERISPZLOCKED;
	else if (getNumber > 5)
		base = RETURNVALUE_NOTPOSSIBLE;
...
void CreatureEvent::executeDamageChange(Creature* creature, Creature* attacker, CombatDamage& damage)
{
	//onDamageChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin)
	if (!scriptInterface->reserveScriptEnv()) {
		std::cout << "[Error - CreatureEvent::executeDamageChange] Call stack overflow" << std::endl;
		return;
	}
