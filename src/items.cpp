...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"reflect", ITEM_PARSE_REFLECTION},
	{"extrahealing", ITEM_PARSE_EXTRAHEALING},
	{"dodge", ITEM_PARSE_DODGE},	
	{"slots", ITEM_PARSE_SLOTS },
	{"bonusdamage", ITEM_PARSE_BONUSDAMAGE },
};
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_REFLECTION: {
					it.reflection = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_EXTRAHEALING: {
					it.extrahealing = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_DODGE: {
					it.dodge = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_SLOTS: {
					it.slots = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
				
				case ITEM_PARSE_BONUSDAMAGE: {
					it.bonusDamage = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
