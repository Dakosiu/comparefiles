...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem, const std::string& getRoom, const std::string& getVocation)
{
	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		player->setName(name);
		player->setPickedRoom(getRoom);
		player->setPickedCharacter(getVocation);
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	std::string getRoom = msg.getString();
	std::string getVocation = msg.getString();
	...
	g_dispatcher.addTask(createTask(std::bind(&ProtocolGame::login, getThis(), characterName, accountId, operatingSystem, getRoom, getVocation)));
}
...
void ProtocolGame::parseSetOutfit(NetworkMessage& msg)
{
	...
	newOutfit.lookAuraBack = otclientV8 ? msg.get<uint16_t>() : 0;
	newOutfit.lookAuraFront = otclientV8 ? msg.get<uint16_t>() : 0;
...
void ProtocolGame::sendDistanceShoot(const Position& from, const Position& to, uint8_t type)
{
	...
	msg.addPosition(to);
	msg.addByte(type);
	//msg.addByte(dashDuration);
	//msg.addByte(motionEffect);
	//msg.addByte(offsetX);
	//msg.addByte(offsetY);
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendOutfitWindow()
{
	...
	const auto& outfits = Outfits::getInstance().getOutfits(player->getVocation()->getId());
	if (outfits.size() == 0) {
		return;
	}
...
/* 	if (player->isAccessPlayer()) {
	...
	for (const Outfit& outfit : outfits) {
		uint8_t addons;
/* 		std::cout << "Outfit Name: " << outfit.name << std::endl;
		std::cout << "Addon 1: " << outfit.addon1 << std::endl;
		std::cout << "Addon 2: " << outfit.addon2 << std::endl;
		std::cout << "Addon 3: " << outfit.addon3 << std::endl;
		
		std::cout << "Addons: " << addons << std::endl; */
		if (!player->getOutfitAddons(outfit, addons)) {
			continue;
		}
        uint16_t id = outfit.lookType;
		uint16_t addon1 = 0;
		uint16_t addon2 = 0;
		uint16_t addon3 = outfit.addon3;
		
		if (player->hasOutfit(id, 1)) {
		   addon1 = outfit.addon1;
		}
		
		if (player->hasOutfit(id, 2)) {
		   addon2 = outfit.addon2;
		}		
		
		protocolOutfits.emplace_back(outfit.name, outfit.lookType, addons, addon1, addon2, addon3);
	...
	for (const ProtocolOutfit& outfit : protocolOutfits) {
		...
		msg.add<uint16_t>(outfit.addon1);
		msg.add<uint16_t>(outfit.addon2);
		msg.add<uint16_t>(outfit.addon3);
	}
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	msg.add<uint16_t>(creature->getDashDuration());
...
void ProtocolGame::AddOutfit(NetworkMessage& msg, const Outfit_t& outfit)
{
	...
	if (otclientV8) {
		...
		msg.add<uint16_t>(outfit.lookAuraBack);
		msg.add<uint16_t>(outfit.lookAuraFront);
...
void ProtocolGame::sendFeatures()
{
	...
	features[GameItemTooltip] = false; // fully available from version 2.6
...
{
	//std::cout << "updateAwareRange" << std::endl;
	if (!otclientV8)
		return;
...
void ProtocolGame::sendProgressbar(uint32_t id, int32_t duration, bool ltr)
{
    NetworkMessage msg;
    msg.addByte(0x3b);
    msg.add<uint32_t>(id);
    msg.add<int32_t>(duration);
    msg.addByte(ltr);
    writeToOutputBuffer(msg);
}

void ProtocolGame::sendUpdateProgressbar(uint32_t id, uint8_t value)
{
    NetworkMessage msg;
    msg.addByte(0x3C);
    msg.add<uint32_t>(id);
