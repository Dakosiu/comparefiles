...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (canSend || operatingSystem >= CLIENTOS_OTCLIENT_LINUX) {
		if(canSend) {
			sendFeatures();
		}
	...
	if (!foundPlayer || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (operatingSystem >= CLIENTOS_OTCLIENT_LINUX || canSend) {
...
void ProtocolGame::onRecvFirstMessage(NetworkMessage& msg)
{
	...
	if(otcV8StringLength == 5 && msg.getString(5) == "OTCv8") {
		//std::cout << "Tu cos w ogole mieli?" << std::endl;
		otclientV8 = msg.get<uint16_t>(); // 253, 260, 261, ...
		//std::cout << "OTCLIENT: " << otclientV8 << std::endl;
	}
	//otclientV8 = 1;
...
bool ProtocolGame::canSee(int32_t x, int32_t y, int32_t z) const
{
	...
	if ((x >= myPos.getX() - awareRange.left() + offsetz) && (x <= myPos.getX() + awareRange.right() + offsetz) && (y >= myPos.getY() - awareRange.top() + offsetz) && (y <= myPos.getY() + awareRange.bottom() + offsetz)) {
		return true;
	}
	return false;
}
...
void ProtocolGame::sendSaleItemList(const std::list<ShopInfo>& shop)
{
	NetworkMessage msg;
	msg.addByte(0x7B);
	if (player->isTimeCurrency == true) {
        msg.add<uint64_t>(player->getTimeMoney());
        //msg.add<uint64_t>(player->getItemTypeCount(ITEM_TIME_COIN));
    } else if (player->isWarriorCurrency == true){
		msg.add<uint64_t>(player->getItemTypeCount(ITEM_WARRIOR_COIN));
	 } else {
...
void ProtocolGame::sendAddCreature(const Creature* creature, const Position& pos, int32_t stackpos, bool isLogin)
{
	...
	if (isLogin) {
		sendMagicEffect(pos, CONST_ME_LOGIN);
	}
...
void ProtocolGame::sendAnimatedText(const std::string& message, const Position& pos, uint8_t color)
{
	if (!canSee(pos)) {
		return;
	}
