...
class ItemAttributes
{
	...
	private:
		...
		const static uint64_t intAttributeTypes = ITEM_ATTRIBUTE_ACTIONID | ITEM_ATTRIBUTE_UNIQUEID | ITEM_ATTRIBUTE_DATE
			...
			| ITEM_ATTRIBUTE_ATTACK_SPEED | ITEM_ATTRIBUTE_REFLECTION | ITEM_ATTRIBUTE_EXTRAHEALING | ITEM_ATTRIBUTE_DODGE | ITEM_ATTRIBUTE_AUTOOPEN | ITEM_ATTRIBUTE_SLOTS 
			| ITEM_ATTRIBUTE_BONUSDAMAGE;
...
class Item : virtual public Thing
{
	public:
		...
		int16_t getExtraHealing() const {
            if (hasAttribute(ITEM_ATTRIBUTE_EXTRAHEALING)) {
                return getIntAttr(ITEM_ATTRIBUTE_EXTRAHEALING);
            }
            return items[id].extrahealing;
        }

		int16_t getDodge() const {
		...
		int16_t getSlotsBonus() const {
			if (hasAttribute(ITEM_ATTRIBUTE_SLOTS)) {
				return getIntAttr(ITEM_ATTRIBUTE_SLOTS);
			}
			return items[id].slots;
		}

		int16_t getBonusDamage() const {
			if (hasAttribute(ITEM_ATTRIBUTE_BONUSDAMAGE)) {
				return getIntAttr(ITEM_ATTRIBUTE_BONUSDAMAGE);
			}
			return items[id].bonusDamage;
		}
		
		int16_t getProtectionAll() const {
			int16_t value = items[id].abilities->absorbPercent[0];			
			if (value == 0) {
				return 0;
			}
			
			for (size_t i = 1; i < COMBAT_COUNT; ++i) {
				int16_t elementValue = items[id].abilities->absorbPercent[i];
                if (elementValue == 0)
				{
					return 0;
				}
