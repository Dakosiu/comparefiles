...
class Player final : public Creature, public Cylinder
{
	public:
		...
		void updateStatisticById(const Statistics_t statisticId, const int16_t points);
		void acceptStatistic(const std::map<Statistics_t, uint16_t>& stats);
		void resetStatistic();
		uint16_t getStatisticPoints() const {
			return statisticPoints;
		}
		uint16_t getStatisticLevel(uint8_t statisticId) const {
		...
		// Tasks
		uint32_t getTaskKills(const std::string& name) const { return m_tasks->getTaskKills(name); }

		bool hasActiveTask(const std::string& name) const { return m_tasks->hasActiveTask(name); }
		bool isActive() const { return m_tasks->isActive(); }

		const TasksList& getTasks() const { return m_tasks->getTasks(); }

		ReturnTaskMessages manageTask(uint8_t id, const std::string& name) { return m_tasks->manageTask(this, id, name); }

		void addTask(const std::string name, uint32_t kills) { m_tasks->addTask(name, kills); }
		void updateTask(const std::string name) { m_tasks->updateTask(this, name); }
		void sendTasks() {
			if (client) {
				client->sendTasks();
			}
		...
		void sendStatistics() const {
			if (client) {
				client->sendStatistics();
			}
		}
	...
	protected:
		...
		uint16_t statistics[STATISTIC_LAST + 1];
		uint16_t statisticPoints = 0;
