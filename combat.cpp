...
bool Combat::closeAttack(Creature* attacker, Creature* target, fightMode_t fightMode, bool fist)
{
	...
	if (Player* player = attacker->getPlayer()) {
		if (player->getVocationId() == 3 || player->getVocationId() == 7) {
			// paladin
			totalDamage += (totalDamage * g_config.getNumber(ConfigManager::PALADIN_DAMAGE)) / 100;
		}

		if (player->getVocationId() == 4 || player->getVocationId() == 8) {
			// paladin
			totalDamage += (totalDamage * g_config.getNumber(ConfigManager::KNIGHT_DAMAGE)) / 100;
		}
	}
...
bool Combat::rangeAttack(Creature* attacker, Creature* target, fightMode_t fightMode)
{
	...
	if (Player* player = attacker->getPlayer()) {
		...
		player->getStorageValue(666, storageValue);
		if(storageValue == 1) {
			removeCharges = false;
		}
	...
	if (weapon->getWeaponType() == WEAPON_DISTANCE) {
		...
		if (Player* player = attacker->getPlayer()) {
			if (player->getVocationId() == 3 || player->getVocationId() == 7) {
				// paladin
				combatDamage.value += (combatDamage.value * g_config.getNumber(ConfigManager::PALADIN_DAMAGE)) / 100;
			}

			if (player->getVocationId() == 4 || player->getVocationId() == 8) {
				// paladin
				combatDamage.value += (combatDamage.value * g_config.getNumber(ConfigManager::KNIGHT_DAMAGE)) / 100;
			}
		}
			...
			if (weapon->getFragility()) {
				if (normal_random(0, 99) <= weapon->getFragility()) {
					...
					if (attacker->getPlayer()) {
						g_events->eventPlayerOnRemoveCount(attacker->getPlayer(), weapon, 1);
					}
		...
		if (ammunition && ammunition->getWeaponType() == WEAPON_AMMO && weapon->getAmmoType() != AMMO_NONE && weapon->getAmmoType() == ammunition->getAmmoType()) {
			...
			if (normal_random(0, 100) <= ammunition->getFragility()) {
				...
				if (attacker->getPlayer()) {
					g_events->eventPlayerOnRemoveCount(attacker->getPlayer(), ammunition, 1);
				}
			}
		...
		if (hitChance == 100) {
			hit = true;
		} else if (normal_random(0, distance) <= skillValue) {
			hit = normal_random(0, 100) <= hitChance;
		}
