...
BehaviourDatabase::BehaviourDatabase(Npc* _npc) : npc(_npc), luaScriptInterface("NPC Interface") {
	topic = 0;
	data = -1;
	type = 0;
	price = 0;
	...
	while (true) {
		...
		if (script.Token == IDENTIFIER) {
			...
			if (identifier == "address") {
				...
				condition->type = BEHAVIOUR_TYPE_MESSAGE_STRING;
				searchTerm = true;
			}
		}
	...
	while (true) {
		...
		if (script.Token == STRING) {
			...
			if (identifier == "idle") {
				...
				action->type = BEHAVIOUR_TYPE_MESSAGE_STRING;
			}
				...
				action->type = BEHAVIOUR_TYPE_TALKER_GUID;
			}
			...
			else if (identifier == "lua") {
				action->type = BEHAVIOUR_TYPE_LUA;
				searchType = BEHAVIOUR_PARAMETER_ONE;
			}
...
NpcBehaviourNode* BehaviourDatabase::readValue(ScriptReader& script)
{
	...
	if (identifier == "topic") {
		...
		node = new NpcBehaviourNode();
		node->type = BEHAVIOUR_TYPE_TALKER_GUID;
	}
	else if (identifier == "price") {
		node = new NpcBehaviourNode();
	...
	else if (identifier == "playerexists") {
		node = new NpcBehaviourNode();
		node->type = BEHAVIOUR_TYPE_PLAYEREXISTS;
	}
	else if (identifier == "hp") {
		node = new NpcBehaviourNode();
...
bool BehaviourDatabase::checkCondition(const NpcBehaviourCondition* condition, Player* player, const std::string& message)
{
	switch (condition->type) {
		...
		case BEHAVIOUR_TYPE_FULL_DIGIT:
			full_digit = true;
			break;
		case BEHAVIOUR_TYPE_MESSAGE_STRING:
			player_string_value = message;
			break;
...
void BehaviourDatabase::checkAction(const NpcBehaviourAction* action, Player* player, const std::string& message)
{
	switch (action->type) {
		...
		case BEHAVIOUR_TYPE_DELETEMONEY:
			...
			break;
		case BEHAVIOUR_TYPE_LUA: {
			luaScriptInterface.reInitState();

			if (luaScriptInterface.loadFile(std::string("data/npc/lua/" + action->expression->string)) == -1) {
				std::cout << "[BehaviourDatabase::checkAction] Failed to load NPC LUA script file " << action->expression->string << ":" << npc->getName() << std::endl;
				std::cout << luaScriptInterface.getLastLuaError() << std::endl;
				return;
			}

			int32_t id = luaScriptInterface.getEvent("onEvent");
			if (id == -1) {
				return;
			}
			...
			env->setScriptId(id, &luaScriptInterface);

			lua_State* L = luaScriptInterface.getLuaState();

			luaScriptInterface.pushFunction(id);

			LuaScriptInterface::pushUserdata<Player>(L, player);
			LuaScriptInterface::setMetatable(L, -1, "Player");

			luaScriptInterface.callFunction(1);
			break;
		}
		...
		case BEHAVIOUR_TYPE_TRANSFER: {
			uint64_t number = evaluate(action->expression, player, message);

			player->bankBalance -= number;

			Player* target = g_game.getPlayerByName(player_string_value);
			if (!target) {
				uint32_t guid = IOLoginData::getGuidByName(player_string_value);
				if (guid == 0) {
					break;
				}

				IOLoginData::increaseBankBalance(guid, number);
			}
			else {
				std::ostringstream ss;
				ss << "You received " << number << " gold coins from " << player->getName() << '.';
				target->sendTextMessage(MESSAGE_INFO_DESCR, ss.str());

				target->bankBalance += number;
			}
			break;
		}
...
{
	switch (node->type) {
		case BEHAVIOUR_TYPE_TALKER_GUID:
		...
		case BEHAVIOUR_TYPE_PLAYEREXISTS: {
			int32_t value = IOLoginData::getGuidByName(player_string_value);
			player_string_value = IOLoginData::getNameByGuid(value);
			return value;
		}
...
std::string BehaviourDatabase::parseResponse(Player* player, const std::string& message)
{
	...
	replaceString(response, "%S", player_string_value);
