...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		boolean[TEST_MODE] = getGlobalBoolean(L, "testMode", false);
	...
	integer[MONEY_RATE] = getGlobalNumber(L, "rateMoney", 1);
	integer[PALADIN_DAMAGE] = getGlobalNumber(L, "paladinDamage", 1);
	integer[KNIGHT_DAMAGE] = getGlobalNumber(L, "knightDamage", 1);
