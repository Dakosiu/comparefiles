...
bool ConditionAttributes::setParam(ConditionParam_t param, int32_t value)
{
	...
	switch (param) {
		...
		case CONDITION_PARAM_BUFF_INCREASESKILLRATE: {
			buffs[BUFF_INCREASESKILLRATE] = std::max<int32_t>(0, value);
			return true;
		}
		
		case CONDITION_PARAM_BUFF_INCREASEHEALING: {
			buffs[BUFF_INCREASEHEALING] = std::max<int32_t>(0, value);
			return true;
		}	

		case CONDITION_PARAM_BUFF_INCREASELOOT: {
			buffs[BUFF_INCREASELOOT] = std::max<int32_t>(0, value);
			return true;
		}

		case CONDITION_PARAM_BUFF_DODGE: {
			buffs[BUFF_DODGE] = std::max<int32_t>(0, value);
			return true;
		}
		
		case CONDITION_PARAM_BUFF_DECREASEMELEEDAMAGE: {
			buffs[BUFF_DECREASEMELEEDAMAGE] = std::max<int32_t>(0, value);
			return true;
		}
		
		case CONDITION_PARAM_BUFF_DAMAGERECEIVED: {
			buffs[BUFF_DAMAGERECEIVED] = std::max<int32_t>(0, value);
			return true;
		}		
