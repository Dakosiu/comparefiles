...
void MoveEvents::clear(bool fromLua)
{
	...
	clearMap(movementIdMap, fromLua);
...
bool MoveEvents::registerEvent(Event_ptr event, const pugi::xml_node& node)
{
	...
	if ((attr = node.attribute("itemid"))) {
		...
		addEvent(std::move(*moveEvent), pugi::cast<int32_t>(attr.value()), movementIdMap);
	} else if ((attr = node.attribute("frommovementid"))) {
		uint32_t id = pugi::cast<uint32_t>(attr.value());
		uint32_t endId = pugi::cast<uint32_t>(node.attribute("tomovementid").value());
		addEvent(std::move(*moveEvent), id, movementIdMap);
		while (++id <= endId) {
...
bool MoveEvents::registerLuaEvent(MoveEvent* event)
{
	...
	} else if (moveEvent->getMovementIdRange().size() > 0) {
		if (moveEvent->getMovementIdRange().size() == 1) {
			int32_t id = moveEvent->getMovementIdRange().at(0);
			addEvent(std::move(*moveEvent), id, movementIdMap);
		} else {
			auto v = moveEvent->getMovementIdRange();
			for (auto i = v.begin(); i != v.end(); i++) {
				addEvent(std::move(*moveEvent), *i, movementIdMap);
			}
		}
...
ReturnValue MoveEvent::EquipItem(MoveEvent* moveEvent, Player* player, Item* item, slots_t slot, bool isCheck)
{
	if (!player->hasFlag(PlayerFlag_IgnoreWeaponCheck) && moveEvent->getWieldInfo() != 0) {
		...
		if (!vocEquipMap.empty() && vocEquipMap.find(player->getVocationId()) == vocEquipMap.end()) {
			return RETURNVALUE_YOURVOCATIONCANNOTUSETHISSPELL;
		}
