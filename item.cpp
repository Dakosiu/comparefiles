	...
	const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.isRune()) {
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			s << " (";
			if (item->getAttack() != 0) {
				uint32_t extraAttack = item->getExtraAttack();
				if (extraAttack > 0) { 
				    s << "Atk:" << static_cast<int>(item->getAttack(false)) << " +" << extraAttack;
				} else {
			...
			if (item->getDefense() != 0) {
				if (item->getAttack() != 0)
					s << " ";
                uint32_t extraDefense = item->getExtraDefense();
				if (extraDefense > 0) {
					s << "Def:" << static_cast<int>(item->getDefense(false)) << " +" << extraDefense;
				} else {
		...
		s << " (";
		if (item->getArmor() > 0) {
			uint32_t extraArmor = item->getExtraArmor();
			if (extraArmor > 0) {
				s << "Arm: " << item->getArmor(false) << " +" << extraArmor;
			} else {
			    s << "Arm:" << item->getArmor();
			}
		}
