...
void ProtocolGame::parsePacket(NetworkMessage& msg)
{
	...
	switch (recvbyte) {
		...
		case 0xEF: parseAcceptStatistic(msg); break;
...
void ProtocolGame::parseSetOutfit(NetworkMessage& msg)
{
	...
	uint16_t wings = msg.get<uint16_t>();
	newOutfit.lookWings = wings;
	std::cout << "Look Wings: " << wings << std::endl; 
...
void ProtocolGame::sendInventoryItem(slots_t slot, const Item* item)
{
	...
	if (item) {
		msg.addByte(0x78);
		msg.addByte(slot);
		const std::string& text = item->getName();
		msg.addString(text);
		msg.addItem(item);
	} else {
		msg.addByte(0x79);
...
void ProtocolGame::sendTasks()
{
	...
	for (auto& [taskName, taskData] : tasks) {
		...
		msg.add<uint32_t>(player->getTaskKills(taskName));
		msg.addByte(player->hasActiveTask(taskName));
		if (updateTasks) {
			msg.addString(taskData.bossName);
			msg.add<uint32_t>(taskData.experience);
			msg.add<uint32_t>(taskData.count);
			AddOutfit(msg, taskData.classOutfit);

			msg.add<uint16_t>(taskData.monsters.size());
			for (auto& [monsterName, monsterOutfit] : taskData.monsters) {
...
void ProtocolGame::sendStatistics()
{
	NetworkMessage msg;
	AddPlayerStatistics(msg);
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::parseAcceptStatistic(NetworkMessage& msg)
{
	switch (msg.getByte())
	{
		...
		{
			// Accept
			std::map<Statistics_t, uint16_t> statistics;
			Statistics_t statisticId = static_cast<Statistics_t>(msg.getByte());
			while (statisticId != 0xFF) {
				statistics.emplace(statisticId, msg.get<uint16_t>());
				statisticId = static_cast<Statistics_t>(msg.getByte());
			}
			addGameTask(&Game::playerAcceptStatistic, player->getID(), statistics);
			break;
		}
		case 1:
		{
			// Reset
			addGameTask(&Game::playerResetStatistic, player->getID());
			break;
		}
		default:
			break;
