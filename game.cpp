	...
	const Position& movingCreaturePos, const Position& toPos)
{
	...
	if(player != movingCreature)
	{
		...
		if(!player->hasFlag(PlayerFlag_CanPushAllCreatures))
		{
			...
			uint64_t protectionLevel = g_config.getNumber(ConfigManager::PROTECTION_LEVEL);
...
bool Game::combatBlockHit(CombatType_t combatType, Creature* attacker, Creature* target,
	int64_t& healthChange, bool checkDefense, bool checkArmor)
{
	if(healthChange > 0)
		return false;
...
bool Game::combatChangeHealth(CombatType_t combatType, Creature* attacker, Creature* target, int64_t healthChange,
	MagicEffect_t hitEffect/* = MAGIC_EFFECT_UNKNOWN*/, Color_t hitColor/* = COLOR_UNKNOWN*/, bool force/* = false*/)
{
	...
	if(healthChange > 0)
	{
		...
		{
			char buffer[100];
			sprintf(buffer, "+%lld", healthChange);
	...
	else
	{
		...
		if(damage != 0)
		{
			...
			{
				int64_t manaDamage = std::min(target->getMana(), damage);
				damage = std::max((int64_t)0, damage - manaDamage);
				if(manaDamage != 0)
				{
					bool deny = false;
					CreatureEventList statsChangeEvents = target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE);
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, int64_t manaChange)
{
	const Position& targetPos = target->getPosition();
	if(manaChange > 0)
	{
	...
	else
	{
		...
		int64_t manaLoss = std::min(target->getMana(), -manaChange);
