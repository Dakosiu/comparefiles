...
void Game::combatGetTypeInfo(CombatType_t combatType, Creature* target, TextColor_t& color, uint8_t& effect)
{
	switch (combatType) {
		...
		case COMBAT_HOLYDAMAGE: {
			...
			effect = CONST_ANI_HOLY;
			break;
		}
		
		case COMBAT_DEATHDAMAGE: {
			color = TEXTCOLOR_LIGHTGREY;
			effect = CONST_ME_DEATH;
			break;
		}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.value > 0) {
		...
		bool isLifeLeeched = false;
		bool isManaLeeched = false;
		std::string lifeLeechDamage;
		std::string manaLeechDamage;
		if (attackerPlayer) {
			if (damage.origin == ORIGIN_MELEE || damage.origin == ORIGIN_RANGED) {
			    uint32_t critChance = attackerPlayer->getCriticalChance();
                if (critChance > 0) {
				...
				double lifeLeech = (double)attackerPlayer->getLifeLeech() / 100;
				if (lifeLeech > 0) {
					uint32_t value = (lifeLeech * damage.value);
					if (value < 1) {
						value = 1;
					}
					CombatDamage leechCombat;
					leechCombat.origin = ORIGIN_NONE;
					leechCombat.value = value;
					attackerPlayer->sendMagicEffect(attackerPlayer->getPosition(), CONST_ME_MAGIC_RED);
					combatChangeHealth(nullptr, attackerPlayer, leechCombat);
					isLifeLeeched = true;
					lifeLeechDamage = std::to_string(value);
				}
				
				double manaLeech = (double)attackerPlayer->getManaLeech() / 100;
				if (manaLeech > 0) {
					uint32_t value = (manaLeech * damage.value);
					if (value < 1) {
						value = 1;
					}
					CombatDamage leechCombat;
					leechCombat.origin = ORIGIN_NONE;
					leechCombat.value = value;
					attackerPlayer->sendMagicEffect(attackerPlayer->getPosition(), CONST_ME_MAGIC_BLUE);
					combatChangeMana(nullptr, attackerPlayer, leechCombat);
					isManaLeeched = true;
					manaLeechDamage = std::to_string(value);
				}				
				
			}

            if (damage.origin == ORIGIN_SPELL) {
                double value = (double)attackerPlayer->getSpellDamage() / 100;
                if (value > 0) {
                    damage.value = (damage.value + (damage.value * value));
                }
            }	
            
			if (target) {
                Monster* attackedMonster = target->getMonster();
			    if (attackedMonster) {
				    double percentage = attackedMonster->getIncreaseDefense();
				    if (percentage > 0) {
				        //std::cout << "Damage Decrease: " << percentage << std::endl;
				        //std::cout << "Damage Before: " << damage.value << std::endl;
				        damage.value = (damage.value - (damage.value * percentage));
				        //std::cout << "Damage After: " << damage.value << std::endl;	
					}
                }
            }				
		...
		if (attacker) {
		    ...
		    if (attackerMonster) {
			    double percentage = attackerMonster->getIncreaseDamage();
			    if (percentage > 0) {
				//std::cout << "Damage Increase: " << percentage << std::endl;
				//std::cout << "Damage Before: " << damage.value << std::endl;
				damage.value = (damage.value + (damage.value * percentage));
				//std::cout << "Damage After: " << damage.value << std::endl;
			   }
		...
		if (color != TEXTCOLOR_NONE) {
			...
			for (Creature* spectator : spectators) {
				...
				if (isLifeLeeched) {
					tmpPlayer->sendAnimatedText(attacker->getPosition(), TEXTCOLOR_RED, lifeLeechDamage);
				}
				if (isManaLeeched) {
					tmpPlayer->sendAnimatedText(attacker->getPosition(), TEXTCOLOR_PURPLE, manaLeechDamage);
				}		
			}
		}
...
void Game::playerAcceptStatistic(uint32_t playerId, const std::map<Statistics_t, uint16_t>& statistics)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
	}
	player->acceptStatistic(statistics);
}
void Game::playerResetStatistic(uint32_t playerId)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
