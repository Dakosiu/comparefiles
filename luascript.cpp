...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnumIn("configKeys", ConfigManager::TEST_MODE)
	...
	registerMethod("Player", "lockHealth", LuaScriptInterface::luaPlayerLockHealth);
	registerMethod("Player", "lockMana", LuaScriptInterface::luaPlayerLockMana);
	registerMethod("Player", "lockStats", LuaScriptInterface::luaPlayerLockStats);
	registerMethod("Player", "lockInventory", LuaScriptInterface::luaPlayerLockInventory);
	registerMethod("Player", "lockStats", LuaScriptInterface::luaPlayerLockStats);

	registerMethod("Player", "setHolyLight", LuaScriptInterface::luaPlayerSetHolyLight);
	...
	registerMethod("MoveEvent", "uid", LuaScriptInterface::luaMoveEventMovementId);
...
int LuaScriptInterface::luaCreatureGetCondition(lua_State* L)
{
	...
	Condition* condition = creature->getCondition(conditionType);
	if (condition) {
...
int LuaScriptInterface::luaCreatureHasCondition(lua_State* L)
{
	...
	ConditionType_t conditionType = getNumber<ConditionType_t>(L, 2);
	pushBoolean(L, creature->hasCondition(conditionType));
	return 1;
}
...
int LuaScriptInterface::luaPlayerLockHealth(lua_State* L)
{
	// player:lockHealth(true or false)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->lockHealth = getBoolean(L, 2);
...
int LuaScriptInterface::luaPlayerLockMana(lua_State* L)
{
	// player:lockMana(true or false)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->lockMana = getBoolean(L, 2);
		lua_pushboolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerLockInventory(lua_State* L)
{
	// player:lockInventory(true or false)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->lockInventory = getBoolean(L, 2);
		lua_pushboolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerLockStats(lua_State* L)
{
	// player:lockStats(true or false)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		player->lockStats = getBoolean(L, 2);
		lua_pushboolean(L, true);
	}
	else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerSetHolyLight(lua_State* L)
{
	// player:setHolyLight(true or false)
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		bool holyLight = player->holyLight;
		player->holyLight = getBoolean(L, 2);

		if (holyLight != player->holyLight) {
			g_game.changeLight(player);
		}
...
{
	// monsterType:addVoice(sentence, yell)
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		voiceBlock_t voice;
		voice.text = getString(L, 2);
		voice.yellText = getBoolean(L, 3);
...
int LuaScriptInterface::luaMoveEventRegister(lua_State* L)
{
	...
	if (moveevent) {
		...
		moveevent->getMovementIdRange().clear();
...
int LuaScriptInterface::luaMoveEventMovementId(lua_State* L)
{
	// moveevent:mid(ids)
	MoveEvent* moveevent = getUserdata<MoveEvent>(L, 1);
	if (moveevent) {
		...
		if (parameters > 1) {
			for (int i = 0; i < parameters; ++i) {
				moveevent->addMovementId(getNumber<uint32_t>(L, 2 + i));
			}
		} else {
			moveevent->addMovementId(getNumber<uint32_t>(L, 2));
		}
		pushBoolean(L, true);
	} else {
		lua_pushnil(L);
