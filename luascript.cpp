...
int LuaScriptInterface::protectedCall(lua_State* L, int nargs, int nresults)
{
	...
	if (scriptInterface) {
		...
		g_stats.addLuaStats(new Stat(ns, scriptInterface->getFileByIdForStats(scriptId), ""));
	}
#endif
...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(CONDITION_PARAM_BUFF_INCREASEDAMAGE)
	registerEnum(CONDITION_PARAM_BUFF_DECREASEDAMAGE)
	registerEnum(CONDITION_PARAM_BUFF_INCREASEEXPERIENCE)
	registerEnum(CONDITION_PARAM_BUFF_INCREASEHEALING)
	registerEnum(CONDITION_PARAM_BUFF_INCREASELOOT)
	registerEnum(CONDITION_PARAM_BUFF_DAMAGERECEIVED)
	registerEnum(CONDITION_PARAM_BUFF_LIFELEECHAMOUNT)
    registerEnum(CONDITION_PARAM_BUFF_LIFELEECHCHANCE)
	registerEnum(CONDITION_PARAM_BUFF_MANALEECHAMOUNT)
    registerEnum(CONDITION_PARAM_BUFF_MANALEECHCHANCE)
	registerEnum(CONDITION_PARAM_STAT_CAPACITY)
	registerEnum(CONDITION_PARAM_STAT_CAPACITYPERCENT)
	registerEnum(CONDITION_PARAM_BUFF_DODGE)
	registerEnum(CONDITION_PARAM_BUFF_INCREASESKILLRATE)
	registerEnum(CONDITION_PARAM_BUFF_INCREASEATTACKSPEED)
	registerEnum(CONDITION_PARAM_BUFF_DECREASEMELEEDAMAGE)
	...
	registerEnum(CONST_ME_DEATHAREA)
	...
	registerEnum(STATISTIC_FIRST)
	registerEnum(STATISTIC_LAST)
	...
	registerMethod("Game", "createEliteMonster", LuaScriptInterface::luaGameCreateEliteMonster);
	...
	registerMethod("Item", "getArmor", LuaScriptInterface::luaItemGetArmor);
	registerMethod("Item", "getDefense", LuaScriptInterface::luaItemGetDefense);
	registerMethod("Item", "getAttack", LuaScriptInterface::luaItemGetAttack);
    registerMethod("Item", "getRealUID", LuaScriptInterface::luaItemGetRealUID);
	
	registerMethod("Item", "getExtraAttack", LuaScriptInterface::luaItemGetExtraAttack);
	registerMethod("Item", "getExtraDefense", LuaScriptInterface::luaItemGetExtraDefense);
	registerMethod("Item", "getExtraArmor", LuaScriptInterface::luaItemGetExtraArmor);
	...
	registerMethod("Player", "addItemEx2", LuaScriptInterface::luaPlayerAddItemEx);
	...
	registerMethod("Player", "getStatistics", LuaScriptInterface::luaPlayerGetStatistics);
	registerMethod("Player", "getStatisticsPoints", LuaScriptInterface::luaPlayerGetStatisticsPoints);
	registerMethod("Player", "getStatisticValue", LuaScriptInterface::luaPlayerGetStatisticValue);
	...
	registerMethod("ItemType", "getRealSlot", LuaScriptInterface::luaItemTypeGetRealSlot);
...
int LuaScriptInterface::luaGameCreateEliteMonster(lua_State* L)
{
	// Game.createEliteMonster(monsterName, position[, extended = false[, force = false]])
	Monster* monster = Monster::createMonster(getString(L, 1));
	if (!monster) {
		lua_pushnil(L);
		return 1;
...
int LuaScriptInterface::luaItemGetExtraAttack(lua_State* L)
{
	// item:getAttack
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
...
int LuaScriptInterface::luaItemGetExtraDefense(lua_State* L)
{
	// item:getAttack
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
...
int LuaScriptInterface::luaItemGetExtraArmor(lua_State* L)
{
	// item:getAttack
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		lua_pushnumber(L, item->getExtraArmor());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
{
	// player:getBuff(specialSkillType)
...
int LuaScriptInterface::luaPlayerGetStatistics(lua_State* L)
{
	Statistics_t id = static_cast<Statistics_t>(lua_tonumber(L, 2));
	Player* player = getUserdata<Player>(L, 1);
	if (player) {
		const StatisticSkills& statisticSkills = PlayerStatistics::getInstance().getStatisticsBySkillId(id);
		lua_createtable(L, statisticSkills.skills.size(), 0);
		int index = 0;
		for (auto& [statisticId, statisticValue] : statisticSkills.skills) {
			lua_createtable(L, 0, 2);
			setField(L, "Id", statisticId);
			setField(L, "Value", statisticValue);
			lua_rawseti(L, -2, ++index);
	    }		
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaPlayerGetStatisticsPoints(lua_State* L)
{
	const Player* player = getUserdata<Player>(L, 1);
	if (player) {
		lua_pushnumber(L, player->getStatisticPoints());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaPlayerGetStatisticValue(lua_State* L)
{
	Player* player = getUserdata<Player>(L, 1);
	uint16_t index = getNumber<uint16_t>(L, 2);
	if (player) {
		lua_pushnumber(L, player->getStatisticLevel(index));
	} else {
		lua_pushnil(L);
	}
	return 1;
...
{
	// monster:setElite(name)
	Monster* monster = getUserdata<Monster>(L, 1);
	if (monster) {
		monster->elite = true;
		monster->customName = getString(L, 2);
		
		uint32_t multiplier = getNumber<uint32_t>(L, 3);
		double percent = (double)multiplier / 100;
		if (percent > 0) {
			uint32_t value = (monster->healthMax + (monster->healthMax * percent));
			monster->healthMax = value;
			monster->health = value;
			g_game.addCreatureHealth(monster);
			
        }
		
		multiplier = getNumber<uint32_t>(L, 4);
		percent = (double)multiplier / 100; 
		if (percent > 0) {
		    int32_t delta = ((monster->getSpeed() * percent) / 2);
	        g_game.changeSpeed(monster, delta);
		}
		
		uint32_t lootRate = getNumber<uint32_t>(L, 5);
		percent = (double)lootRate / 100;
		if (percent > 0) {
			monster->increaseLoot = percent;
		}
		
		uint32_t defenseRate = getNumber<uint32_t>(L, 6);
		percent = (double)defenseRate / 100;
		if (percent > 0) {
			monster->increaseDefense = percent;
		}
		
		uint32_t damageRate = getNumber<uint32_t>(L, 7);
		percent = (double)damageRate / 100;
		if (percent > 0) {
			monster->increaseDamage = percent;
		}	
		
		uint32_t experienceRate = getNumber<uint32_t>(L, 8);
		percent = (double)experienceRate / 100;
		if (percent > 0) {
			monster->increaseExperience = percent;
		}	
...
int LuaScriptInterface::luaItemTypeGetRealSlot(lua_State* L)
{
	// itemType:getRealSlot()
	const ItemType* itemType = getUserdata<const ItemType>(L, 1);
	if (itemType) {
		pushString(L, itemType->realSlot);
	}
	else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaItemTypeGetSpeed(lua_State* L)
{
	...
	{
		Abilities& abilities = itemType->getAbilities();
		lua_pushinteger(L, abilities.speed);
	}
	else {
		lua_pushnil(L);
	}
