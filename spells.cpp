...
bool Spell::playerSpellCheck(Player* player) const
{
	...
	std::string spellName = asLowerCaseString(name);
    if (isInstant()) {
        if (InstantSpell* instantSpell = g_spells->getInstantSpellByName(name)) {
            spellName = instantSpell->getWords();
        }
    }

	if (!g_events->eventPlayerOnCastSpell(player, spellName)) {
		return false;
	}
...
bool InstantSpell::loadFunction(const pugi::xml_attribute& attr, bool)
{
	...
	} else if (strcasecmp(functionName, "conjure") == 0) {
		function = Conjure;
		aggressive = false;
	} else {
...
bool InstantSpell::SummonMonster(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	int32_t t;
	if (player->getStorageValue(120003, t)) {
		if (t > 0) {
			player->sendCancelMessage("You cannot summon monsters inside the event.");
			return false;
		}
	}
...
bool InstantSpell::Illusion(const InstantSpell*, Creature* creature, const std::string& param)
{
	...
	if (player->getStorageValue(120003, t)) {
		if (t > 0) {
			player->sendCancelMessage("You cannot use this spell inside of the event.");
			g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
			return false;
		}
	}
...
bool InstantSpell::Conjure(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	Player* player = creature->getPlayer();
	if (!player) {
		return false;
	...
	const uint32_t conjureCost = spell->getManaCost(player);
	const uint32_t soulCost = spell->getSoulCost();

	if (spell->reagentId != 0) {
		if (player->getMana() < conjureCost) {
			player->sendCancelMessage(RETURNVALUE_NOTENOUGHMANA);
			g_game.addMagicEffect(player->getPosition(), CONST_ME_POFF);
			return false;
		}
		...
		if (item && item->getID() == spell->reagentId && player->getMana() >= conjureCost) {
			foundReagent = true;
			...
			Item* newItem = Item::CreateItem(spell->conjureId, spell->conjureCount);
			if (!newItem) {
				return false;
			}
...
bool RuneSpell::loadFunction(const pugi::xml_attribute& attr, bool)
{
	const char* functionName = attr.as_string();
...
bool RuneSpell::executeUse(Player* player, Item* item, const Position&, Thing* target, const Position& toPosition)
{
	...
	if (hasCharges && item && g_config.getBoolean(ConfigManager::REMOVE_RUNE_CHARGES)) {
		...
		if (player) {
			g_events->eventPlayerOnRemoveCount(player, item, 1);
		}
	}
	return true;
}
