		...
		for (auto childNode : vocationNode.children()) {
			if (strcasecmp(childNode.name(), "skill") == 0) {
				...
				if (meleeDamageAttribute) {
					voc.meleeDamage = pugi::cast<float>(meleeDamageAttribute.value());
					voc.rangedDamage = pugi::cast<float>(childNode.attribute("rangeddamage").value());
				} else {
					std::cout << "[Notice - Vocations::loadFromXml] Missing damage for vocation: " << voc.id << std::endl;
				}
			}
		}
	}
