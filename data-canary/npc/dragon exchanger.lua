local npcName = "Dragon Exchanger"
...
if not DragonExchanger then
   DragonExchanger = {}
   DragonExchanger.customers = {}
end
...
local function creatureSayCallback(npc, player, type, msg)
	...
	local mainTable = DRAGON_EGG_EVENT.config
	if MsgContains(msg, "exchange") then
	   local t = DRAGON_EGG_EVENT.config["Rewards"]
	   local rewardString = dakosLib:addReward(player, t, customString, true, false, false, true, true, true)
	   npcHandler:say("You can exchange dragon fruit's for " .. rewardString, npc, player)
	...
	if exchangeTable then
	   ...
	   local rewardsTable = DRAGON_EGG_EVENT.config["Rewards"]
	   ...
	   DragonExchanger.customers[playerId] = id 
	...
	if npcHandler:getTopic(playerId) == 2 then
	   if MsgContains(msg, "no") then
	      ...
	      DragonExchanger.customers[playerId] = nil
		  npcHandler:say("Then no...", npc, player)
	  elseif MsgContains(msg, "yes") then
	      local id = DragonExchanger.customers[playerId]
