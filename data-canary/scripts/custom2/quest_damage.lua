QUEST_BONUS_DAMAGE = 1215

local quest_bonus_check = TalkAction("!questbonus")
function quest_bonus_check.onSay(player, words, param)
        local currBonus = math.max(player:getStorageValue(QUEST_BONUS_DAMAGE), 0)
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, string.format("Your current bonus damage is %d%%", currBonus))
	return false
end
quest_bonus_check:register()
...
local function playerAddItem(params, item)
	...
	player:setStorageValue(QUEST_BONUS_DAMAGE, math.max(0, player:getStorageValue(QUEST_BONUS_DAMAGE)) + 10)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You got 1% extra bonus damage.")
...
local function playerAddContainerItem(params, item)
	...
	player:setStorageValue(QUEST_BONUS_DAMAGE, math.max(0, player:getStorageValue(QUEST_BONUS_DAMAGE)) + 1)
	player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You got 1% extra bonus damage.")
	player:setStorageValue(params.storage, 1)
	return true
end
...
local creatureevent = CreatureEvent("onQuestDamage")
function creatureevent.onHealthChange(creature, attacker, primaryDamage, primaryType, secondaryDamage, secondaryType, origin)
	   ...
	   if attacker:isPlayer() then
	      if attacker:isPlayer() then 
		     ratio = math.max(attacker:getStorageValue(QUEST_BONUS_DAMAGE), 0) / 100
			 if ratio > 0 then				
