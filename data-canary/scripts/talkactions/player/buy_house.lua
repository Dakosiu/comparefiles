...
function buyHouse.onSay(player, words, param)
	...
	if GOLDEN_ACCOUNT_FEATURES:Houses() then
	   if not player:isGoldenAccount() then
	      player:sendCancelMessage("You need a golden account.")
		  return false
       end
	end

    if player:getLevel() < requiredLevel then
	   player:sendCancelMessage("You need " .. requiredLevel .. " level to buy this house.")
	   return false
	end
