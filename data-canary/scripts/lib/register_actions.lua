...
ActionsLib.useShovel = function(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if target:getAttribute(ITEM_ATTRIBUTE_ACTIONID) and target:getAttribute(ITEM_ATTRIBUTE_ACTIONID) == fishermanTask.aid + 1 then 
	   local t = FISHERMAN_SHOVEL
       local wormid = t.worm_id
	   local randomValue = math.random(1, 100)
	   if randomValue < 25 then
	      player:addItem(wormid, 1)
