function Monster:onDropLoot(corpse)
	...
	if not player or player:getStamina() > 840 then
		...
		-- Rare item drop logic
		local rareItemDropChance = math.random(1, 1000)
		if rareItemDropChance <= 2 then -- Approximately 1 in 500 to 1 in 1000 chance
			local rareItem = corpse:addItem(19083, 1)
			if not rareItem then
				Spdlog.warn(string.format("[4][Monster:onDropLoot] - Could not add rare item to monster: %s, from corpse id: %d.", self:getName(), corpse:getId()))
			end
		end

		local rareItem2DropChance = math.random(1, 150)
		if rareItem2DropChance == 1 then -- 1 in 150 chance
			local rareItem = corpse:addItem(32771, 1)
			if not rareItem then
				Spdlog.warn(string.format("[4][Monster:onDropLoot] - Could not add rare item to monster: %s, from corpse id: %d.", self:getName(), corpse:getId()))
			end
		end
		...
		if player then
			...
			print(corpse:getContentDescription())
			if preyLootPercent > 0 then
...
function Monster:onSpawn(position)
    ...
    --self:registerEvent("FAMILIAR_SYSTEM_onHealthChange")
	self:registerEvent("SUMMON_SYSTEM_onDeath")
	self:registerEvent("SUMMON_SYSTEM_onHealthChange")
	self:registerEvent("onWandItemUpgrade")
	self:registerEvent("tasks_onDeath")
	self:registerEvent("expPlaceBoss_onDeath")
	--self:registerEvent("expPlaceBoss_onDeath")
