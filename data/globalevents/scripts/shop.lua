            ...
            if (cid) then
                ...
                if (add_item_type == 'mount') then
                    ...
                    if (getPlayerStorageValue(cid, 1) == -1) then
                        ...
                        db.query("UPDATE `z_shop_history_item` SET `trans_state`='realized', `trans_real`=" ..
                            os.time() .. " WHERE id = " .. id .. ";")
                    else
                ...
                if (add_item_type == 'addons') then
                    ...
                    if (getPlayerStorageValue(cid, adddonn) == -1) then
                        ...
                        doPlayerSendTextMessage(cid, SHOP_MSG_TYPE,
                            'You received ' .. add_item_name .. ' from The awakening Shop.')
                        db.query("DELETE FROM `z_ots_comunication` WHERE `id` = " .. id .. ";")
                        db.query("UPDATE `z_shop_history_item` SET `trans_state`='realized', `trans_real`=" ..
                            os.time() .. " WHERE id = " .. id .. ";")
                    else
                ...
                if (full_weight <= free_cap) then
                    ...
                    if (type(received_item) == "number" and received_item == RETURNVALUE_NOERROR) then
                        doPlayerSendTextMessage(cid, SHOP_MSG_TYPE,
                            'You received ' .. add_item_name .. ' from The Evotronus Shop.')
                        db.query("DELETE FROM `z_ots_comunication` WHERE `id` = " .. id .. ";")
                        db.query("UPDATE `z_shop_history_item` SET `trans_state`='realized', `trans_real`=" ..
                            os.time() .. " WHERE id = " .. id .. ";")
                    else
                        doPlayerSendTextMessage(cid, SHOP_MSG_TYPE,
                            ' ' ..
                            add_item_name ..
                            ' from Th3 Insanity is waiting for you. Please make place for this item in your backpack/hands and wait about '
                    ...
                    doPlayerSendTextMessage(cid, SHOP_MSG_TYPE,
                        ' ' ..
                        add_item_name ..
                        ' Th3 Insanity is waiting for you. It weight is ' ..
                        full_weight ..
                        ' oz., you have only ' ..
                        free_cap ..
