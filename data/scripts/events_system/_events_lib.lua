...
function startRoomEvent(roomId, roomPlayers)
	...
	if getConfig.barrels then
	    local barrels = #getConfig.barrels
		local maxMonsters = barrels / 2
		local maxChests = barrels - maxMonsters
			...
			if barrelTile then
				...
				if not barrelFound then
					...
					if barrelMonster then
					    ...
					    if monstersCount < maxMonsters and chestsCount < maxChests then
						   local random_number = math.random(1, 100)
	                       if random_number < 50 then
					          barrelMonsters[barrelMonster:getId()] = "Chest"
							  chestsCount = chestsCount + 1
					       else
						      barrelMonsters[barrelMonster:getId()] = "Monster"
							  monstersCount = monstersCount + 1
						   end
					    else
						   if chestsCount == maxChests then
						      barrelMonsters[barrelMonster:getId()] = "Monster"
							  monstersCount = monstersCount + 1
						   elseif monstersCount == maxMonsters then
						      barrelMonsters[barrelMonster:getId()] = "Chest"
							  chestsCount = chestsCount + 1
						   end
						end
...
function Player:findEvent(room, vocation, vocation_string)
	...
	if not vocation and getConfig then
		...
		if eventName == "Rival" or eventName == "1 vs 1" then
		...
		if realEventConfig then
		    --finalTable.rewards = deepCopy(realEventConfig.rewards)
            finalTable.rewards = {}
			-- if not finalTable.rewards then
			    -- finalTable.rewards = {}
			-- end
			
			-- if not finalTable.rewards[player:getName()] then
			    -- finalTable.rewards[player:getName()] = {}
			-- end
			finalTable.rewards[player:getName()] = {}
			finalTable.rewards[player:getName()] = deepCopy(realEventConfig.rewards)
			...
			--print("Ilosc Alpha Pointow: " .. alphaPoints)
			--print("Miejsce: " .. getPlayers)
		    ...
		    if alphaPoints and alphaPoints > 0 then
				...
				--print("Player: " .. player:getName() .. " Otrzmyal " .. alphaPoints .. " Za Miejsce: " .. getPlayers)
			    table.insert(finalTable.rewards[player:getName()], alphaTable)
			end
			
			
			--print("Rewardy: " .. dump(finalTable))
			--print(dump(finalTable.rewards))
			player:addRewards(finalTable.rewards[player:getName()])
	...
	if #roomPlayers == #getConfig.players and #QUEUE_ROOM_SYSTEM:getPlayers(getRoom.name, getRoom.ID) == #getConfig.players then
		...
		eventRoomMemory[roomId].eventName = room
...
function getHighscoreInfo(player, onlyRefresh)	
	...
	if not onlyRefresh then
	    print("Punktow: " .. player:getPoints())
		repetitiveSend(player, HIGHSCORE_OPCODE, "refreshHighscore",{info = _EVENTS_HIGHSCORES, refresh = _EVENTS_HIGHSCORES_LASTREFRESH, time_end = SEASON_RESTART_SYSTEM:getTimeString(), points = player:getPoints(), division = player:getDivision(), pointsString = DIVISION_SYSTEM:getPointsString(player)})
	end
end
