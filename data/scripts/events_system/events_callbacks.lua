		...
		for i,child in pairs(player:getDamageMap()) do
			if child.attackerName and string.len(child.attackerName) > 1 then
				if os.mtime() - child.ticks < evconfig.assistData.duration*1000 then
					...
					local resultAsistance = db.storeQuery("SELECT `event_name`, `points`, `kills_asistance` FROM `"..evconfig.dbs.playersLog.."` WHERE `player_id` = " .. child.attackerGuid .. " AND `event_name` = ".. db.escapeString(getConfig.name) .."")
					if resultAsistance == false then	
						db.query(string.format("INSERT INTO `"..evconfig.dbs.playersLog.."`(`player_id`, `event_name`, `points`, `kills_total`, `kills_combo`, `kills_asistance`, `reward_taken`) VALUES (%s, %s, %s, %s, %s, %s, %s)", child.attackerGuid, db.escapeString(getConfig.name), evconfig.assistData.points, 0, 0, 1, 0))
					else
						...
						db.query("UPDATE `"..evconfig.dbs.playersLog.."` SET `kills_asistance`='".. getAsistance .."', `points`='".. getPoints .."' WHERE `player_id` = " .. child.attackerGuid .. " AND `event_name` = ".. db.escapeString(getConfig.name) .."");
					end
...
function survivalEventLogout.onLogout(player)
	...
	if evconfig.dbs.playersLog then
			...
			print("Participant: " .. player:getName() .. " RewardPoints: " .. getRewardPoints)
			print("Participant: " .. player:getName() .. " Kills: " .. kills)
			print("Participant: " .. player:getName() .. " KillsPoints: " .. getKillsPoints)
			...
			if resultPoints == false then	
				db.query(string.format("INSERT INTO `"..evconfig.dbs.playersLog.."`(`player_id`, `event_name`, `points`, `kills_total`, `kills_combo`, `kills_asistance`, `reward_taken`) VALUES (%s, %s, %s, %s, %s, %s, %s)", player:getGuid(), db.escapeString(getConfig.name), getKillsPoints, kills, kills, 0, 0))
			else
...
function highscoreOpcode.onExtendedOpcode(player, opcode, buffer)
    ...
    elseif opcode == AUTOTRAVEL_OPCODE then
        if action == "teleportTo" then
			player:findEvent(data['room'], true)
			player:prepareEventCharacter(data['vocation'])
        end
    end
end
