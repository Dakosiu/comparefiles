local Config = {
    { BossName = "Dark Hero", position = { x = 830, y = 925, z = 5 } },
    { monsterName = "Hero", count = 53, position = { from_x = 820, to_x = 843, from_y = 893, to_y = 927, z = 5 } },
}

local exhaust = {
    storage = 17121210, -- storage of raid (every raid should have a unique storage)
    delay = 90, -- in minutes
}
...
function action.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if exhaustValue and exhaustValue >= os.time() then
		...
		player:getPosition():sendMagicEffect(849)
        return true
    end
    ...
    addEvent(function()
        for i = 1, #Config do
            ...
            Game.broadcastMessage("Heros and its boss are attacking at Temple Of Dragonia in Evotronus City!", MESSAGE_EVENT_ADVANCE)
        end
    end, 20000)

    addEvent(function()
        Game.broadcastMessage("Many Heros and the Dark Hero have been sent to Evotronus world, humans prepare!", MESSAGE_EVENT_ADVANCE)
    end, 5000)

    addEvent(function()
        Game.broadcastMessage("Many Heros and their Boss are seen near Evotronus city, they are very close, be prepared!", MESSAGE_EVENT_ADVANCE)
    end, 10000)

    addEvent(function()
        Game.broadcastMessage("Warriors, help the weak, and protect Evotronus!", MESSAGE_EVENT_ADVANCE)
    end, 15000)
