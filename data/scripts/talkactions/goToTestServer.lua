local talk = TalkAction("/servers", "!servers")
...
function Player:sendProxyModalWindow()
    ...
    if #proxyList > 0 then
        ...
        window:addButton("Select", function(button, choice)
            db.query("UPDATE `accounts` SET `proxy_id` = " .. choice.proxyId ..
                         " WHERE `id` = " .. accountId)
            local player = Player(playerId)
            if player then
                player:sendTextMessage(MESSAGE_INFO_DESCR, choice.proxyName ..
                                           " Proxy Server selected. Logout and type your account details again to start using it!")
            end
        end)
...
function talk.onSay(player, words, param)
    if player:getStorageValue(testServer) == 1 then
