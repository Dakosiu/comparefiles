...
function onShopCallback(player, opcode, buffer)
	...
	elseif action == "openChest" then
		...
		CHEST_SYSTEM:openChest(player, data.id, data.count)
...
function Player.addRewards(self, t)
    --print("Alpha Dump Table Reward: " .. dump(t))
	--print("Dodaje rewardy dla gracza: " .. self:getName())
	--print("Reward Table: " .. dump(t))
    for i, v in pairs(t) do
		if v.type == "alpha points" then
			if v.count > 0 then
			    local vocation = v.vocation
				local z = POINTS_SYSTEM.earnPoints[vocation]
				--print("POINTS TABLE: " .. dump(z))
				
				
				--print("Generuje " .. v.count .. " Alpha Pointsow.")
				POINTS_SYSTEM:generatePoints(self, v.count, z)
...
function Player.sendChestRewards(self, items, name)
...
function Player.addChestCount(self, index, count)
	...
	CHEST_SYSTEM:addChest(self, t, true)
