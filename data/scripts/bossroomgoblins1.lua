local config = {
    ...
    bossName = "goblin torn",
    bossPosition = Position(793, 946, 6), -- Position where the boss will appear
    bossArea = {
        fromPos = Position(790, 939, 6), -- Upper left corner of the room
        toPos = Position(796, 946, 6), -- Lower right corner of the room
        entrancePos = Position(793, 939, 6), -- Position where players will be teleported when they enter
    ...
    participantsPos = {
        Position(791, 949, 5), -- Player 1, this player should be the one to pull the lever
        Position(792, 949, 5), -- Player 2
    },
    attempts = {
        level = 25, -- Level required to enter
        storage = 20001, -- Storage where we keep the waiting time
        seconds = 120 -- 20 hours
    },
    createTeleportPos = Position(792, 939, 6), -- Position where the teleport is created when the boss dies
...
function action.onUse(player, item, fromPos, target, toPos, isHotkey)
    ...
    boss:registerEvent("bossroomgoblins1")
...
local creatureEvent = CreatureEvent("bossroomgoblins1")

function creatureEvent.onDeath()
    stopEvent(config.kickEventId)
    local teleport = Game.createItem(25417, 1, config.createTeleportPos)
    if teleport then
        teleport:setDestination(config.teleportToPosition)
        addEvent(function ()
            local tile = Tile(config.createTeleportPos)
            if tile then
                local teleport = tile:getItemById(25417)
                if teleport then
                    teleport:remove()
