...
local config = {
    ...
    start = {
        time = "18:00:00", -- time to start event each day
        waiting = '5m' -- wait player to enter event, examples: 10s -> 10 seconds | 2m -> 2 minutes
    },
    area = {
        center = Position(81, 2017, 5), -- center room event
        radius = {
            x = 24, -- radius X
            y = 24 -- radius Y
        }
    ...
    players = {
        ...
        storage = 10000 -- storage, to remove players from the event in case they get trapped! (it should never happen)
    },
    state = {
        type = 'stoped', -- no edit!
    ...
    rewards = { -- In this table, you can add the rewards, as are the example:
        ...
        { itemId = 41211, count = 1, chance = 1 }  -- firestorm token
    },
