...
local config = {
    miscellaneous = {
        name = "Rainstorm", -- event name
        talkaction = "!rainstorm", -- Talkaction words
    ...
    start = {
        time = "9:00:00", -- time to start event each day
        waiting = '5m' -- wait player to enter event, examples: 10s -> 10 seconds | 2m -> 2 minutes
    },
    area = {
        center = Position(101, 2085, 7), -- center room event
        radius = {
            x = 24, -- radius X
            y = 24 -- radius Y
        }
    ...
    players = {
        ...
        corpseId = 10499, -- corpse id :v
        storage = 10002 -- storage, to remove players from the event in case they get trapped! (it should never happen)
    },
    state = {
        type = 'stoped', -- no edit!
    ...
    waves = { -- Wave properties in the event
        ...
        distanceEffect = 68, -- rain effect
        impactEffect = 341, -- impact effect
        warningEffect = 327, -- warning effect
    ...
    rewards = { -- In this table, you can add the rewards, as are the example:
        ...
        { itemId = 41210, count = 1, chance = 1 }  -- rainstorm token
    },
...
function _RSE.isWalkable(x, y, z)
...
local function loadTiles()
    ...
    for x = config.area.center.x - config.area.radius.x, config.area.center.x + config.area.radius.x do
        for y = config.area.center.y - config.area.radius.y, config.area.center.y + config.area.radius.y do
            local tile = _RSE.isWalkable(x, y, config.area.center.z)
            if tile then
...
function _RSE.debug(message, player)
...
function _RSE.removeCachePlayer(player)
...
function _RSE.finish()
    ...
    if winner then
        ...
        _RSE.eventSay(string.format("The player %s is the winner!", winner:getName()))
        config.cache.eventIds['winner'] = addEvent(_RSE.sendRewards, 3000)
    else
        _RSE.close()
    end
end

function _RSE.sendRewards()
    local winner = Player(config.cache.players[1])
    if not winner then
        _RSE.close()
        return
...
function _RSE.killPlayer(player)
    if not _RSE.removeCachePlayer(player) and config.state.debug then
        _RSE.debug(string.format("The player %s could not be removed from the cache, this is weird.", player:getName()))
    end
    local playerPos = player:getPosition()
    player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You are burned.")
    _RSE.resetPlayer(player)
...
function _RSE.resetPlayer(player)
    ...
    for _, tile in pairs(config.cache.tiles) do
        ...
        for _, creature in pairs(creatures) do
            if creature:isPlayer() then
                _RSE.resetPlayer(creature)
            end
        end
    end
end

function _RSE.checkWaiting()
    if #config.cache.players >= config.players.min then
        _RSE.eventPreparing()
        return
    end
    config.state.type = "stoped"
    _RSE.eventSay("The event has been closed due to lack of participants.")
    _RSE.kickPlayers()
end

function _RSE.kickPlayers()
    for _, playerId in pairs(config.cache.players) do
        local player = Player(playerId)
        if player then
            _RSE.resetPlayer(player)
...
function _RSE.kickPlayer(player)
    local index = _RSE.isPlayerExist(player)
    if not index then
        return false
    end
    _RSE.resetPlayer(player)
...
function _RSE.isPlayerExist(player)
    local playerId = player:getId()
...
function _RSE.joinPlayer(player)
...
function _RSE.eventPreparing()
    _RSE.eventSay("We are ready to start, get ready...")
    config.state.type = "preparing"
    config.cache.eventIds['warningLooping'] = addEvent(_RSE.eventWaveWarningLooping, 6000)
end


function _RSE.eventWaveWarningLooping()
    if config.cache.dificulty == 0 then
        ...
        if config.state.debug then
            _RSE.debug(string.format("load default dificulty: %d", config.cache.dificulty))
        end
    ...
    config.cache.eventIds['looping'] = addEvent(_RSE.eventWaveLooping, interval, affectedTileList, interval)
end

function _RSE.eventWaveLooping(affectedTileList, interval)
    for _, tNumber in pairs(affectedTileList) do
        local tile = config.cache.tiles[tNumber]
    ...
    if config.cache.waves > 0 and (config.cache.waves % config.waves.iDWhenWaves.waves == 0) then
        ...
        _RSE.sayToPlayers("The difficulty has increased!")
    end
    config.cache.waves = config.cache.waves +1
    config.cache.eventIds['warningLooping'] = addEvent(_RSE.eventWaveWarningLooping, interval)
end
...
function _RSE.init()
    if config.cache.tileCount == 0 then
        loadTiles()
        if config.state.debug then
            _RSE.debug(string.format("loaded %d tiles.", config.cache.tileCount))
        end
    end
    _RSE.cleanCorpses()
    local s, m ,h = getTime(config.start.waiting)
    local tseconds = s+(m*60)+(h*60*60)
    _RSE.eventSay(string.format("The event will start in %s, use the command %s to enter.", formatTime(tseconds), config.miscellaneous.talkaction))
    config.cache.eventIds['waiting'] = addEvent(_RSE.checkWaiting, tseconds * 1000)
...
local global = GlobalEvent("rainstorm")
...
function talk.onSay(player, words, param)
    ...
    if table.contains({'back', 'exit'}, param:lower()) then
        ...
        elseif not _RSE.kickPlayer(player) then
...
local cEvent = CreatureEvent("rainstorm")
...
local cEvent = CreatureEvent("rainstormLogout")
...
elseif config.state.debug then
    _RSE.debug("Your current version does not support EventCallback functionalities.")
end