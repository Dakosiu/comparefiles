...
for divisor = 1, config.maxTargets do
    ...
    function onGetFormulaValues(player, level, magicLevel)
        local min = (level * 5) + (magicLevel * magicLevel) + 100
        local max = (level * 10) + (magicLevel * magicLevel) + 500
...
function BounceSpell:execute()
    ...
    self.lastTargetPos:sendDistanceEffect(targetPos, 11)
