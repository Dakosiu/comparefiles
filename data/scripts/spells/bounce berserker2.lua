...
local config = {
    name = "Strong Berserkers Bleeding dance",
    words = "exevo gran axetreka hur",
    level = 240,
    magicLevel = 7,
    range = 3,
    cooldown = 2000,
...
for divisor = 1, config.maxTargets do
	...
	local min = (player:getLevel() * 4) + (skill * 7 + attack) + 500
	local max = (player:getLevel() * 8) + (skill * 9 + attack) + 2000
...
function BounceSpell:execute()
    ...
    if not combat then error("Strong Berserkers Bleeding dance Spell: Invalid combat.") end

    local targetPos = target:getPosition()
    self.lastTargetPos:sendDistanceEffect(targetPos, 562)
...
function spell.onCastSpell(player, variant, isHotkey)
    ...
    if not target then error("Strong Berserkers Bleeding dance Spell: Invalid target.") end
