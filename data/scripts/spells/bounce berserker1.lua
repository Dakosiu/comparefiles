...
local config = {
    name = "Berserkers Bleeding dance",
    words = "exevo axetreka hur",
    level = 70,
    magicLevel = 5,
    range = 2,
    cooldown = 2000,
...
for divisor = 1, config.maxTargets do
	...
	local min = (player:getLevel() * 2) + (skill * 5 + attack) + 100
	local max = (player:getLevel() * 4) + (skill * 6 + attack) + 1000
...
function BounceSpell:execute()
    ...
    if not combat then error("Berserkers Bleeding dance Spell: Invalid combat.") end

    local targetPos = target:getPosition()
    self.lastTargetPos:sendDistanceEffect(targetPos, 26)
...
function spell.onCastSpell(player, variant, isHotkey)
    ...
    if not target then error("Berserkers Bleeding dance Spell: Invalid target.") end
...
spell:group("attack")
spell:vocation("Berserker", "Warlord Berserker")
spell:id(1)
