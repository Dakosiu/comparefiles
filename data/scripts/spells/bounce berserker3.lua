...
local config = {
    name = "Powerfull Berserkers Bleeding dance",
    words = "exevo max axetreka hur",
    level = 470,
    magicLevel = 9,
    range = 4,
    cooldown = 2000,
...
for divisor = 1, config.maxTargets do
	...
	local min = (player:getLevel() * 9) + (skill * 11 + attack + attack + attack) + 1000
	local max = (player:getLevel() * 14) + (skill * 14 + attack + attack + attack) + 3000
...
function BounceSpell:execute()
    ...
    if not combat then error("Powerfull Berserkers Bleeding dance Spell: Invalid combat.") end

    local targetPos = target:getPosition()
    self.lastTargetPos:sendDistanceEffect(targetPos, 605)
    ...
    if self:getTargets(target:getPosition()) then
        addEvent(self.execute, 200 + (#self.targets * 10), self)
    end
    return true
end
...
function spell.onCastSpell(player, variant, isHotkey)
    ...
    if not target then error("Powerfull Berserkers Bleeding dance Spell: Invalid target.") end
...
spell:group("attack")
spell:vocation("Warlord Berserker")
spell:id(1)
