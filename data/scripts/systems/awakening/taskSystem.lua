local configTasks = {
    ...
    [2] = {
        nameOfTheTask = "Rotworm", -- same as target name e.g. rat / Rat (doesnt matter, the name will be lowered later anyway)
        looktype = { type = 26 },
        killsRequired = 25,
        rewards = {
            expReward = 1500,
        ...
        elseif (data['action'] == 'finish') then
            for _, task in ipairs(TaskSystem.list) do
                if (task.id == data['entry']) then
                    ...
                    player:setStorageValue(taskPointStorage,
                        (player:getStorageValue(taskPointStorage) + task.taskPoints
                        ))
                    player:sendExtendedJSONOpcode(215, {
...
local taskModalSystem = CreatureEvent("taskModalSystem")
function taskModalSystem.onExtendedOpcode(player, opcode, buffer)
    --print('data/scripts/systems/awakening/taskSystem.lua')
    if opcode == 215 then
        TaskSystem.onAction(player, json.decode(buffer))
    end
end

taskModalSystem:register()

local taskModalSystemOnLogin = CreatureEvent("taskModalSystem_onLogin")

function taskModalSystemOnLogin.onLogin(player)
    player:registerEvent("taskModalSystem")
    player:registerEvent("TaskKill")
    return true
end

taskModalSystemOnLogin:register( -1)
