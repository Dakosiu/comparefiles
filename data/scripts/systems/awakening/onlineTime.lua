...
function onlineTimeRewards.onThink(interval)
   ...
   for _, player in pairs(Game.getPlayers()) do repeat
       ...
       if seconds >= config.seconds then
           ...
           player:sendTextMessage(MESSAGE_INFO_DESCR, "You have received the following reward(s): " .. table.concat(rewards, ', ') .. " for being online 30 minutes.")
           break
       end
