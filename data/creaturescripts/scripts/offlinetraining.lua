...
if table.contains({SKILL_CLUB, SKILL_SWORD, SKILL_AXE, SKILL_DISTANCE}, offlineTrainingSkill) then
    local modifier = topVocation:getAttackSpeed() / 2000  -- Adjusted the division by 2
    updateSkills = player:addOfflineTrainingTries(offlineTrainingSkill, (trainingTime / modifier) / (offlineTrainingSkill == SKILL_DISTANCE and 8 or 4))  -- Adjusted the division by 2
elseif offlineTrainingSkill == SKILL_MAGLEVEL then
    local gainTicks = topVocation:getManaGainTicks()  -- Removed the multiplication by 2
    if gainTicks == 0 then
        gainTicks = 1
    end
...
if updateSkills then
    player:addOfflineTrainingTries(SKILL_SHIELD, trainingTime / 8)  -- Adjusted the division by 2
end
