...
function Player.missionIsStarted(self, questId, missionId)
	...
	if mission then
		...
		if value < mission.startvalue or (not mission.ignoreendvalue and value > mission.endvalue) then
			return false
		end
...
function Player.sendQuestLog(self)
	...
	for questId = 1, #Quests do
		if self:questIsStarted(questId) then
			msg:addU16(questId)
			local str = Quests[questId].name
			if self:questIsCompleted(questId) then
			   str = Quests[questId].name .. " (completed)"
			end
			msg:addString(str)
