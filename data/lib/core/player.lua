...
function Player.addSkillLevel(self, skillId, value)
	...
	if value > 0 then
		...
		print(sum)
		print(self:getSkillTries(skillId))
...
function Player.getSkill(self, skillId)
	if skillId == SKILL_LEVEL then
...
local KRUUM_COIN_STORAGE = 171567
local ALPHA_POINTS_STORAGE = KRUUM_COIN_STORAGE + 1

function Player.getKruumCoins(self)
   return math.max(0, self:getStorageValue(KRUUM_COIN_STORAGE))
end

function Player.addKruumCoins(self, value)
   self:setStorageValue(KRUUM_COIN_STORAGE, self:getKruumCoins() + value)
end

function Player.getAlphaPoints(self)
   return math.max(0, self:getStorageValue(ALPHA_POINTS_STORAGE))
end

function Player.addAlphaPoints(self, value)
   self:setStorageValue(ALPHA_POINTS_STORAGE, self:getAlphaPoints() + value)
end
