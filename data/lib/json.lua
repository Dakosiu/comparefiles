...
json = {}
local actualI=0
local actualSended=0
function json.sendJSONEvent(player, opcode, action, data)
	local msg = NetworkMessage()
	local getPlayer = Player(player)
	if getPlayer then
	msg:addByte(50)
	msg:addByte(opcode)
	if action== 'wikipediaMonsters' then
	printTable(data,"")
	end
	msg:addString(OBJDEF:encode({action = action, data = data},nil,{decodeNumbersAsObjects=true,decodeIntegerObjectificationLength=99999}))
	msg:sendToPlayer(player)
		actualSended = (actualSended+1)
	if actualI == actualSended then
	end
	end
end
function json.sendJSONAdapt(player, opcode, action, data)
	
	
		local getData = json.encode(data)
		local lastGroup = math.ceil(string.len(getData)/5000)

		for i = 1,lastGroup do
		print("send LAstgr "..i.." "..action)
		local sendData = string.sub(getData, ((i-1)*5000)+1, i*5000)
	if action== 'wikipediaMonsters' then
		--print(sendData)
		end
		local sendFirst = "false"
		local sendLast = "false"
		print("sending "..action.." "..actualI)
		if i == lastGroup then
		print("send LAst")
		sendLast = "true"
		end
		...
		actualI = (actualI+1)
        addEvent(json.sendJSONEvent,(actualI*300), player:getId(), opcode, action, {opcodeDataFirst = sendFirst,opcodeDataLast = sendLast,opcodeData = sendData} )
		end
...
function splitTable(table,returnVariable,mainTable,addedV,mainVariable,lastGroup)
    local newSpaces = spaces .. "  "
    for k, v in pairs(table) do
        if type(v) == "table" then
	if not returnVariable[v] then returnVariable[v] = {} end
            if type(k) == "table" then
                for k2, v2 in pairs(k) do
				
		local getSize = json.encode(returnVariable)
		local getNewSize = json.encode(v2)
		if (string.len(getSize) + string.len(getNewSize)) < 7000 then
		if not addedV[v2] then
		returnVariable[v][k2] = v2
		addedV[v2] = true
               return splitTable(v,returnVariable[v],mainTable, addedV, returnVariable,lastGroup)
		end
		else
		mainTable[lastGroup] = deepCopy(returnVariable)
		lastGroup = lastGroup + 1
		returnVariable = {}
               return splitTable(mainTable,returnVariable,mainTable, addedV, returnVariable,lastGroup)
		end
                end
				
            else
				returnVariable[v] = k
               return splitTable(v, returnVariable[v], mainTable)
            end
        else
            if type(k) == "table" then
                local Tstr = newSpaces .. "{"
...
return OBJDEF:encode(data,nil,{decodeNumbersAsObjects=true,decodeIntegerObjectificationLength=99999})
end
function json.decode(data)
return OBJDEF:decode(data,nil,{decodeNumbersAsObjects=true,decodeIntegerObjectificationLength=99999})
end
