...
function Player.setPointsBalance(self, points)
	db.query("UPDATE `"..GameStore.table.."` SET `"..GameStore.tableName.."` = " .. points .. " WHERE `account_id` = " .. self:getAccountId())
	return true
end
...
function Creature:sendAttributes(player)
	...
	print("sendtable 2")
		sendTable.chests[1] = {ammount = self:getStorageValue(Wikipedia.storageTable["Chest"]), icon="/images/game/store/chest1"}
		sendTable.chests[2] = {ammount = self:getStorageValue(Wikipedia.storageTable["Chest2"]), icon="/images/game/store/chest2"}
		sendTable.chests[3] = {ammount = self:getStorageValue(Wikipedia.storageTable["Chest3"]), icon="/images/game/store/chest3"}
