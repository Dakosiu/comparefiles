...
if getAttr and getAttr.condition.name == "creatureAction" and getAttr.targetPos then
	...
	if not hasObstacle(getAttr.targetPos, "solid") then
	    local tmpCaster = Creature(getAttr.casterId)
		if tmpCaster then
	        if tmpCaster:getPathTo(getAttr.targetPos,0,50,true,true) then
			    local tmpPosition = getAttr.targetPos
				if #Tile(tmpPosition):getCreatures() > 0 then
				    tmpPosition = NIIDE_LIB:getClosestFreeSquare(tmpCaster, tmpPosition)
				end
				if tmpPosition then
			        addEvent(function() tmpCaster:teleportTo(tmpPosition,true,getAttr.dashDuration or 0) end,10)
				end
	        end
		end
   end
...
--	else
	...
	if getAttr and getAttr.outfit then
		...
		if getOutfit then
			...
			--print("GET OUtFIT: " .. dump(getOutfit))
		...
		--print("Get Attr: " .. dump(getAttr))
		if not getAttr.animationDuration then 
		  -- print("Tuu back?")
		   return		   
		end
...
function getGlobalAnimation(animationName, creature)
    if animationName == "Basic Attack" then
	    local monster = Monster(creature)
	    if monster then
	       local getConfig = Wikipedia.creatureAnimations[animationName][monster:getName()]
	       return getConfig
	    end
        local p = Player(creature)
	    if p then
	       local basicAttackAnimation = OUTFIT_SYSTEM:getBasicAttackAnimation(p:getVocation():getName(), p:getOutfit().lookType)
		   if basicAttackAnimation then
...
function Creature:sendAttributes(player)
	...
	if self:isPlayer() then
		...
		sendTable.kruumBalance = self:getKruumCoins()
		sendTable.alphaBalance = self:getAlphaPoints()
		sendTable.chests = {} 
		...
		for i, v in ipairs(t) do
		    sendTable.chests[i] = {}
			sendTable.chests[i].ammount = CHEST_SYSTEM:getChestsCount(player, v.name)--self:getChestCount(v.id)--math.max(0, self:getStorageValue(Wikipedia.storageTable["Chest" .. i]))
