...
local CONST_HEALTH_POTION_EFFECT = 77
local potions = {
	...
	[862] = { -- strong health potion
		health = {200, 300},
		vocations = {1, 2, 3, 4},
		effect = CONST_HEALTH_POTION_EFFECT,
		level = 1,
		flask = 7634,
		description = "Health potion"
	},
	[863] = { -- strong mana potion
		mana = {115, 185},
		vocations = {1, 2, 3, 4},
		level = 1,
		flask = 7634,
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	elseif potion.transform then
		...
		if potion.health then
		    --local percentageValue = ((player:getMaxHealth()/100) * 0.5)
			local hpMax = player:getMaxHealth() / 100
			local percentageValue = (hpMax * 50)
			doTargetCombat(player, player, COMBAT_HEALING, percentageValue, percentageValue)
		end

		if potion.mana then
		    --local percentageValue = ((player:getMaxMana()/100) * 0.5)
			local mpMax = player:getMaxMana() / 100
			local percentageValue = (mpMax * 50)
			player:addMana(percentageValue)
			--doTargetCombat(player, player, COMBAT_MANADRAIN, potion.mana[1], potion.mana[2])
		end
