...
local cfg = {
              chance = 70,
              storage = 432474,
			  attempts = { 15, 20, 30, 40, 60, 90 },
            }

local function generateAttempts(item)

      if not item then
	     return nil
	  end
	  
	  local randAttempts = math.random(1, #cfg.attempts)
	  local value = cfg.attempts[randAttempts]
	  
	  
	  item:setCustomAttribute("ATTEMPTS", value)
	  return
end

local function setAttempts(item)
      
	  if not item then
	     return nil
	  end
	  
	  item:setCustomAttribute("ATTEMPTS", item:getCustomAttribute("ATTEMPTS") - 1)
	  return
end

local function getAttempts(item)
      
	  if not item then
	     return nil
	  end
      
	  if not item:getCustomAttribute("ATTEMPTS") then
	     return nil
      end
	  
	  return item:getCustomAttribute("ATTEMPTS")
end

local function getTeleport()
      
	local value = cfg.storage
	  
	if getGlobalStorageValue(value) == 1 then
	   return true
	end
...
local function createTeleport(item)
	  local teleport = doCreateTeleport(11798, PIANO_QUEST.teleport.destination, PIANO_QUEST.teleport.position)
	  if teleport then
	     local value = cfg.storage
		 setGlobalStorageValue(value, 1)
		 addEvent(function()
		    local pos = Position(PIANO_QUEST.teleport.position)
            local thing = Tile(pos):getItemById(11798)
            if thing then
            thing:remove()
            end
			setGlobalStorageValue(value, 0)
			generateAttempts(item)
         end, 1000 * 60 * 1)
	  end
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if item:getId() == 2080 then
	
	   if getTeleport() then
	      player:sendCancelMessage("magical gate already exist.")
	   ...
	   if chance <= cfg.chance then
	      toPosition:sendMagicEffect(CONST_ME_SOUND_GREEN)
	      setAttempts(item)
	   else
	      toPosition:sendMagicEffect(CONST_ME_SOUND_PURPLE)
	   end
