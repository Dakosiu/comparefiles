...
function has_valueM(tbl, value) -- Function to find Index of the Table
...
function printColor(a, color)
	...
	local variableType = type(a)
	if not colors[color] then
		error(string.format("Invalid color: %s", tostring(color)))
	end
	if variableType == "string" then
		print(string.format("\27[1;%sm%s\27[0m", colors[color], a))
	elseif variableType == "number" then
		print(string.format("\27[1;%sm%d\27[0m", colors[color], tonumber(a)))
	else
		print(string.format("\27[1;%sm%s\27[0m", colors[color], tostring(a)))
	end
end
...
debug.sethook(function(event, line)
	if ExtendedDebug and log_slow_exec_currExecProg ~= debug.getinfo(2).source then
		if log_slow_exec_currExecProg ~= "" then
			...
			if execTime >= 0.999 and execTime < 1.500 then
				printColor(string.format(">> Long file %s executed in %.3f ms", log_slow_exec_currExecProg, execTime * 1000), "green")
			elseif execTime >= 1.501 and execTime < 2.000 then
				printColor(string.format(">>> Long file %s executed in %.3f ms", log_slow_exec_currExecProg, execTime * 1000),
					"yellow") --
			elseif execTime >= 2.001 then
				printColor(string.format(">>>> Long file %s executed in %.3f ms", log_slow_exec_currExecProg, execTime * 1000), "red")
			end
		end
	end
	if ExtendedDebug and log_slow_exec_currExecProg ~= debug.getinfo(2).source then
