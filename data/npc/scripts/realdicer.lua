...
local config = {
    bet = {
        min = 50000, -- gold coins // 30k
        max = 50000000,
        win = 180, -- 120% high/low
        winNum = 300, -- 300% numbers
    },

    playerPosition = Position(66, 2075, 8), -- player must stay on this position to talk with npc
    dicerCounter = Position(65, 2074, 8), -- counter position
}
...
local function getBetValue()
    ...
    if tile then
        ...
        for _, item in pairs(items) do
            if table.contains({2160, 2152, 2148, 41529}, item:getId()) then
...
local function createMoney(money)
	...
	local ambers = math.floor(currentMoney / 1000000)
	currentMoney = currentMoney - ambers * 1000000
	while ambers > 0 do
		local cnt = math.min(10000, ambers)
		table[#table + 1] = {41529, cnt}
		ambers = ambers - cnt
	end
