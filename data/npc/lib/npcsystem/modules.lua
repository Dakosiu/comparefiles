...
if Modules == nil then
	...
	function StdModule.promotePlayer(cid, message, keywords, parameters, node)
		...
		if player:isPremium() or not parameters.premium then
			...
			elseif player:getLevel() < parameters.level then
				npcHandler:say(
					"I am sorry, but I can only promote you once you have reached level " .. parameters.level .. ".", cid)
	...
	function StdModule.travel(cid, message, keywords, parameters, node)
		...
		if player:isPremium() or not parameters.premium then
			...
			elseif parameters.cooldown and player:getStorageValue(TravelCooldown) > os.time() then
				npcHandler:say("The ship is not ready, wait a bit, please.", cid)
			elseif not player:removeTotalMoney(parameters.cost) then
				...
				if parameters.direction then
					player:setDirection(parameters.direction)
				end

				position:sendMagicEffect(CONST_ME_TELEPORT)
				destination:sendMagicEffect(CONST_ME_TELEPORT)
				if parameters.cooldown then
					player:setStorageValue(TravelCooldown, os.time() + parameters.cooldown)
					npcHandler:releaseFocus(cid)
				end
			end
		else
	...
	function TravelModule:parseDestinations(data)
		for destination in string.gmatch(data, "[^;]+") do
			...
			if name and x and y and z and cost then
				...
				print("[Warning : " .. Npc():getName() .. "] NpcSystem:", "Parameter(s) missing for travel destination:",
					name, x, y, z, cost, premium)
			end
		end
	end
	...
	function ShopModule:parseBuyable(data)
		...
		for item in string.gmatch(data, "[^;]+") do
			...
			if it:getId() == 0 then
				-- invalid item
				print("[Warning : " .. Npc():getName() .. "] NpcSystem:", "Item id missing (or invalid) for parameter item:",
					item)
			else
			...
			if SHOPMODULE_MODE == SHOPMODULE_MODE_TRADE then
				...
				if name and itemid and cost then
					...
					print("[Warning : " .. Npc():getName() .. "] NpcSystem:", "Parameter(s) missing for item:", name, itemid,
						cost)
				end
			end
		end
	...
	function ShopModule:parseBuyableContainers(data)
		for item in string.gmatch(data, "[^;]+") do
			...
			if name and container and itemid and cost then
				...
				print("[Warning : " .. Npc():getName() .. "] NpcSystem:", "Parameter(s) missing for item:", name, container,
					itemid, cost)
			end
		end
	end
		...
		if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
			...
			if shopItem == nil then
				self.npcHandler.shopItems[#self.npcHandler.shopItems + 1] = {
					...
					sell = -1,
					subType = itemSubType,
					name = realName or ItemType(itemid):getName()
				}
			else
				if cost < shopItem.sell then
					print("[Warning : " ..
					Npc():getName() .. "] NpcSystem: Buy price lower than sell price: (" .. shopItem.name .. ")")
				end
				shopItem.buy = cost
			end
		end
		...
		if SHOPMODULE_MODE ~= SHOPMODULE_MODE_TALK then
			...
			if shopItem == nil then
				...
				if cost > shopItem.buy then
					print("[Warning : " ..
					Npc():getName() .. "] NpcSystem: Sell price higher than buy price: (" .. shopItem.name .. ")")
				end
				shopItem.sell = cost
			end
		end
	...
	function ShopModule:callbackOnBuy(cid, itemid, subType, amount, ignoreCap, inBackpacks)
		...
		if creature:getStorageValue(49821) > os.time() then
			creature:sendCancelMessage("You need to wait one second to buy again.")
			return false
		end

		creature:setStorageValue(49821, os.time() + 1) -- 1 seconds delay

		local totalCost = amount * shopItem.buy
		if inBackpacks then
			totalCost = ItemType(itemid):isStackable() and totalCost + 20 or
				 totalCost + (math.max(1, math.floor(amount / ItemType(ITEM_SHOPPING_BAG):getCapacity())) * 20)
		end
	...
	function ShopModule:callbackOnSell(cid, itemid, subType, amount, ignoreEquipped, _)
		...
		local creature = Player(cid)
		if creature:getStorageValue(49822) > os.time() then
			creature:sendCancelMessage("You need to wait one second to sell again.")
			return false
		end

		creature:setStorageValue(49822, os.time() + 1) -- 1 seconds delay
	...
	function ShopModule.requestTrade(cid, message, keywords, parameters, node)
		...
		openShopWindow(cid, itemWindow,
			function(cid, itemid, subType, amount, ignoreCap, inBackpacks)
				module.npcHandler:onBuy(cid, itemid, subType,
					amount, ignoreCap, inBackpacks)
			end,
			function(cid, itemid, subType, amount, ignoreCap, inBackpacks)
				module.npcHandler:onSell(cid, itemid, subType,
					amount, ignoreCap, inBackpacks)
			end)
		module.npcHandler:say(msg, cid)
		return true
	...
	function ShopModule.onConfirm(cid, message, keywords, parameters, node)
		...
		elseif shop_eventtype[cid] == SHOPMODULE_BUY_ITEM then
			...
			local a, b = doNpcSellItem(cid, shop_itemid[cid], shop_amount[cid], shop_subtype[cid], false, false,
				ITEM_SHOPPING_BAG)
		...
		elseif shop_eventtype[cid] == SHOPMODULE_BUY_ITEM_CONTAINER then
			local ret = doPlayerBuyItemContainer(cid, shop_container[cid], shop_itemid[cid], shop_amount[cid],
				shop_cost[cid] * shop_amount[cid], shop_subtype[cid])
			if ret == true then
				local msg = module.npcHandler:getMessage(MESSAGE_ONBUY)
				msg = module.npcHandler:parseMessage(msg, parseInfo)
				module.npcHandler:say(msg, cid)
